...
Let us recall @exm-bisim-sim of the “trolled philosophers,” where we determined $Q$ and $Q'$ to be non-bisimilar.
This bisimulation game graph for the system looks like this:
...
\begin{adjustbox}{max width=\textwidth, center}
  \begin{tikzpicture}[>->, shorten <=1pt, shorten >=0.5pt, auto, node distance=2cm,
    ...
    \node[posStyle]
      (Att_Qp_Q) [below = 1.6cm of Att_Q_Qp] {$[\literal{Q'}, \literal{Q}]$};
    ...
    \node[posStyle, defender]
      (Dfn_a_q1_qab) [above right = .1cm and 1.5cm of Att_qab_q3] {$(\literal{a}, \literal{q_1}, \literal{q_3})$};
    \node[posStyle, defender]
      (Dfn_bs_q2_qab) [below right = .1cm and 1.6cm of Att_qab_q3] {$(\literal{b}, \literal{q_2}, \literal{q_3})$};
    \node[posStyle, defender]
      (Dfn_t_qab_Q) [above right = .1cm and 1.6cm of Att_Qp_Q] {$(\tau, \literal{q_{AB}}, \literal{Q})$};
    \node[posStyle, defender]
      (Dfn_t_q3_Q) [below right = .1cm and 1.5cm of Att_Qp_Q] {$(\tau, \literal{q_{3}}, \literal{Q})$};
    \node[posStyle]
      (Att_q3_qab) [below right = .1cm and 1.5cm of Dfn_t_qab_Q] {$[\literal{q_3}, \literal{q_{AB}}]$};

    \path
      (Att_Q_Qp) edge
          node {swap} (Att_Qp_Q)
      (Att_Q_Qp) edge
          node {$\literal{Q} \step{\tau} \literal{q_{AB}}$} (Dfn_t_qab_Qp);
  \end{tikzpicture}
\end{adjustbox}
```
:::
...
{{<noindent>}} @lem-attacks-imply-formulas, @lem-formulas-imply-attack, and @lem-game-formula-hml together with the determinacy of games yield an alternative proof of the game characterization @lem-bisim-game-characterization.