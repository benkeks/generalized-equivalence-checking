...
::: {#nte-idea-energy-games .callout-note}
...
::: {#nte-idea-deciding-energy-games .callout-note}
...
This approach is summarized in @fig-spectroscopy-big-picture.
...
\begin{adjustbox}{max width=\textwidth, center}
  \begin{tikzpicture}[>->, shorten <=1pt, shorten >=0.5pt, auto, node distance=2cm,
    ...
    \node[posStyle, initial, initial text={}, label={[text=gray]90:$\hmlands\set{\hmlneg\hmlobs{\tau}\hmlands\set{\hmlneg\hmlobs{\literal{a}}\hmltrue}} \in \observationsvar{(2,2,2)}^\literal{poly-strong}$}]
      (Att_Q_Qp){$[\literal{Q}, \literal{T}]$};
    \node[posStyle, label={[text=gray]270:$\hmlobs{\tau}\hmlands\set{\hmlneg\hmlobs{\literal{a}}\hmltrue} \in \observationsvar{(2,1,1)}^\literal{poly-strong}$}]
    ...
    \node[posStyle, label={[text=gray]95:$\hmlobs{\literal{a}}\hmltrue \in \observationsvar{(1,0,0)}^\literal{poly-strong}$}]
    ...
    \node[posStyle, label={[text=gray]270:$\hmlands\set{\hmlneg\hmlobs{\literal{a}}\hmltrue} \in \observationsvar{(1,1,1)}^\literal{poly-strong}$}]
...
In both cases, the algorithm needs $\bigo{\relsize{G} \cdot \relwidth_{\abstractenergies}(e_{\game}) \cdot N}$ space to store the Pareto fronts.
:::
:::proof
