...
@fig-equivalence-big-picture shows the scaffold of this section to understand behavioral equivalences, instantiated to the classic notion of bisimilarity:
...
{{<noindent>}} There is a canonical example used to discuss equivalences within transition systems, which we want to draw from.
...
A behvioral equivalence formally defines when to consider two processes (or states, or programs) as *equivalent*.
...
In a setting where all components use the same order $(B_1, \leq_1) = \cdots = (B_n, \leq_n) = (C, \leq)$, the relation $\leq$ is then referred to as a *congruence* for $f$.^[
  Sometimes it is also demanded that congruences are equivalence relations, and relations without symmetry are called *pre-*congruences.
  For our discussion, this disambiguation is not necessary.
]
:::

::: {#lem-ccs-congruences}
The operators of CCS are congruences for trace preorder, trace equivalence and bisimilarity.
TODO: Check, prove!
:::
