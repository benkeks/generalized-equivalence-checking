...
\begin{array}{lr}
  ...
  \quad \quad \quad \literal{attacker\_win} := \literal{attacker\_win} \cup \set{g} & \\
  \quad \quad \quad \kw{for} g_{p}\in (\cdot \gamemoveblank g) \colon & \\
  \quad \quad \quad \quad \literal{defender\_options}[g_{p}]:=\literal{defender\_options}[g_{p}]-1 & \\
  \quad \quad \quad \quad \kw{if} g_{p}\in G_\attacker \lor \literal{defender\_options}[g_{p}]=0 \colon & \\
  \quad \quad \quad \quad \quad \literal{todo} := \literal{todo} \cup \set{g_{p}} & \\
