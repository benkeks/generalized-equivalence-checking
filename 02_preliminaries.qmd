...
- **Fixed-point iteration.**
  ...
  Fixed-point iteration computes such relations on finite-state transition systems by initially assuming all states are related and iteratively removing pairs that violate the (bi-)simulation condition.
...
- **Characteristic formulas.**
  Equivalence can also be handled through characteristic modal formulas.
  Intuitively, one constructs a formula for a state that is precisely true for all conceivable equivalent states.
  Checking equivalence with another state then reduces to model checking the formula on that state.
