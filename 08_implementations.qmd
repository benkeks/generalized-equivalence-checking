...
3. **Spectroscopy:**
    ...
    for the respective spectroscopy variants of @sec-strong-spectroscopy and @sec-silent-step-spectroscopy.
...
Among them, the shader version of @sec-gpu-equiv adds the highest level of implementation cleverness with respect to packing of data and parellelization of computation.
...
Experiments by @vogel2024energyGamesWebGPU suggest that the Pareto frontiers usuallly stay sufficiently small that adding the implementation complexity of @dr2000symbolicUpwardClosedSets seems uncalled for.
...
There are easy ways to profit from the communitiy's advances in handling big spaces of possibilities.
The most prominent would be to instantiate the game rules for a transition system, and feed them into an SMT solver.
Alredy @shr1995hornsatGames suggest a comparable approach.
The energy aspect should be perfectly expressible in linear integer arithmetic [cf. @chistikov2024linearIntegerArithmetic].
...
The specifics of individual equivalences are expressed as *signature* that prescribe how to refine equivalence classes in an iterative partition-refinement.
