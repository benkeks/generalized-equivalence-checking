...
An *attacker strategy* is a (partial) function mapping play fragments ending at attacker positions to next positions to move to, $\rel s_\attacker \colon G^*G_\attacker \to G$, where $g_\attacker \gamemoveblank \rel f_\attacker(\rho g_\attacker)$ must hold for all $\rho g_\attacker$ where $\rel s_\attacker$ is defined.

A play ${g_0}{g_1} … ∈ G^{\infty}$ is consistent with an attacker strategy $\rel s_\attacker$ if, for all its prefixes $g_0 … g_i$ ending in $g_i \in G_\attacker$, $g_{i + 1} = \rel s_\attacker(g_0 … g_i)$.

*Defender strategies* are defined analogously, $\rel s_\defender \colon G^*G_\defender \to G$.

If $\rel s$ ensures a player to win, $\rel s$ is called a *winning strategy* for this player.
The player with a winning strategy for $\game(g_{0})$ is said to *win* $\game(g_{0})$.

Usually, we will focus on positional strategies, that is, strategies that only depend on the most recent position, which we will type $\rel s_\attacker \colon G_\attacker \to G$ (or $\rel s_\defender \colon G_\defender \to G$, respectively).
:::
...
This means, for each possible initial position, exactly one of the two players has a positional winning strategy $\rel s$.
...
- If $\rel R$ is a symmetric simulation with $(p,q) ∈ \rel R$,
  ...
  $$\rel s (α, p', q) ≔ [p', \operatorname{choose} q'\ldotp (p', q') ∈ \rel R \land q \step{α} q'].$$
- If there is a positional defender strategy $\rel s$ winning from $[p, q]$,
  ...
  $$\rel{R_s} ≔ \set{(p,q) \mid \text{there is a play $(p_0, q_0),...,(p,q)$ following strategy $\rel s$}}.$$
:::
...
For instance, @peacock2020 implements a game about simulation and bisimulation as well as several weaker notions.
...
Let $\rel s$ be a positional winning strategy for the attacker on ${\game}_\notionname{B}$ from $[p,q]$.
Construct formulas recursively from game positions, ${φ}_{\rel{s}}(g)$, as follows:

$$
φ_{\rel s}([p,q]) ≔ \begin{cases}
  \hmlneg φ_{\rel s}([q,p])  & \text{if } \rel s([p,q]) = [q,p] \\
  \hmlobs{\alpha} \hmlands \set{ φ_{\rel s}([p',q']) \mid q \step{\alpha} q' } & \text{if } \rel s([p,q]) = (\alpha,p',q)
\end{cases}
$$

Then $φ_{\rel s}([p,q])$ is well-defined and distinguishes $p$ from $q$.
Also, $φ_{\rel s}([p,q]) ∈ \observations{\floor{B}}$.
:::
::: proof
1. The given construction is well-defined as $\rel s$ must induce a well-founded order on game positions in order to be attacker-winning.
2. The distinction can be derived by induction on the construction of $φ_{\rel s}([p,q])$.
   TODO \qedhere
:::
...
We have observed that behavioral equivalences can be arranged in hierarchies and how these hierarchies can be handled nicely using modal characterizations to rank distinguishing powers (Subsection [-@sec-perks-modal-characterizations]).
