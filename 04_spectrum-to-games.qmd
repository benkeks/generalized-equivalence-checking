...
\begin{tikzpicture}[auto,node distance=1.1cm,
  ...
  \node[algstep, draw, right of=States, node distance=8.1cm] (Preorders) {Finest notions \\ to preorder $p$ to $q$};
  ...
  \path
    (States) edge[->] node {Spectroscopy problem} (Preorders)
...
How we will employ energy games to solve the spectroscopy problem.
:::

## Energy Games

### Monotonic Energy Games
...
{{<noindent>}} For infinite plays $ρ ∈ G^ω$, we define energy levels to equal $⊥$.
...
We lift positional strategies of @def-strategies to be *energy-positional* in the sense that they pick next moves depending on the current energy level, i.e.\ $\rel f \colon G_\attacker × \abstractenergies \to G$.
...
- Updates are monotonic: If $e_A \leq e_B$, then $\energyupdate(u, e_A) \leq \energyupdate(u, e_B)$.
...
then there is $φ ∈ \strat_\notionname{B}([p, q], e)$ with $φ ∈ \observationsvar{e}^\literal{poly-strong}$, $p ∈ \semantics{φ}$ and $q \notin \semantics{φ}$.
:::
::: proof
We prove the following, more general, property by induction.

1. If $e ∈ \attackerwins^{\game_\notionname{B}^🗲}([p, q])$,
then there is $ψ ∈ \strat_\notionname{B}([p, q], e)$ of form $ψ = \hmlobs{α}φ'$ or $\hmlneg\hmlobs{α}φ'$ with $\hmlands\set{ψ} ∈ \observationsvar{e}^\literal{poly-strong}$, $p ∈ \semantics{ψ}$ and $q \notin \semantics{ψ}$.
2. If $e ∈ \attackerwins^{\game_\notionname{B}^🗲}((α, p', q))$,
then there is $φ ∈ \strat_\notionname{B}((α, p', q), e)$ of form $φ = \hmlands Ψ$ with $φ ∈ \observationsvar{e}^\literal{poly-strong}$, $p' ∈ \semantics{φ}$ and $q \notin \semantics{\hmlobs{α}φ}$.
...
1. Assume $e ∈ \attackerwins^{\game_\notionname{B}^🗲}([p, q])$.
  This must be due to one of the following moves:
    - Case $[p, q] \gamemoveblank^🗲_\notionname{B} [q, p]$ with $e' = \energyupdate((\updmin{1,2},0,-1), e) ∈ \attackerwins^{\game_\notionname{B}^🗲}([q, p])$.
      By induction hypothesis on $[q, p]$, there is $ψ ∈ \strat_\notionname{B}([q, p], e')$ with $\hmlands\set{ψ} ∈ \observationsvar{e'}^\literal{poly-strong}$, $q ∈ \semantics{ψ}$ and $p \notin \semantics{ψ}$.
      Consider the possible forms of $ψ$:
        - $ψ = \hmlobs{α}φ'$.
          ...
          Also, $\hmlands\set{\hmlneg\hmlobs{α}φ'} ∈ \observationsvar{e}^\literal{poly-strong}$, by the calculations of the grammar in @def-polynomial-strong-spectrum.
        - $ψ = \hmlneg\hmlobs{α}φ'$.
          This can only be in $\strat_\notionname{B}([q, p], e')$ due to $φ'$ being in $\strat_\notionname{B}([p, q], \energyupdate((\updmin{1,2},0,-1), e'))$.
          Clearly, $φ'$ must distinguish $p$ from $q$ and $\hmlands\set{φ'} ∈ \observationsvar{e}^\literal{poly-strong}$.
    - Case $[p, q] \gamemoveblank^🗲_\notionname{B} (α, p', q)$ with $p \step{α} p'$ and $e' = \energyupdate((-1,0,0), e) ∈ \attackerwins^{\game_\notionname{B}^🗲}((α, p', q))$.
      By induction hypothesis on $(α, p', q)$, there is $\hmlands Ψ ∈ \strat_\notionname{B}((α, p', q), e')$ and $\hmlands Ψ ∈ \observationsvar{e'}^\literal{poly-strong}$ with $p' ∈ \semantics{\hmlands Ψ}$ and $q \notin \semantics{\hmlobs{α}\hmlands Ψ}$.
      ...
      Also, $\hmlobs{α}\hmlands Ψ ∈ \observationsvar{e}^\literal{poly-strong}$ as $\hmlands Ψ ∈ \observationsvar{e - \unit{1}}^\literal{poly-strong}$.
2. Assume $e ∈ \attackerwins^{\game_\notionname{B}^🗲}((α, p', q))$.
  ...
  For each $q'$, we employ the induction hypothesis to obtain $ψ_{q'} ∈ \strat_\notionname{B}([p', q'], e)$ of form $ψ_{q'} = \hmlobs{α}φ'$ or $\hmlneg\hmlobs{α}φ'$ with $\hmlands\set{ψ_{q'}} ∈ \observationsvar{e}^\literal{poly-strong}$, $p' ∈ \semantics{ψ_{q'}}$ and $q' \notin \semantics{ψ_{q'}}$.
  ...
  Moreover, $(\hmland{q'}{\derivatives{q, α}} ψ_{q'}) ∈ \observationsvar{e}^\literal{poly-strong}$ by the grammar.\qedhere 
:::

::: {#lem-bisim-energy-game-completeness}
If there is $φ ∈ \observationsvar{e}^\literal{poly-strong}$ with $p ∈ \semantics{φ}$ and $q \notin \semantics{φ}$,
then $e ∈ \attackerwins^{\game_\notionname{B}^🗲}([p, q])$.
:::
::: proof
By induction on the grammar of $\observationsvar{e}^{\literal{poly-strong}}$.

Consider the cases of $φ ∈ \observationsvar{e}^\literal{poly-strong}$ with $p ∈ \semantics{φ}$ and $q \notin \semantics{φ}$.

- $φ = \hmltrue$.
  This cannot be the case as $q \notin \semantics{φ}$ and $\semantics{\hmltrue} = \states$.
- $φ = \hmlobs{α}φ'$ with $φ' ∈ \observationsvar{e - \unit{1}}^\literal{poly-strong}$.
...
- $φ = \hmlands\setΨ$ one of the $ψ ∈ Ψ$ must be false for $q$.
  Consider the possible forms of $ψ$:
    - Case $ψ = \hmlobs{α}φ'$ with $φ' ∈ \observationsvar{e - \unit{1}}^\literal{poly-strong}$.
      Then we can apply the same argument as in the case of $φ = \hmlobs{α}φ'$.
    - Case $ψ = \hmlneg\hmlobs{α}φ'$ with $φ' ∈ \observationsvar{(e \sqcap (e_2, \infty, \infty)) - \unit{1} - \unit{3}}^\literal{poly-strong}$.
      Therefore, $\hmlobs{α}φ' ∈ \observationsvar{(e \sqcap (e_2, \infty, \infty)) - \unit{3}}^\literal{poly-strong}$ distinguishes $q$ from $p$.
...
Galois connections can be thought of as a pair of *monotone functions that invert each other up to their partial ordering relations*.
...
- $\gamma \colon A \to C$ is a monotone function, and $α(c) = \inf \set{a \mid c \leq_C \gamma(a)}$ for all $c ∈ C$.
...
with monotone $\abstractupdateinv$-functions for all moves
and with computable undo function $\abstractupdateinv(g, g')(e') = \min \set{e \mid e' \leq \abstractupdate(g, g')(e)}$,
@alg-deciding-energy-games computes the Pareto frontier of minimal attacker winning budgets, solving 
the *monotone energy game winner problem* (@def-energy-game-problem).
:::
:::proof
...
The *height*, $\relheight_B(b)$, below $b$ is defined as the size of the maximal size of chains $B' \subseteq \downset \set{b}$.
:::
...
$$\bigo{ (\relsize{G} + \relheight_{\abstractenergies}(e_{\game})) \cdot \relsize{G} \cdot o_{\gamemoveblank} \cdot (\relwidth_{\abstractenergies}(e_{\game}))^2 \cdot (t_{\abstractupdateinv} + t_{\sup} + \relwidth_{\abstractenergies}(e_{\game}) \cdot t_\leq) }.$$

If $\abstractupdateinv$ is strictly declining, then the following suffices:
...
In both cases, the algorithm needs $\bigo{\relsize{G} \cdot \relwidth_{\abstractenergies}(e_{\game}) \cdot N}$ space to store the Pareto frontiers.
:::
:::proof
...
We denote as *flattened* monotone energy game winner problem the variant of @def-energy-game-problem that outputs
$$
...
Effectively, this decouples the size of Pareto frontiers from the game size.
...
We have now gathered enough material to have this section prove that the spectroscopy problem for the polynomial-time strong spectrum is indeed solvable in polynomial time, thus justifying the name we chose for the spectrum.
...
:::{#rem-shortest-paths}
The energy game problem of this chapter can be seen as a *generalization of the shortest path problem* on graphs to account for two players as well as for much more general distances and cost functions.

To see how, consider energy games on $(\ints, \leq)$ where only deadlock positions belong to the defender
...
In this instance, @alg-deciding-energy-games behaves like the well-know Bellman--Ford algorithm for shortest paths.
:::