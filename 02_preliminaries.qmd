...
1. Should one care about non-determinism in programs? @sec-ccs shows how non-determinism arises naturally in concurrent programs.
...
Then, $\ccs$ is the set of processes, communicating via actions $\ccsactions ≔ \ccschannels \cup \set{ \coaction{c} \mid c ∈ \ccschannels } \cup \set{τ}$, are given by the following grammar:^[
  ...
  For the examples of this thesis, a simple CCS variant without value passing and relabeling suffices.
]
$$
\begin{array}{cllr}
  P \grammardef
...
The smaller bisimulation logic $\observations{\floor{B}}$ will become important again later in the next section (in @sec-bisim-game-hml).
...
- **Partition refinement.**
  The fastest algorithms for bisimilarity use partition refinement [@paigetarjan1987partitionrefinement].
