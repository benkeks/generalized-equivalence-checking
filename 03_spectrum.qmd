...
This chapter presents the spectrum of modal languages from @bisping2023equivalenceEnergyGames in a slightly clarified manner, which itself derives from @bjn2022decidingAllBehavioralEqs.
The alignment to the authorative @glabbeek1990ltbt1 has been studied by @mattes2024expressivePowerHML.
::::
...
So far, we have only looked into the characterization of bisimilarity through the whole of $\hml$ in @thm-hennessy-milner or through $\observations{\floor{B}}$ in @def-game-formulas.
...
The formula distinguishes $\literal P$ from $\literal Q$ on @fig-ts-determinism, setting failures apart from simulation, as  $\literal P \bpreord{S} \literal Q$ (cf. @exm-phil-sim).
...
\begin{tikzpicture}[auto,node distance=2.1cm,
  notion/.style={minimum width=2.5cm, minimum height=.8cm, draw=gray, rectangle,align=center,rounded corners}]
...
Preorder/equivalence hierarchy including failures.
::::
...
When working with vectors, we assume standard notation, in particular, for $n$-dimensional vector addition $(b + c) := (b_1 + c_1, …, b_n + c_n)$.
...
### The Strong Notions of Observability {#sec-strong-notions}
...
and the family of strong observation languages $\observationsvar[\literal{strong}]{N \in \notions^\literal{strong}}$ given by the parameterized grammar starting from $φ^N$:{{<isb primrec Priced_HML formula_of_price>}}

$$
\begin{array}{rcll}
  φ^N & \grammardef & \hmltrue{} & \\
      & \grammaror  & \hmlobs{α}φ^{N - \unitvec{1}} & \\
      & \grammaror  & \hmlands\set{\varphi^{(N-\unitvec{2}) \sqcap (N_3, \infty, \infty, \infty, \infty, \infty)}, \\
      &             & \quad\;
                        ψ^{(N-\unitvec{2}) \sqcap (\infty, \infty, \infty, N_3 \infty, \infty)},
                        ψ^{(N-\unitvec{2}) \sqcap (\infty, \infty, \infty, N_3, \infty, \infty)}, \ldots} & \\
  ψ^N & \grammardef &
      φ^{(N \sqcap (N_4, \infty, \infty, \infty, \infty, \infty))} & \\
      & \grammaror  &
      \hmlneg φ^{(N \sqcap (N_5, \infty, \infty, \infty, \infty, \infty)) - \unitvec{6}} & \\
\end{array}
$$
...
\end{array}
$$
If we reexamine the grammar of $\observations{F}$ in @def-failures-modal, we notice that the formulas it can produce match those of $\observationsvar[\literal{strong}]{(\infty,1,0,0,1,1)}$ of the strong spectrum in @def-strong-spectrum.
:::
...
In my papers, the syntactic expressiveness price of formulas is explicitly defined (instead of using a family of grammars).
In this thesis, $\expr$ is indirectly defined through @def-expressiveness-price.

The expressiveness price $\expr[\literal{strong}]$ of formulas in the strong spectrum (@def-strong-spectrum, we will drop the $\literal{strong}$ for readability) can be recursively characterized as described in @bisping2023equivalenceEnergyGames:

\begin{align*}
  \expr(\hmltrue) & \defeq \zerovec \\
  \expr(\hmlobs{\alpha}\varphi) & \defeq \textcolor{obsColor}{\unitvec{1}} + \expr(\varphi) \\
  \expr(\hmlneg\varphi) & \defeq \textcolor{negsColor}{\unitvec{6}} + \expr(\varphi) \\
  \textstyle\expr(\bigwedge\limits_{i \in I}\psi_i) & \defeq
  \textcolor{conjColor}{\unitvec{2}} + \sup\Biggl(\big\{\!
  \begin{pmatrix}
    0\\
    0\\
    \textcolor{mainPosColor}{\sup_{i \in \mathit{Pos}}}\, \expr_1(\psi_i)\\
    \textcolor{otherPosColor}{\sup_{i \in \mathit{Pos} \setminus \mathit{R}}}\, \expr_1(\psi_i)\\
    \textcolor{negObsColor}{\sup_{i \in \mathit{Neg}}}\, \expr_1(\psi_i)\\
    0
  ...
  \begin{array}{rl}
    ...
    \begin{cases}
      ...
      \{ r \} & \text{for some $r \in \mathit{Pos}$ where $\expr_1(\psi_r)$ maximal for $\mathit{Pos}$.}
    \end{cases}
  \end{array}
\end{align*}

There is a minor divergence from @bisping2023equivalenceEnergyGames in that the present thesis prices $\expr(\hmltrue)$ at $\zerovec$ instead of $\unitvec{2}$.
...
The grammar-view, on the other hand, is particularly nice to see how our $\hml$ sublanguages align to games.
:::
...
:::
::: {.proof}
- Case (1) for traces is trivial.
- Case (2) for simulation is trivial, given flattening of conjunctions.
- For case (3) of bisimilarity,
...
- For case (4) of failures, we must note that $\observationsvar[\literal{strong}]{(\infty, \infty, \infty, \infty,  \infty, \infty)}$ also contains failure observations that end in conjunctions with a $\hmlneg \hmltrue$-conjunct.
  But these are trivially false and thus cannot add to distintiveness.\qedhere
:::
...
\begin{tikzpicture}[auto,node distance=2.5cm,align=center]
  ...
  \node (RT)[below right of=RS]{readiness trace $\notionname{RT}$\\$\infty,\infty,\infty,1,1,1$};
...
To establish the same for the other notions, entails a lot of (repetitive) definitions, which mostly reproduce van Glabbeek's survey [-@glabbeek2001ltbtsiReport].

@mattes2024expressivePowerHML proves in Isabelle/HOL that our coordinate system spectrum matches the distinctiveness of the modal characterizations in [@glabbeek2001ltbtsiReport].^[
  To be precise, @mattes2024expressivePowerHML works with the version of the spectrum of @bisping2023equivalenceEnergyGames.
...
We still should illustrate how *equivalences of decorated traces* are handled:
The idea behind *failure traces* is that one observes a trace of actions and *at every step* a set of actions that are *disabled*.
*Ready traces* work similarly, but with disabled and enabled actions in between.

::: {#exm-failure-trace-distinguished}
#### Failure traces

Consider the following processes, which are slighlty deeper variants of the situation we encountered with $\literal{Q}$ and troll process $\literal{T}$ in @exm-bisim-sim.
$$
\begin{aligned}
  \literal{Q'} := & \ccsprefix{\tau} (\ccsprefix{\literal a} \literal a \ccschoice \ccsprefix{\literal b} \literal{b}) \\
  \literal{T'_{\tau a a}} := & \literal{Q'} \ccschoice \ccsprefix{\tau}\ccsprefix{\literal a} \literal a \\
  \literal{T'_{\tau a}} := & \literal{Q'} \ccschoice \ccsprefix{\tau}\literal a 
\end{aligned}
$$
...
The intuition is that we can observe the trace $\tau \literal{a} \literal{a}$ together with the fact that $\literal b$ is impossible after the first step of $\literal{T'_{\tau a a}}$, but not of $\literal{T'_{\tau a}}$.
...
The important point here is that we can allow *one* arbitrarly deep positive conjunct but prevent any others,
which would underminde the *trace-likeness* of the observation.
This is where it pays off that the spectrum grammar (@def-strong-spectrum) has a more lenient pricing for one of the positive conjuncts.
We will sometimes refer to the deep positive conjunct of a conjunction as the *revival*, because revival equivalence is the first notion of equivalence with this feature.
:::
...
At the same time, some notions could be characteirzed by striclty smaller coordinates due to sublogics of matching expressiveness.
...
- $k$-stratified bisimilarity: $(\infty, k,\infty,\infty,\infty,\infty)$ would be what @milner1989comcon[§ 10.4] calls “stratification of bisimilarity.”
...
::: {#rem-dimensions}
#### Fewer/other dimensions 

In principle, one can choose different dimensions to characterize the strong linear-time--branching-time spectrum.
(Indeed, @bjn2022decidingAllBehavioralEqs, @bisping2023equivalenceEnergyGames and the present thesis all use slightly different dimensions.)

Naturally, one may ask whether the same spectrum can be characterized with less than six dimensions.
...
With a grid of five ternary dimensions, we can recreate the hierarchy of @fig-strong-ltbt-spectrum, and “hard-code” the logis for coordinates, a bit as it happens in @exm-simple-spectrum.
...
::: {#exm-non-ready-sim}
#### Simulation & Failures, donot mean ready-simulation
...
So the question “Which equivalence from a spectrum relates two states?” is too simple---one has to ask: “Which equivalence**s** relate two processes?”
...
\begin{tikzpicture}[]
  \begin{scope}[rotate=30, scale=1]
    ...
    \draw[thick, opacity=0.5, text opacity=.8, xshift=.7505cm, yshift=1.3cm] (2,-.25) -- (2,4) node[pos=1,left] {neg.\ conjunct depth = 2};
...
The best known bisimilarity algorithms for finite-state transition systems take $\bigo{\relsize{\step{}} \log \relsize{\states}}$ time [usually deriving from @paigetarjan1987partitionrefinement].
...
For coarser simulation-like equivalences, the best known algorithms need $\bigo{\relsize{\step{}} \relsize{\states}}$ time [@rt2010efficientSimulation].^[Or $\bigo{\relsize{\step{}} \relsize{\relqotient{S}{\beq{S}}}}$ to name the bound as @rt2010efficientSimulation present it.]
...
As we will see, the perspective of one quantitative problem is more insightful than the view of losely-related, isolated equivalence problems.
...
In this grammar, we force observations in conjuncts of conjunctions and count $\hmltrue$ as part of the $\zerovec$-notion.
...
After that, we will also consider the weak spectrum in the next part.
