...
Consider the following parameters of an energy game problem on $\game^🗲=(G,G_\defender,\gamemoveblank, \abstractupdate)$:
...
$$\bigo{ (\relsize{G} + h(e_{\game})) \cdot \relsize{G} \cdot o_{\gamemoveblank} \cdot (w(e_{\game}))^2 \cdot (t_{\abstractupdateinv} + t_{\sup} + t_\leq) }.$$
