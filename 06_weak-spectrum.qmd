...
This chapter is based on parts of the draft “Deciding all behavioral equivalences at once II: The silent-step spectrum” [@bj2025silentStepSpectroscopyJournal] and of “Characterizing contrasimilarity through games, modal logic, and complexity” [@bm2024contrasimilarityInfComp].
...
At its heart, this chapter aims to find a modal spectrum characterization, where HML grammar and pricing are designed in such a way that we can apply the spectroscopy approach in the following @sec-weak-spectroscopy.
The *top element of the weak spectrum* will be the logic of *stability-respecting branching bisimilarity*.
...
In particular, we try to be much more restricted in the use of special modalities than the presentation of @glabbeek1993ltbt.

::: {#idea-weak-hml .callout-idea .callout-tip}
#### Taking a sublogic to weaken HML
...
@sec-case-studies-other-eqs illustrates the use of some of the weak notions through small case studies from compiler verification and concurrency theory.
...
\begin{tikzpicture}[auto,node distance=1.4cm,
  ...
  \node[] (tta){$\!\ccsprefix{\tau\!}\ccsprefix{\tau\!}\literal{a}\!$};
  \node[below of=tta] (ta){$\ccsprefix{\tau}\literal{a}$};
  \node[below of=ta] (a){$\literal{a}$};
  \node[node distance=2.25cm, below of=a] (Null){$\ccsnull$};
  \node[right of=tta] (tat){$\!\ccsprefix{\tau\!}\ccsprefix{\literal{a}} \tau\!$};
  \node[below of=tat] (at){$\ccsprefix{\literal{a}} \tau$};
  \node[node distance=2.25cm, below of=at] (t){$\tau$};
  \path[->]
    (tta)
    ...
    (t)
      edge node {$\tau$} (Null);
  \begin{pgfonlayer}{bg}
    \draw[blue, double, dotted, double distance=14mm, cap=round]
...
Processes that we would like to equate in weak equivalences (blue groups).
:::
...
For a transition system $\system=(\states,\actions,\step{})$ with an *internal action* $\tau \in \actions$,
...
: $p \stepweak p'$ iff $p \mathrel{\smash{\step{\tau}}^*} p'$, where $\mathrel{\smash{\step{\tau}}^*}$ is the reflexive transitive closure of silent steps.{{<isb inductive LTS lts_tau.silent_reachable>}}
...
Weak word steps
: $p \stepweakword{\vec w} p'$ for $\vec w = (w_1 w_2 \dots w_n) \in \actionsvis^*$ iff there is a path with $p_i \stepweak\step{w_{i+1}} p_{i+1}$ for each $0 \leq i < n$ such that $p_0 = p$ and $p_n \stepweak p'$.
...
$\literal{a}$, $\ccsprefix{\literal{a}} \tau$ and $\ccsnull$ are stable.
...
On systems with finite branching degree, however, the possibility of unbounded $\tau$-trees and infinite $\tau$-chains coincides, and divergence can be captured modally by $\hmland{n}{\nats} \hmlobs{\tau}^n$.
:::
...
The set of *weak traces* of a state $\wtraces{p} \subseteq \actionsvis^*$ is defined as $\vec w \in \wtraces{p}$ iff there is $p'$ such that $p \stepweakword{\vec w} p'$.{{<isb abbreviation LTS lts_tau.weak_traces>}}^[
  The Isabelle theory has a slightly different definition that also allows $\tau$s in the trace words, which do not necessarily stand for proper $\step{\tau}$-steps.
]

Two states are *weakly trace-preordered*, $p \bpreord{WT} q$, precisely if $\wtraces{p} ⊆ \wtraces{q}$.{{<isb definition LTS lts_tau.weakly_trace_preordered>}}
...
Weak simulation preorder, weak simulation equivalence and weak bisimilarity follow analogously to @def-bisimilarity:
...
- $p$ is *weakly bisimilar* to $q$, $p \beq{\observations{WB}} q$, iff there is a *symmetric* weak simulation $\rel R$ (i.e. $\rel R = \inverse{\rel R}$) with $(p, q) ∈ \rel R$.
:::
...
A mutual weak simulation $\rel{R}_{\literal{PQ}}$ to justify this is drawn in dashed blue in @fig-ts-determinism-repeated.

::::{#fig-ts-determinism-repeated fig-pos="h"}
```tikz
...
In HML, this difference could be expressed by $\hmleps \hmlneg \hmleps \hmlobs{\literal{b}}$---we will turn to modal characterizations soon.
:::
...
We will use the following sublogic that, naturally, must be designed to correspond to the strongest weak notion we are interested in.
...
We call a relation $\rel R$ *stability-respecting* iff,
...
or (2) $q \stepweak q' \step{\alpha} q''$ for some $q'$ and $q''$ with $(p, q') \in \rel R$ and $(p', q'') \in \rel R$.{{<isb definition Branching_Bisimilarity lts_tau.branching_simulation>}}
...
and a subgraph-variant $\literal{P}_{\literal{a}\tau\literal{b}} \defeq \literal{a} \ccschoice \ccsprefix{\tau} \literal{b}$, where the $\literal{b}$ is *only* possible after committing to its branch.
...
A formal argument would be that $\identity{\states} \cup \set{(\literal{P_{ab}}, \literal{P}_{\literal{a}\tau\literal{b}}), (\literal{P}_{\literal{a}\tau\literal{b}}, \literal{P_{ab}})}$ is a symmetric weak simulation.
:::

::: {#rem-equivalence-indeed}
#### An equivalence indeed

It is a popular anecdote among those who know branching bisimilarity that, originally, a trivial proof of transitivity was “assumed,” but turned out not to exist.
Six years after branching bisimilarity's inception, “Branching bisimilarity is an equivalence indeed!” by @bastern1996branchingBisimEqIndeed closed the gap with a proof that is surprisingly complex.
...
## Case Studies—and the Need for Other Weak Equivalences {#sec-case-studies-other-eqs}
...
### Parallelizing Compilers---and Contrasimulation
...
The claim behind such optimizations is that the communication behavior of the program stays the same.^[
  ...
  It is the *intention* to change aspects such as performance!
]
So, naturally, the original program and the optimized program should be equivalent with respect to some notion of equivalence.
...
\begin{tikzpicture}[auto,node distance=1.5cm]
  ...
  \path[->]
    (PSeq)
      ...
      edge node {$\tau$} (p2)
    (p1)
      edge[swap, thick] node {$\literal{printO}$} (p3)
    (p2)
      edge[thick] node {$\literal{printO}$} (p4)
    (p3)
      edge[thick, swap] node {$\literal{printA}$} (p5)
    (p4)
      edge[thick] node {$\literal{printB}$} (p5);
\end{tikzpicture}
```
```tikz
\begin{tikzpicture}[auto,node distance=1.5cm]
  ...
  \path[->]
    (PPara)
      edge[swap] node {$\tau$} (p1)
      edge node {$\tau$} (p2)
      edge[thick, swap, pos=.9] node {\scalebox{.9}[1.0]{$\literal{printO}$}} (p23)
    (p1)
      edge[swap, thick] node {$\literal{printO}$} (p3)
    (p2)
      edge[thick] node {$\literal{printO}$} (p4)
...
\begin{tikzpicture}[auto,node distance=1.5cm]
  ...
  \node[right of=PSeq, node distance=5cm] (PPara){$\literal{P_{Para}}$};
  \node[below left of=PPara] (pp1){$\circ$};
  \node[below right of=PPara] (pp2){$\circ$};
  \node[below of=PPara] (pp23){$\literal{p_{ab}}$};
  \node[below of=pp1] (pp3){$\circ\vphantom{\literal{p_a}}$};
  \node[below of=pp2] (pp4){$\circ\vphantom{\literal{p_a}}$};
  \node[below of=pp3, node distance=1cm] (pp31){$\circ$};
  \node[below of=pp4, node distance=1cm] (pp41){$\circ$};
  \node[below right of=pp31] (pp5){$\circ$};
  \path[->]
    (PPara)
      edge[swap] node {$\tau$} (pp1)
      edge node {$\tau$} (pp2)
      edge[thick, swap, pos=.9] node {\scalebox{.9}[1.0]{$\literal{printO}$}} (pp23)
    (pp1)
      edge[swap, thick] node {$\literal{printO}$} (pp3)
    (pp2)
      edge[thick] node {$\literal{printO}$} (pp4)
    (pp23)
      edge[swap] node {$\tau$} (pp3)
      edge node {$\tau$} (pp4)
    (pp3)
      edge[swap] node {$\tau$} (pp31)
    (pp4)
      edge node {$\tau$} (pp41)
    (pp31)
      edge[thick, swap] node {$\literal{printA}$} (pp5)
    (pp41)
      edge[thick] node {$\literal{printB}$} (pp5);
\end{tikzpicture}
...
The action $\literal{printO}$ stands for `print("output:")` announcing the coming output, and $\literal{printA}$ and $\literal{printB}$ are the values of `x` that are actually produced by `print(x)`.

Clearly, $\literal{P_{Seq}}$ and $\literal{P_{Para}}$ have the same weak traces.
But they do not weakly simulate each other:
$\literal{P_{Para}}$ can perform a $\step{\literal{printO}}$-transition, immediately after which $\literal{printA}$ and $\literal{printB}$ are weakly possible at $\literal{p_{ab}}$.
The sequential $\literal{P_{Seq}}$ has no such states.
:::

@bell2013certifiably proposes “eventual bisimilarity” as a one-off bisimulation-like notion to equate $\literal{P_{Seq}}$ and $\literal{P_{Para}}$.
...
and, in case $p'$ is stable, moreover $q' \nostep{\tau}$ and $(p', q') \in \rel R \cap \inverse{\rel R}$.
...
Intuitively, stable bisimilarity is allowed to “skip” intermediate states that would break the bisimulation---such as $\literal{p_{ab}}$ in our example.
...
::::{#fig-divergence-problem .column-margin}
```tikz
\begin{tikzpicture}[auto,node distance=1.5cm]
  \node[] (A){$\literal{A}$};
...
All transitions lead to divergence.
::::

Defined like this, stable bisimilarity would not even imply weak trace equivalence.
Consider the processes $\literal{A}$ and $\literal{B}$ in @fig-divergence-problem.
...
“Our” stable bisimilarity of @def-stablebisim, however, does not suffer from this defect, for it demands matching of all word transitions.
...
::::{#fig-idle-process-abstraction fig-pos="t"}
...
Consequently,  $\literal{P}^\tau_e$ and $\literal{P}^\tau_\ell$ are stable-failure-equivalent.
:::
...
because $\literal{a} \beq{\observations{WB}} \ccsprefix{\tau}\literal{a}$, but $\literal{a} \ccschoice \literal{b} \nbeq{WB} \ccsprefix{\tau}\literal{a} \ccschoice \literal{b}$.
...
## Expressing the Weak Spectrum by Quantities {#sec-weak-spectrum-quantities}
...
5. **Modified:** Nesting depth of *immediate conjunctions* (that are not preceded by $\hmleps$).
6. **Modified:** Maximal modal depth of *positive conjuncts* in conjunctions.^[
  To simplify matters, we drop the distinction of two kinds of positive conjunct depths of @sec-strong-notions.
...
This time, we do this by directly providing an expressiveness pricing metric.
...
$$
\begin{array}{rl}
  \expr[\literal{weak}]\left(\hmltrue\right) & \hspace{-2mm} \defeq
  \zerovec
  \\
  \expr[\varepsilon]\left(\hmltrue\right) & \hspace{-2mm} \defeq
  \zerovec
  \\
  \textstyle\expr[\literal{weak}]\left(\hmleps\chi\right) & \hspace{-2mm} \defeq
  \textstyle\expr[\varepsilon]\left(\chi\right)
  \\
  \textstyle\expr[\literal{weak}]\left(\hmlands\Psi\right) & \hspace{-2mm} \defeq
  \unitvec{5} + \textstyle\expr[\varepsilon]\left(\hmlands \Psi\right)
  \\
  \expr[\varepsilon]\left(\hmlobs{a}\varphi\right) & \hspace{-2mm} \defeq
  \unitvec{1} + \expr[\literal{weak}]\left(\varphi\right)
  \\
  \textstyle\expr[\varepsilon]\left(\hmlands \Psi\right) & \hspace{-2mm} \defeq
  ...
  \end{cases}
  \\
  \expr[\wedge]\left(\hmlneg\hmlobs{\tau}\hmltrue\right) & \hspace{-2mm} \defeq
  (0,0,0,0,0,0,0,1)
  \\
  \expr[\wedge]\left(\hmlneg\varphi\right) & \hspace{-2mm} \defeq
  ...
  \\
  \expr[\wedge]\left(\hmlsoft{\alpha}\varphi\right) & \hspace{-2mm} \defeq
  ...
  \\
  \expr[\wedge]\left(\varphi\right) & \hspace{-2mm} \defeq
...
Using @def-weak-spectrum, we can give coordinates to the common notions of weak behavioral equivalence in @fig-weak-spectrum.
...
:::{#fig-weak-spectrum fig-env='figure*' fig-pos='tp'}

```tikz
%%| image-class: lightbox
\newcommand*{\EqCoords}[1]{\textcolor{gray}{\small $#1$}}
\begin{adjustbox}{center,trim=1cm 0cm, max width=1.05\textwidth}
  \begin{tikzpicture}[auto,node distance=2.5cm,align=center]
    ...
    \node (T)[below right of=F,node distance=2cm]{weak trace $\notionname{WT}$\\
...
The whole list of logics for weak linear-time notions can be found in @fig-weak-grammars-linear.
...
Grammars induced by coordinates for weak linear-time notions.
:::
...
For the abstractions of bisimilarity in @fig-weak-grammars-bisim-like, some more interesting things happen.
...
\parbox{12cm}{
  ...
  \shortintertext{
    ...
    \chi_\notionname{SB} \grammardef &
      \grammarsp \hmlobs{a}\varphi_\notionname{SB}
      \grammaror \hmlands \set{ \mathhl{\hmlneg\hmlobs{\tau}\hmltrue}, \psi_\notionname{SB}, \psi_\notionname{SB}, \ldots } \quad
    \psi_\notionname{SB} \, \grammardef
      \grammarsp \hmlneg\mathdl{\hmleps}\chi_\notionname{SB}
      \grammaror \mathdl{\hmleps\chi_\notionname{SB}}\\
  \shortintertext{
    Stability-respecting delay bisimulation, $\notionname{DB^{sr}}$ at $(\infty,0,\infty,\mathhl{\infty},\infty,\infty,\infty,\infty)$:
  }
    \varphi_\notionname{DB^{sr}} \grammardef &
      \grammarsp \hmleps\chi_\notionname{DB^{sr}}
      \grammaror \mathhl{\hmlands \set{ \psi_\notionname{DB^{sr}}, \psi_\notionname{DB^{sr}}, \ldots }} \\
    \chi_\notionname{DB^{sr}} \grammardef &
      \grammarsp \hmlobs{a}\varphi_\notionname{DB^{sr}}
      \grammaror \hmlands \set{ \psi_\notionname{DB^{sr}}, \psi_\notionname{DB^{sr}}, \ldots }
      \grammaror \hmlands \set{ \mathhl{\hmlneg\hmlobs{\tau}\hmltrue}, \psi_\notionname{DB^{sr}}, \psi_\notionname{DB^{sr}}, \ldots } \\
    \psi_\notionname{DB^{sr}} \grammardef &
      ...
      \grammaror \hmleps\chi_\notionname{DB^{sr}}\\
  \shortintertext{
    Stability-respecting branching bisim., $\notionname{BB^{sr}}$ at $(\infty,\mathhl{\infty},\infty,\mathhl{\infty},\infty,\infty,\infty,\infty)$:
  }
    \varphi_{\notionname{BB^{sr}}} \grammardef &
      \grammarsp \hmleps\chi_{\notionname{BB^{sr}}}
      \grammaror \mathhl{\hmlands \set{ \psi_\notionname{BB^{sr}}, \psi_\notionname{BB^{sr}}, \ldots }} \\
    \chi_{\notionname{BB^{sr}}} \grammardef &
      \grammarsp \hmlobs{a}\varphi_{\notionname{BB^{sr}}}
      \grammaror \hmlands \set{ \psi_{\notionname{BB^{sr}}}, \psi_{\notionname{BB^{sr}}}, \ldots }
      \grammaror \hmlands \set{ \mathhl{\hmlsoft{\alpha}\varphi_{\notionname{BB^{sr}}}}, \psi_{\notionname{BB^{sr}}}, \psi_{\notionname{BB^{sr}}}, \ldots } \\
      & \hphantom{\grammarsp \hmlobs{a}\varphi_{\notionname{BB^{sr}}}} \grammaror \hmlands \set{ \mathhl{\hmlneg\hmlobs{\tau}\hmltrue}, \psi_\notionname{BB^{sr}}, \psi_\notionname{BB^{sr}}, \ldots } \\
    \psi_{\notionname{BB^{sr}}} \grammardef &
      \grammarsp \hmlneg\hmleps\chi_{\notionname{BB^{sr}}}
      \grammaror \hmleps\chi_{\notionname{BB^{sr}}}\\
  \end{align*}
...
Grammars induced by coordinates for weak bisimulation-like notions.
:::
...
The Isabelle theory [@bhlmmb2025weakSpectroscopyIsabelle] features theorems for
...
We do not enforce this and thus our $\mathord{\bpreordvar{\observations{WB}}} \neq \mathord{\beq{\observations{WB}}}$;
for instance, $\ccsprefix{\tau} \literal{a} \bpreordvar{\observations{WB}} \tau \ccschoice \ccsprefix{\tau} \literal{a}$, but $\tau \ccschoice \ccsprefix{\tau} \literal{a} \nbpreordvar{\observations{WB}} \ccsprefix{\tau} \literal{a}$ due to $\hmleps\hmlands\set{\hmlneg\hmleps\hmlobs{\literal{a}}\hmltrue}$.
However, as a distinction by $\hmlneg \varphi$ in one direction implies one by $\varphi$ in the other, we know that this difference is ironed out once we consider the equivalence $\beq{\observations{WB}}$.
:::
...
$$\semantics{\hmleps\hmlands \set{\hmlneg\hmlobs{\tau}, \mathhl{\varphi_1}, \hmlneg\varphi_2,\ldots }} =
\semantics{\hmleps\hmlands \set{\hmlneg\hmlobs{\tau}, \mathdl{\hmlneg\hmleps\hmlands \set{\hmlneg\hmlobs{\tau}, \hmlneg\mathhl{\varphi_1}}}, \hmlneg\varphi_2,\ldots }}.$$
...
Grammars induced by coordinates for weak simulation-like notions.
:::
...
This adds the preliminaries to consider their decision procedures in @sec-weak-spectroscopy.
...
Practical scenarios, such as parallelizing compilers and process abstraction of @sec-case-studies-other-eqs, reveal the possibilities and limitations of the different design decisions behind process equivalences.
...
Our spectrum, however, leaves out some classical notions such as stable failure traces and coupled simulation, which would have called for even more dimensions in the already 8-dimensional pricing metric.^[
  ...
  But it would just not have been economic to add a whole new dimension for this single darling of mine.
]
We will return to the topic of how such notions could be included in @sec-covering-revivals and -@sec-extending-to-other-eqs.
...
Call this avenue for gamification yet another perk of modal characterizations---besides the benefits of built-in transitivity and consistency that we have noticed along the way of this chapter!