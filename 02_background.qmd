...
- $\literal P, \literal Q ∈ \semantics{\hmlobs{τ}\hmlobs{\literal a}}$ as both expose the trace $τ\literal{a}$,
...
We say a sublogic $\observationsvar{X} \subseteq \hml$ *preorders* state $p$ to $q$, $p \bpreordvar{\observationsvar{X}} q$, if no $φ ∈ \observationsvar{X}$ is distinguishing $p$ from $q$.
...
::: {#prp-hml-eq}
Consider an arbitrary HML sublogic $\observationsvar{X} \subseteq \hml$.
