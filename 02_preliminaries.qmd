...
In summary, CCS processes exhibit via the actions $\ccsactions ≔ \ccschannels \cup \set{ \coaction{c} \mid c ∈ \ccschannels } \cup \set{τ}$. 
...
Given an assignment of names to processes, $\ccsasg \colon \ccsnames → \ccs$, the operational semantics ${\step{}_\ccs} ⊆ \ccs × \ccsactions × \ccs$ is defined inductively by the inference rules:^[
  Such rules are read like this:
  “If we can derive the facts above the line, we may infer the relation below.”
  $\mathrm{Pre}$, having no premises, serves as the only axiom of this inference system.
