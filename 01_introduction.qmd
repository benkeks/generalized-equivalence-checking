...
The first time that I formally faced the question has been when working as a student research assistant:
...
CAAL [@aaehlow2015caal] could establish mutual weak similarity, mCRL2 [@groote2014mcrl2; @bgklnvww2019mcrl2tacas] would rule out stable-failure equivalence, and, for contrasimilarity, we would need to implement our own solution because there is no tool supporting it.
...
- @sec-preliminaries lays some foundations and makes precise *how bisimulation games relate to grammars of distinguishing formulas* from Hennessy–Milner modal logic.
  - Main result: Attacker strategies in the bisimulation game ${\game}_\notionname{B}$ correspond to distinguishing formulas in a subset $\observations{\floor{B}}$ of modal logic $\hml$ (@thm-game-formulas).
...
- @sec-approach introduces the approach of *characterizing equivalence spectra through energy games* and *how to decide such games*.
  - Main result: The preorder and equivalence problems in the $\complexityname{P}$-easy part of the strong spectrum $\notions^\literal{peasy}$ are characterized by an energy game version of the bisimulation game ${\game}_\notionname{B}^🗲$ (@thm-bisim-energy-game-characterization).
...
My prior publications are only indirectly connected to this dissertation project.
...
During my dissertation project, I have also supervised several other Bachelor theses, many of which played important roles in shaping the research.
