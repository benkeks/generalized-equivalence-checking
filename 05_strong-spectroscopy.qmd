...
We can input the philosopher processes of @exm-ccs and @exm-deterministic-phil, obtaining a transition system matching @exm-ts:
...
Starting `@compare` (on {{< equivio >}}) triggers a spectroscopy along the lines we have discussed, leading to the output:^[
  The spectroscopy game on this example, can also be explored in a computer game, see @sec-spectro-invaders.
]
...
This comes as no surprise, for we have already discussed in @sec-equivalence-complexities that the spectroscopy problem on the whole strong spectrum is PSPACE-hard.
...
But we can restrict the attacker to only use a polynomially-sized portion of the game.
