...
- when exactly to demand upward closure and monotonicity?
...
Given a partially ordered set of energies $(\abstractenergies, \leq)$, an *energy reachability game* $\game^🗲$ is a reachability game $(G, G_\defender, \gamemoveblank)$ extended by an edge labeling of energy updates $\abstractupdate \colon (\gamemoveblank) \to (\abstractenergies \to (\set{⊥} ∪ \abstractenergies))$, where $⊥ \notin \abstractenergies$ is final in the sense that $\abstractupdate(m)(⊥) = ⊥$.
...
:::
:::proof
This is proved as Theorem 3 in @lemke2024multiweightedEnergyGames.
:::
...
For all updates $u \in \energyupdates$, $\energyupdateinv(u, \cdot)$ and $\energyupdate(u, \cdot)$ form a Galois connection on the domain $\domain(u) = \set{e ∈ \energies \mid \energyupdate(u, e) \neq \bot}$.
:::
:::proof
...
@alg-deciding-energy-games solves the energy game problem on an $N$-dimensional *declining* energy game $\game^🗲=(G,G_\defender,\gamemoveblank, w)$ in
$$
...
For declining energy games, we can fill in several blanks:
...
Effectively, this makes decouples the size of Pareto frontiers from the game size.
In this instance, @lem-declining-update-complexity becomes:

::: {#lem-declining-update-complexity-flattened}
@alg-deciding-energy-games solves the *flattened* energy game problem on an $N$-dimensional declining energy game $\game^🗲=(G,G_\defender,\gamemoveblank, w)$ in
$$
\bigo{ o_{\gamemoveblank} \cdot \relsize{G}^{2N} \cdot (N^2 + 3^{N-1} \cdot N) }
$$
:::

TODO: Also consider space bound!
...
Thus, all that remains to do is to instantiate @lem-declining-update-complexity for the case $N=3$ with the size of ${\game}_\notionname{B}^🗲$ according to @def-bisim-energy-game.

The amount of attacker positions is bounded by $\relsize{\states}^2$.
These positions can initiate up to $\relsize{\states} \cdot \relsize{\step{}}$ simulation challenges, leading to similarly bounded amount of defender positions, which again can be left by $\relsize{\states} \cdot \relsize{\step{}}$ moves.

In total, this amounts to $\bigo{o_{\gamemoveblank_\notionname{B}^🗲}} = \bigo{\relsize{\states} \cdot \relsize{\step{}}}$ and $\bigo{G_\notionname{B}^🗲} = \relsize{\states}^2$.

Inserting the parameters in @lem-declining-update-complexity yields:

$$
\begin{array}{rl}
    & \bigo{ o_{\gamemoveblank} \cdot \relsize{G}^{2N} \cdot (N^2 + \relsize{G}^{N-1} \cdot N) } \\
  = & \bigo{ (\relsize{\states} \cdot \relsize{\step{}}) \cdot (\relsize{\states}^2)^{2 \cdot 3} \cdot (3^2 + (\relsize{\states}^2)^{3-1} \cdot 3)} \\
  = & \bigo{ (\relsize{\states} \cdot \relsize{\step{}}) \cdot \relsize{\states}^{12} \cdot \relsize{\states}^4}\\
  = & \bigo{ \relsize{\states}^{17} \cdot \relsize{\step{}}}.
\end{array}
$$\qedhere
:::
