...
Still, $\game_\literal{leq}$ is quite archetypical for the preorder and equivalence games we will use in the following pages.
:::
...
$$
\begin{array}{lr}
  \kw{def} \literal{compute\_winning\_region}(\game=(G,G_\defender,\gamemoveblank)) \colon & \\
  \quad \literal{defender\_options} :=
    [g \mapsto n \mid g \in G \land n = \relsize{\set{g' \mid g \gamemoveblank g'}}] & \\
  \quad \literal{attacker\_win} := \varnothing & \\
  \quad \literal{todo} := \set{g\in G_\defender \mid \literal{defender\_options}[g]=0} & \\
  ...
  \quad \quad \kw{if} \literal{g} \notin \literal{attacker\_win} \colon & \\
  % note: the if should not be necessary for sets.
  \quad \quad \quad \literal{attacker\_win} := \literal{attacker\_win} \cup \set{\literal{g}} & \\
  \quad \quad \quad \kw{for} \literal{g_{p}} \in (\cdot \gamemoveblank \literal{g}) \colon & \\
  \quad \quad \quad \quad \literal{defender\_options}[\literal{g_{p}}]:=\literal{defender\_options}[\literal{g_{p}}]-1 & \\
  \quad \quad \quad \quad \kw{if} \literal{g_{p}} \in G_\attacker \lor \literal{defender\_options}[\literal{g_{p}}]=0 \colon & \\
  \quad \quad \quad \quad \quad \literal{todo} := \literal{todo} \cup \set{\literal{g_{p}}} & \\
  \quad \attackerwins := \literal{attacker\_win} & \\
...
Intuitively, $\literal{compute\_winning\_region}$ first assumes that the defender were to win everywhere and that each outgoing move of every position might be a winning option for the defender.
Over time, every position that is determined to be lost by the defender is added to a $\literal{todo}$ list.^[
...
Each defender-lost position is added to the $\literal{attacker\_win}$ set.
...
Once we run out of $\literal{todo}$ positions, we know that the attacker has winning strategies exactly for each position we have visited.
...
| $\literal{g}$ | $\literal{defender\_options}$ | $\literal{todo}$ |
|-------|------|------|
