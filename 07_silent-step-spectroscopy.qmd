...
For our standard philosopher system, @exm-phil-weak-eqs has determined that $\literal P$ and $\literal Q$ are not weakly bisimilar, but weakly similar (and thus weakly trace-equivalent).

The stable failure $\hmleps\hmlands\set{\hmlneg\hmleps\hmlobs{\literal{a}}}$ discussed in @sec-abstraction-failures corresponds to the following game moves, needing budget $(1,0,0,1,0,0,1,1)$ for delay--procrastination--stable conj.--stable conj. answer--negative conjunct--observation--finishing:
$$
\begin{array}{rcl}
