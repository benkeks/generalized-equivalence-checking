...
Precisely if $\gamma \colon A \to C$ is a monotone function, and $α(c) = \inf \set{a \mid c \leq_C \gamma(a)}$ for all $c ∈ C$,
...
$$\energyupdateinv(u, e') := \sup( \set{e} ∪ \set{m(i) \mid \exists D. u_i = \mathtt{min}_D} )$$
with $e_i = \begin{cases}
  e'_i - u_i  & \text{if } u_i ∈ \set{0, -1}\\
  e'_i        & \text{otherwise}
\end{cases}$ for all $i$ and<br>
with $(m(i))_j = \begin{cases}
  e'_i        & \text{for } u_i = \mathtt{min}_D \text{ and } j ∈ D \\
  0           & \text{otherwise}
