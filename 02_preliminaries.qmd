...
The program described by the transitions from $\literal{P}$ chooses non-deterministically during a $τ$-step between two options and then offers only either $\literal a$ *or* $\literal b$.
...
The action $τ$ (the greek letter “tau”) will in later chapters stand for *internal behavior* and receive special treatment in @sec-weak-spectrum.
...
There are several reasons why computer scientist did not content themselves with trace equivalence when studying interactive systems.
...
For instance, assume there is a user that wants to communicate $(\dots \ccspar \ccsprefix{\coaction{\literal{b}}}\ccsprefix{\literal{success}} \ccsnull) \ccsrestrict \set{\literal{a}, \literal{b}}$.
In interactions with $\literal{Q}$, they will always reach $\literal{success}$.
With $\literal{P}$, there is a possibility of ending up deadlocked in $((\ccsnull \ccspar \literal a \ccspar \literal{P_B} ) \ccsrestrict \set{\literal{fork}} \ccspar \ccsprefix{\coaction{\literal{b}}}\ccsprefix{\literal{success}} \ccsnull) \ccsrestrict \set{\literal{a}, \literal{b}}$, never achieving success.
...
We also have seen with example @exm-phil-sim that this hierarchy (visualized in @fig-core-hierarchy) is strict between trace and simulation preorder in the sense that there exist $p,q$ with $p \bpreord{T} q$ but not $p \bpreord{S} q$.
...
\begin{tikzpicture}[auto,node distance=2cm]
  ...
  \node[state, below right of=q0] (q3){$\troll$};
  \path[->]
    (q0)
