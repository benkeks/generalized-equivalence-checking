...
We say that formula $φ ∈ \hml$ *distinguishes* state $p$ *from* state $q$ if $p ∈ \semantics{φ}$ but $q \notin \semantics{φ}$.{{<isb abbreviation Hennessy_Milner_Logic lts.distinguishes>}}

We say a sublogic $\observationsvar{} ⊆ \hml$ *preorders* state $p$ to $q$, $p \bpreordvar{\observationsvar{}} q$, if no $φ ∈ \observationsvar{}$ is distinguishing $p$ from $q$.{{<isb definition Hennessy_Milner_Logic lts.hml_preordered>}}
If the preordering goes in both directions, we say that $p$ and $q$ are equivalent with respect to sublogic $\observationsvar{}$, written $p \beqvar{\observationsvar{}} q$.{{<isb definition Hennessy_Milner_Logic lts.hml_equivalent>}}
:::
...
Then, $\bpreordvar{\observationsvar{}}$ is a preorder, and $\beqvar{\observationsvar{}}$ an equivalence relation.{{<isb lemma Hennessy_Milner_Logic lts.hml_equiv_equiv>}}
:::

::: {#lem-hml-eq-sim}
Hennessy–Milner logic equivalence $\beqvar{\hml}$ is a simulation relation.{{<isb lemma Hennessy_Milner_Logic lts.hml_equiv_sim>}}
:::
...
If $p ∈ \semantics{φ}$ and $p \beq{B} q$ then $q ∈ \semantics{φ}$.{{<isb lemma Hennessy_Milner_Logic lts.hml_bisim_invariant>}}
:::
...
Bisimilarity and HML equivalence coincide, that is, $p \beq{B} q$ precisely if $p \beq{\hml} q$.{{<isb theorem Hennessy_Milner_Logic lts.Hennessy_Milner_theorem>}}
:::
...
If $\observationsvar{} \subseteq \observationsvar{}'$ then $p \bpreordvar{\observationsvar{}'} q$ implies $p \bpreordvar{\observationsvar{}} q$ for all $p,q$.{{<isb lemma Hennessy_Milner_Logic lts.hml_preorder_contraposition>}}
:::
...
In Chapter [-@sec-spectrum], we will discuss how the hierarchy of behavioral equivalences can be nicely and uniformly understood if viewed through the modal lense.
...
:::: {#fig-bisim-game-even-odd}
  ```{mermaid}
  flowchart TD
    eo["p<sub>even</sub>, p<sub>odd</sub>"] -->|challenge| too(["α, p<sub>odd</sub>, p<sub>odd</sub>"]) -->|answer| oe["p<sub>odd</sub>, p<sub>even</sub>"] -->|challenge| tee(["α, p<sub>even</sub>, p<sub>even</sub>"]) -->|answer| eo -->|swap| oe -->|swap| eo
  ```
...
TODO non-bisim example

::: {#lem-bisim-game-characterization}
...
- If there is a positional defender strategy $\rel f$ winning from $[p, q]$,
  ...
  $$\rel R ≔ \set{(p,q) \mid \text{there is a play $(p_0, q_0),...,(p,q)$ following strategy $\rel f$}}.$$\qedhere
:::

### How Bisimulation Game and HML Are Connected
...
2. The defender decides where to start the HML game:
    1. Either at $(p, \hmlneg \varphi)$ (claiming $\varphi$ to be non-distinguishing because it is untrue for $p$)
...
This naive game, too, has the property that the defender from $[p,q]$ wins if $p \beq{B} q$.
The downside of the game is that the attacker has infinitely many options $\varphi \in \hml$ to pick from!

The proper bisimulation game of @def-bisim-game is finite for finite transition systems.
Therefore, it induces decision procedures.

We will now argue that the *bisimulation game actually is a variant of the naive game*, where (1) the attacker names its formula *gradually*, and (2) the formulas stem from a *functionally complete subset* $\observations{\floor{B}} \subseteq \hml$.
To this end, we will show that attacker's winning strategies imply distinguishing formulas, and that a distinguishing formula from $\observations{\floor{B}}$ certify the existence of winning attacker strategies.

::: {#def-game-formulas}

$\observations{\floor{B}} \subseteq \hml$

:::

::: {#lem-attacks-imply-formulas}
Let $\rel f$ be a deterministic winning strategy for the attacker on ${\game}_\notionname{B}$ from $[p,q]$.
Construct formulas recursively from game positions, ${\varphi}_{\rel{f}}(g)$, as follows:

$$
\varphi_{\rel f}([p,q]) ≔ \begin{cases}
  \hmlneg \varphi_{\rel f}([q,p])  & \text{if } \rel f([p,q]) = [q,p] \\
  \hmlobs{\alpha} \hmlands \set{ \varphi_{rel f}([p',q']) \mid q \step{\alpha} q' } & \text{if } \rel f([p,q]) = (\alpha,p',q)
\end{cases}
$$

Then $\varphi_{\rel f}([p,q])$ is well-defined and distinguishes $p$ from $q$.
:::
::: proof
...
2. The distinction can be derived by induction on the construction of $\varphi_{\rel f}([p,q])$.
:::

::: {#lem-formulas-imply-attack}
$\varphi \in \observations{\floor{B}} \subseteq \hml$.
Construct non-deterministic $\rel F_{\varphi}$
:::