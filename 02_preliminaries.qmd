...
This chapter is based on my introductory lectures and Isabelle-theory on behavioral equivalences, given in the Master course “Research at Work” at TU Berlin in Winter 2023/24.
We pursue to be mostly compatible with the textbook *Reactive Systems* @ails2007reactiveSystems.
::::
...
\begin{tikzpicture}[auto,node distance=1.1cm,
  ...
  \node[below of=Distinction, text=Peach, rotate=270, label=90:{Section \ref{sec-bisim-game-hml}}] (Giff) {$\iff$};
  ...
  \node[text=Fuchsia, right=2mm of Distinction] (Sec23) {Section \ref{sec-hml}};
...
The two core insights this chapter aims to seed for the upcoming chapters are:
...
To provide them with a bridge, we will start with relational and denotational definitions---but once we have crossed this bridge, we will not go back.
...
Tom Henzinger employing @exm-ts during CAV'23 to stress that “everything is a game.”
...
Consider the transition system $\system_\literal{PQ} = (\set{\literal P, \literal{p_a}, \literal{p_b}, \literal{p_1}, \literal{p_2}, \literal Q, \literal{q_{ab}}, \literal{q_1}, \literal{q_2}}, \set{\literal a, \literal b, τ}, \step{}_\literal{PQ})$ given by the following graph:
...
The program described by the transitions from $\literal{P}$ chooses non-de{{<shy>}}terministically during a $τ$-step between two options and then offers only *either* $\literal a$ *or* $\literal b$.
...
Then, $\ccs$ is the set of processes, communicating via actions $\ccsactions ≔ \ccschannels \cup \set{ \coaction{c} \mid c ∈ \ccschannels } \cup \set{τ}$, are given by the following grammar:^[
  To those familiar with CCS dialects:
  For the examples of this thesis, a simple CCS variant without value passing and renaming suffices.
]
$$
\begin{array}{cllr}
  P \grammardef
...
\end{array}
$$
Intuitively, action $c$ represents receiving and its “coaction” $\coaction c$ expresses sending on channcel $c \in \ccschannels$.
...
\begin{tikzpicture}[auto,node distance=1.7cm]
  \node[state] (Pa){$\literal{P_A}$};
  \node[state, below of=Pa] (a){$\literal{a}$};
  \node[state, below of=a] (Null){$\ccsnull$};
  \path[->]
    (Pa)
...
Philosopher $\literal{P_A}$, on their own.
:::::
...
So, consider two philosophers $\literal{P_A}$ and $\literal{P_B}$ who want to grab a resource $\literal{fork}$, in order to eat, which we model as receiving communication.
...
\end{gathered}
$$
A transition system representation of $\literal{P_A}$'s behavior can be seen in the @fig-isolated-philosopher.
...
The formal relationship between process terms $\ccs$ and their transition system is given by the following semantics.

::: {#def-ccs-semantics}
#### CCS semantics

Given an assignment of names to processes, $\ccsasg \colon \ccsnames → \ccs$, the operational semantics ${\step{}_\ccs} ⊆ \ccs × \ccsactions × \ccs$ is defined inductively by the rules:
...
\noindent
A process $P ∈ \ccs$ now denotes a position in the transition system $(\ccs, \ccsactions, \step{}_\ccs)$.
:::
...
For instance, the transition $\literal P \step{τ} \literal{p_a}$ in the CCS system of @exm-ts would be justified by the derivation in @fig-deriving-transition.

::: {#fig-deriving-transition .column-body-outset}
$$
\inferruleapp[Rec]{
  \hspace{-2em}
  \inferruleapp[Res]{
...
Non-determinism like in $\literal P$ of @exm-ts can be understood as a natural phenomenon in models with concurrency.
...
Accordingly, comparing two programs in terms of the sequences of input/output events they might expose, referred to as their *traces*, is a natural starting point to talk about behavioral equivalences.
...
If we insert $\literal{Q}$ into this context, the choice after the collective $\literal{fork}$-acquision is resolved to the $\literal{B}$-path.
So the overall system choses for $\literal{B}$ to eat, and the lecture happens:
...
But there moreover is a possibility of $\literal{A}$ obtaining the fork, which prevents $\literal{B}$ from eating.
...
The following relations are simulations on the of @exm-ts:
...
- the universal relation between all terminal states $\rel R_\mathrm{term} ≔ \set{\literal{p_1}, \literal{p_2}, \literal{q_1}, \literal{q_2}} \times \set{\literal{p_1}, \literal{p_2}, \literal{q_1}, \literal{q_2}}$,
...
@exm-phil-sim shows that similarity and bisimilarity do not imply trace equivalence.
Still, the notions are connected.
...
```tikz
\begin{tikzpicture}[auto,node distance=2cm]
  \node[state] (q0){$\literal{T}$};
  \node[state, below left of=q0] (qAB){$\literal{t_{ab}}$};
  \node[state, below left of=qAB] (q1){$\literal{t_{1}}$};
  \node[state, below right of=qAB] (q2){$\literal{t_{2}}$};
...
```tikz
\begin{tikzpicture}[auto,node distance=2cm]
  \node[state] (pEven){$\literal{p_{even}}$};
  \node[state, below of=pEven] (pOdd){$\literal{p_{odd}}$};
  \path[->]
    (pEven)
...
This thesis builds on the classic idea that behavioral equivalences form a *spectrum* between linear-time notions and branching-time notions.
...
CSP (“Communicating Sequential Processes”) is due to Tony Hoare, another Turing award winner.
...
The first comprehensive study on the correspondence between the hierachy of equivalences and algebraic laws has been dubbed “linear-time–branching-time spectrum” by @glabbeek1990ltbt1.

Historically, therefore, this thesis originates from the variants of models between the works of two Turing award winners: Milner's CCS and Hoare's CSP.
...
We can thus merge them into one state $\literal{t_{12}^m}$ as depicted in @fig-bisim-min-ts, without affecting the behavioral properties of the other states with respect to bisimilarity, that is, $\literal{t_{ab}} \beq{B} \literal{t_{ab}^m}$ and $\literal{T} \beq{B} \literal{T^m}$.

:::: {#fig-bisim-min-ts .column-margin}
```tikz
\begin{tikzpicture}[auto,node distance=2cm]
  \node[state] (q0){$\literal{T^m}$};
  \node[state, below left of=q0] (qAB){$\literal{t^m_{ab}}$};
...
Combining the bisimulation invariance for one direction and that $\beq{\hml}$ is a symmetric simulation (@prp-hml-eq and @lem-hml-eq-sim) for the other, we obtain that $\hml$ precisely characterizes bisimilarity:
...
We denote by $\game(g_0)$ the game played from starting position $g_0 ∈ G$.
:::
...
\begin{tikzpicture}[>->, shorten <=1pt, shorten >=0.5pt, auto, node distance=2cm, 
  ...
  \node[posStyle, defender] (p1){$1$};
  \node[posStyle, below left of=p1] (p2a){$\literal{2a}$};
  \node[posStyle, below right of=p1] (p2b){$\literal{2b}$};
  \node[posStyle, defender, below of=p2a] (p3){$\literal{3}$};
  \path[->]
    (p1)
      edge node {} (p2a)
      edge node {} (p2b)
...
and they can win using a positional strategy.^[
  ...
  Parity games extend reachability games by number-coloring positions.
...
::: {#fig-bisim-sim-game fig-pos='t' fig-env="figure*" .column-body-outset}
```tikz
...
\begin{adjustbox}{max width=\linewidth, center}
  \begin{tikzpicture}[>->, shorten <=1pt, shorten >=0.5pt, auto, node distance=2cm,
    ...
    \path
      ...
      (Att_qab_q3)
        edge[bend right = 35] node {swap} (Att_q3_qab)
      (Att_q3_qab)
        edge[bend left = 30] node {} (Att_qab_q3)
      (Dfn_t_qab_Qp)
      ...
      ([yshift=.15cm]Att_q3_q1.east)
        edge[bend right = 30] node[swap] {swap} ([yshift=-.15cm]Att_q1_q3.east)
      (Att_q1_q3.east)
        edge[bend left = 35] node {} (Att_q3_q1.east)
...
- If there is a positional defender strategy $s$ winning from $[p, q]$,
  ...
  $$\rel{R_s} ≔ \set{(p,q) \mid \text{there is a play $(p_0, q_0),\dots,(p,q)$ following strategy $s$}}.\qedhere$$
:::
...
%%| image-class: lightbox
  \begin{tikzpicture}[>->, shorten <=1pt, shorten >=0.5pt, auto, node distance=2cm,
    ...
    \node[posStyle, initial, initial text={}, label={100:$\textcolor{red}{\hmlneg\hmlobs{\tau}\hmlands\set{\hmlneg\hmlobs{\literal{a}}\hmltrue}}$}]
      (Att_Q_Qp){$[\literal{Q}, \literal{T}]$};
    \node[posStyle, label={100:$\textcolor{red}{\hmlobs{\tau}\hmlands\set{\hmlneg\hmlobs{\literal{a}}\hmltrue}}$}]
      (Att_Qp_Q) [below = 2cm of Att_Q_Qp] {$[\literal{T}, \literal{Q}]$};
    \node[posStyle, defender, defWins]
      (Dfn_t_qab_Qp) [right = 1.5cm of Att_Q_Qp] {$(\tau, \literal{q_{ab}}, \literal{T})$};
    ...
    \node[posStyle, label={100:$\textcolor{red}{\hmlobs{\literal{a}}\hmltrue}$}]
      (Att_qab_q3) [above right = .1cm and 1.5cm of Dfn_t_qab_Qp] {$[\literal{q_{ab}}, \troll]$};
    ...
    \node[posStyle, defender]
      (Dfn_t_q3_Q) [below = 1cm of Dfn_t_qab_Q] {$(\tau, \troll, \literal{Q})$};
    \node[posStyle, label={100:$\textcolor{red}{\hmlneg\hmlobs{\literal{a}}\hmltrue}$}]
...
:::
::: proof
1. The given construction is well-defined as $s$ must induce a well-founded order on game positions in order for the attacker to win, and the recursive invocations remain in the attacker winning strategy.
...
@thm-game-formulas is just another way of gluing together the Hennessy–Milner theorem on bisimulation (@thm-hennessy-milner) and Stirling's bisimulation game characterization (@thm-bisim-game-characterization), modulo the determinacy of games.
...
: Solving the game of @def-game-formulas {#tbl-game-example-execution}
...
\begin{tikzpicture}[auto,node distance=2.7cm,
  ...
  \path
    ...
    (BisimGame) edge[->] node {Alg.~\ref{alg-deciding-games}} (WinningRegion)
    ...
    (Formula) edge[->, swap] node[align=left] {Check distinction using \\ HML game Def~\ref{def-hml-game} + Alg.~\ref{alg-deciding-games}\vphantom{p}} (No);
  \draw
...
There are different frameworks for deciding behavioral equivalences that this thesis will not be using.

- *Fixed-point iteration.*
  Simulation-like behavioral equivalences and preorders can be expressed as greatest fixed points of monotonic functions.
  Thus, fixed-point iteration can compute such relations on finite-state transition systems.
  Intuitively, one first assumes that all states are related and then iteratively throws out pairs that immediately violate the (bi-)simulation condition, until there are no such pairs.
  In effect, the same chain reaction happens that @alg-deciding-games performs on the bisimulation game.
  Compared to the game approach, fixed point iteration usually is slower as it possesses less knowledge about logical connections between pairs in the candidate relation.
- *Partition refinement.*
  The fastest algorithms for bisimilarity use partition refinement.
  Paritions represent equvialence relations as colorings on graphs, which is linear in the state space $\relsize{\states}$ (opposed to quadratic combinations for pairs in relations and attacker positions).
  Intuitively, a similar refinement happens as in the general fixed-point iteration, repeatedly splitting up parts of the partitions.
  The splits are also informed by modal distinctions as explicated in @cleaveland1991explainingBisimInequiv.
...
- *Reduction to bisimilarity.*
  Other equivalences are usually checked by transforming the transition system in some way to make bisimilarity and the respective equivalence coincide on the system.
  A standard example would be to make the transition system deterministic via subset construction, after which trace equivalence can be checked as bisimilarity.
- *Term rewriting.*
  If working on process-algebraic terms, equivalence can also be established through equational reasoning.
  This approach requires a behavioral *congruence* with an axiomatic characterization which proccesses to consider as equal.
...
- *Characteristic formulas.*
  Equivalence can also be checked through characteristic modal formulas.
  Intuitively, one constructs a formula for a state that is precisely true for all conceivable equivalent states.
  Equivalence checking with another state then reduces to model checking the formula on that state.
  This approach requires a more complex language of *recursive* modal formulas.
  It seems to be mostly academic in the sense that no relevant tool actually uses it to decide equivalences.

{{< paragraph_heading "Modal logics point the way." >}}
We have observed that behavioral equivalences form hierarchies and that these hierarchies are handled nicely using modal characterizations to rank distinguishing powers (Subsection [-@sec-perks-modal-characterizations]).
...
In later chapters, we will rather derive equivalence games from grammars of modal formulas (@idea-modal-first), to generalize the framework of @fig-game-algo-overview for whole spectra of equivalences.
