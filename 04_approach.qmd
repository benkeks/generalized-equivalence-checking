...
\begin{adjustbox}{max width=\textwidth, center}
  \begin{tikzpicture}[>->, shorten <=1pt, shorten >=0.5pt, auto, node distance=2cm,
    ...
    \node[posStyle, initial, initial text={}, label={[text=red]90:$\hmlands\set{\hmlneg\hmlobs{\tau}\hmlands\set{\hmlneg\hmlobs{\literal{a}}\hmltrue}} \in \observationsvar[\literal{poly-strong}]{(2,2,2)}$}]
      (Att_Q_Qp){$[\literal{Q}, \literal{T}]$};
    \node[posStyle, label={[text=red]270:$\hmlobs{\tau}\hmlands\set{\hmlneg\hmlobs{\literal{a}}\hmltrue} \in \observationsvar[\literal{poly-strong}]{(2,1,1)}$}]
    ...
    \node[posStyle, label={[text=red]95:$\hmlobs{\literal{a}}\hmltrue \in \observationsvar[\literal{poly-strong}]{(1,0,0)}$}]
    ...
    \node[posStyle, label={[text=red]270:$\hmlands\set{\hmlneg\hmlobs{\literal{a}}\hmltrue} \in \observationsvar[\literal{poly-strong}]{(1,1,1)}$}]
...
Thus, $\notions^{\literal{poly-strong}}_{\literal{Q}, \literal{T}} = \set{N ∈ \notions^\literal{poly-strong} \mid (2,2,2) \nleq N}$ is the solution for the (polynomial-time strong) spectroscopy problem.
...
1. Assume $e ∈ \attackerwins^{\game_\notionname{B}^🗲}([p, q])$.
  This must be due to one of the following moves:
    - Case $[p, q] \gamemoveblank^🗲_\notionname{B} [q, p]$ with $e' = \energyupdate((\updmin{1,2},0,-1), e) ∈ \attackerwins^{\game_\notionname{B}^🗲}([q, p])$.
      ...
      Consider the possible forms of $ψ$:
        ...
        - $ψ = \hmlneg\hmlobs{α}φ'$.
          ...
          Clearly, $\hmlobs{α}φ'$ must distinguish $p$ from $q$ and $\hmlands\set{\hmlobs{α}φ'} ∈ \observationsvar[\literal{poly-strong}]{e}$.
    - Case $[p, q] \gamemoveblank^🗲_\notionname{B} (α, p', q)$ with $p \step{α} p'$ and $e' = \energyupdate((-1,0,0), e) ∈ \attackerwins^{\game_\notionname{B}^🗲}((α, p', q))$.
      ...
      Also, $\hmlobs{α}\hmlands Ψ ∈ \observationsvar[\literal{poly-strong}]{e}$ as $\hmlands Ψ ∈ \observationsvar[\literal{poly-strong}]{e - \unitvec{1}}$.
...
If there is $φ ∈ \observationsvar[\literal{poly-strong}]{N}$ with $p ∈ \semantics{φ}$ and $q \notin \semantics{φ}$,
then $N ∈ \attackerwins^{\game_\notionname{B}^🗲}([p, q])$.
:::
::: proof
By induction on the grammar of $\observationsvar[\literal{poly-strong}]{N}$.

Consider the cases of $φ ∈ \observationsvar[\literal{poly-strong}]{N}$ with $p ∈ \semantics{φ}$ and $q \notin \semantics{φ}$.

- $φ = \hmltrue$.
  This cannot be the case as $q \notin \semantics{φ}$ and $\semantics{\hmltrue} = \states$.
- $φ = \hmlobs{α}φ'$ with $φ' ∈ \observationsvar[\literal{poly-strong}]{N - \unitvec{1}}$.
...
- $φ = \hmlands Ψ$ one of the $ψ ∈ Ψ$ must be false for $q$.
  Consider the possible forms of $ψ$:
    - Case $ψ = \hmlobs{α}φ'$ with $φ' ∈ \observationsvar[\literal{poly-strong}]{N - \unitvec{1}}$.
      Then we can apply the same argument as in the case of $φ = \hmlobs{α}φ'$.
    - Case $ψ = \hmlneg\hmlobs{α}φ'$ with $φ' ∈ \observationsvar[\literal{poly-strong}]{(N \sqcap (N_2, \infty, \infty)) - \unitvec{1} - \unitvec{3}}$.
      Therefore, $\hmlobs{α}φ' ∈ \observationsvar[\literal{poly-strong}]{(N \sqcap (N_2, \infty, \infty)) - \unitvec{3}}$ distinguishes $q$ from $p$.
...
#### $\notions^\literal{poly-strong}$-characterization
...
Thereby, @prob-spectroscopy-problem for $\notions^\literal{poly-strong}$ on a transition system reduces to @prob-energy-game-problem on the corresponding bisimulation energy game $\game_\notionname{B}^🗲$.
