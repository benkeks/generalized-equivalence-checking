...
then there is $φ \in \strat_\notionname{B}([p, q], e)$ with $φ \in \observationsvar{e}^\literal{poly-strong}$, $p \in \semantics{φ}$ and $q \notin \semantics{φ}$.
:::
::: proof
We prove the following, more general, property by induction.

1. If $e \in \attackerwins^{\game_\notionname{B}^🗲}([p, q])$,
then there is $\psi \in \strat_\notionname{B}([p, q], e)$ of form $\psi = \hmlobs{α}φ'$ or $\hmlneg\hmlobs{α}φ'$ with $\hmlands\set{\psi} \in \observationsvar{e}^\literal{poly-strong}$, $p \in \semantics{\psi}$ and $q \notin \semantics{\psi}$.
2. If $e \in \attackerwins^{\game_\notionname{B}^🗲}((α, p', q))$,
then there is $φ \in \strat_\notionname{B}((α, p', q), e)$ of form $φ = \hmlands \Psi$ with $φ \in \observationsvar{e}^\literal{poly-strong}$, $p' \in \semantics{φ}$ and $q \notin \semantics{\hmlobs{α}φ}$.
  ...
  This must be due to one of the following moves:
    - Case $[p, q] \gamemoveblank^🗲_\notionname{B} [q, p]$ with $e' = \energyupdate((\updmin{1,2},0,-1), e) \in \attackerwins^{\game_\notionname{B}^🗲}([q, p])$.
      ...
      Consider the possible forms of $\psi$:
        - $\psi = \hmlobs{α}φ'$.
          By @def-strategy-formulas-poly of $\strat_\notionname{B}$, we obtain $\hmlneg\hmlobs{α}φ' \in \strat_\notionname{B}([p, q], e)$.
          Because of the semantics of $\hml$, $\hmlneg\hmlobs{α}φ'$ must distinguish $p$ from $q$.
          Also, $\hmlands\set{\hmlneg\hmlobs{α}φ'} \in \observationsvar{e}^\literal{poly-strong}$, by the calculations of the grammar in @def-polynomial-strong-spectrum.
        - $\psi = \hmlneg\hmlobs{α}φ'$.
          This can only be in $\strat_\notionname{B}([q, p], e')$ due to $φ'$ being in $\strat_\notionname{B}([p, q], \energyupdate((\updmin{1,2},0,-1), e'))$.
          Clearly, $φ'$ must distinguish $p$ from $q$ and $\hmlands\set{φ'} \in \observationsvar{e}^\literal{poly-strong}$.
...
2. Assume $e \in \attackerwins^{\game_\notionname{B}^🗲}((α, p', q))$.
  ...
  For each $q'$, we employ the induction hypothesis to obtain $\psi_{q'} \in \strat_\notionname{B}([p', q'], e)$ of form $\psi_{q'} = \hmlobs{α}φ'$ or $\hmlneg\hmlobs{α}φ'$ with $\hmlands\set{\psi_{q'}} \in \observationsvar{e}^\literal{poly-strong}$, $p' \in \semantics{\psi_{q'}}$ and $q' \notin \semantics{\psi_{q'}}$.
...
If there is $φ \in \observationsvar{e}^\literal{poly-strong}$ with $p \in \semantics{φ}$ and $q \notin \semantics{φ}$,
then $e \in \attackerwins^{\game_\notionname{B}^🗲}([p, q])$.
:::
::: proof
By induction on the grammar of $\observationsvar{e}^{\literal{poly-strong}}$.

Consider the cases of $φ \in \observationsvar{e}^\literal{poly-strong}$ with $p \in \semantics{φ}$ and $q \notin \semantics{φ}$.

- $φ = \hmltrue$.
  This cannot be the case as $q \notin \semantics{φ}$ and $\semantics{\hmltrue} = \states$.
- $φ = \hmlobs{α}φ'$ with $φ' \in \observationsvar{e - \unit{1}}^\literal{poly-strong}$.
  As $φ$ distinguishes $p$ from $q$, there must be a $p' \in \semantics{φ'}$ such that $p \step{α} p'$ and, for all $q' \in \derivatives{q, α}$, $q' \notin \semantics{φ'}$.
  Due to the induction hypothesis, $e - \unit{1} \in \attackerwins^{\game_\notionname{B}^🗲}([p', q'])$ for all such $q' \in \derivatives{q, α}$.
  Therefore, $e - \unit{1} \in \attackerwins^{\game_\notionname{B}^🗲}((α, p', q))$.
  As the attacker can move $[p, q] \gamemove{-\unit{1}} (α, p', q)$, this proves $e \in \attackerwins^{\game_\notionname{B}^🗲}([p, q])$ by @prp-winning-budget-rules.
- $φ = \hmlands\set\Psi$ one of the $ψ \in \Psi$ must be false for $q$.
  Consider the possible forms of $ψ$:
    - Case $ψ = \hmlobs{α}φ'$ with $φ' \in \observationsvar{e - \unit{1}}^\literal{poly-strong}$.
      Then we can apply the same argument as in the case of $φ = \hmlobs{α}φ'$.
    - Case $ψ = \hmlneg\hmlobs{α}φ'$ with $φ' \in \observationsvar{(e \sqcap (e_2, \infty, \infty)) - \unit{1} - \unit{3}}^\literal{poly-strong}$.
...
A *Galois connection* between two partially ordered sets $(C, \leq_C)$ and $(A,\leq_A)$ is a pair of functions $α \colon C \to A$ and $\gamma \colon A \to C$ such that, for all $a \in A$ and $c \in C$:
...
Precisely if $\gamma \colon A \to C$ is a monotone function, and $α(c) = \inf \set{a \mid c \leq_C \gamma(a)}$ for all $c \in C$,
