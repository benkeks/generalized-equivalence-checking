...
\end{array}
$$
Intuitively, action $c$ represents receiving and its “coaction” $\coaction c$ expresses sending on channel $c \in \ccschannels$.
...
If we insert $\literal{Q}$ into this context, the choice after the collective $\literal{fork}$-acquisition is resolved to the $\literal{B}$-path.
...
- **Partition refinement.**
  ...
  Partitions represent equivalence relations as colorings on graphs, which is linear in the state space $\relsize{\states}$ (opposed to quadratic combinations for pairs in relations and attacker positions).
