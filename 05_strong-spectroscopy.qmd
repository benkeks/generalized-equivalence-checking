...
#### Subsets to break up observations and conjunctions
...
\begin{tikzpicture}[auto,node distance=1.1cm,
  ...
  \path
    ...
    (States) edge[->, draw=TealBlue] node {Definition~\ref{def-bisim-energy-game}} (EnergyGame)
...
As long as observations happen, the defender is moved to the set of states $Q$ reachable through nondeterminism; only at conjunction moves does the defender have to choose a $q \in Q$.
...
To point out a similar distinction in the spectroscopy game, the attacker moves:
$$
\begin{array}{rcl}
...
In the upcoming @sec-correctness-strong-spectroscopy we will also give the rules that the tool uses to construct this witness.
...
precisely if
the defender wins $\gamespectroscopy$ from $\attackerpos{p_0, \{q_0\}}$ with $e_0$ for every initial energy budget $e_0$,
...
::: {#fig-strategy-formulas-strong fig-env="figure*"}
$$
...
$p \bpreordvar{\observationsvar{N}} q$ for $N ∈ \notions^\literal{strong}$ precisely if the defender wins $\gamespectroscopy$ from $\attackerpos{p,\set{q}}$ for the attacker starting with budget $N$.
:::
...
In particular, the named notions of the strong spectrum in @fig-strong-ltbt-spectrum can be covered by subsets of notions that are characterized by simpler games.
...
The subgame of @fig-derived-trace-eq-game that actually is used in {{< equivio >}}, disregarding some attacker options.
:::
...
All prior publications with generalized game characterization of the spectrum describe a hierarchy of reachability games---as opposed to our view of *one* energy game per system.
...
Our initial 2021 game is unable to correctly handle failure-traces and similar observations, which has been corrected in @bjn2022decidingAllBehavioralEqs, thanks to David N. Jansen's input.
...
Interestingly, @sec-regaining-polynomiality shows that, by the prioritizing conjunctions in derived simulation-like games, one obtains $\bigo{\relsize{\states} \cdot \relsize{\step{}}}$ complexity, restoring the bound of last chapter's @sec-polynomial-spectroscopy-complexity.
...
Practical applications of concurrency theory tend to need the weak semantics,
and, other than with the strong spectrum, nobody has previously proposed generalized game characterizations for the weak spectrum, at all.