...
So, @prp-difference-contraposition that sublogics will equate at least as much as their parent logic, yields another way of establishing the entailment hierarchy between bisimilarity, similarity, and trace equivalence of @sec-hierarchy-equivalences.^[Taking the two previous lemmas together with @thm-hennessy-milner for $\beq{B}$ and $\hml$.]
...
Note that this minimum does not need to exists---but it does for the spectra we will be working with.
...
If a non-terminal appears only once in the conjunction production, at most one such sub formula is allowed.
...
The best known bisimilarity algorithms for finite-state transition systems take $\bigo{\relsize{\step{}} \log \relsize{\states}}$ time [usually deriving from @paigetarjan1987partitionrefinement; e.g. @valmari2009bisimMin].
...
However, there is a polynomial-time-easy part of spectrum around bisimilarity and (ready) similarity.
