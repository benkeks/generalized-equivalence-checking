...
$$\bigo{ \relsize{G}^2 \cdot o_{\gamemoveblank} \cdot (w_{\abstractenergies}(e_{\game}))^2 \cdot (t_{\abstractupdateinv} + t_{\sup} + t_\leq) }.$$
:::
:::proof
...
The undo-update function $\energyupdateinv$ on an $N$-dimensional declining energy game is defined component-wise for $i \in \set{1 … N}$ as:
$$
\begin{array}{rl}
(\energyupdateinv(u, e'))_i := \max(
      & \set{e'_i - u_i  \mid  u_i \in \set{0,-1}} \\
  \cup & \set{e'_j        \mid \exists D \ldotp u_j = \mathtt{min}_D \land i \in D}).
\end{array}
$$
:::
...
:::
:::proof
We show $\energyupdateinv(u, e') \leq e$ iff $e' \leq \energyupdate(u, e)$ for all $e, e' \in \domain(u)$.

- Assume $\energyupdateinv(u, e') \leq e$.
This means that, for all $i$, $\max(\set{e'_i - u_i \mid u_i \in \set{0,-1}} \cup \set{e'_j \mid \exists D \ldotp u_j = \mathtt{min}_D \land i \in D}) \leq e_i$.

  - Case $u_i \in \set{0,-1}$.
    Then $e'_i - u_i \leq e_i$.
    Thus $e'_i \leq e_i + u_i = (\energyupdate(u, e))_i$.
  - Case there is $D$ such that $u_i = \mathtt{min}_D$.
    For all $j \in D$, we have $e'_j \leq e_i$.
    TODO!!
    Thus, $e'_i \leq \min\set{e_j \mid j \in D} = (\energyupdate(u, e))_i$
Moreover, if $$
