...
{{<paragraph_heading "The problem">}} can be summarized as follows:
...
It includes numerous concepts we will discuss on our way there.
...
Are there notions relating the two *besides* mutual weak similarity?
:::

{{<paragraph_heading "State of the art.">}}
For many of the existing behavioral preorders and equivalences, there are various algorithms and implementations to decide whether processes are equivalent.
...
{{<paragraph_heading "Our solution.">}}
This thesis will outline the ideas one needs to decide all equivalences in one uniform approach.
As we will discuss in more detail in @sec-equivio-peterson, our accompanying tool {{<equivio>}} can answer the questions of @exm-peterson-mutex in the following query:^[
...
Moreover, $\literal{Pe}$ is *preordered* to $\literal{Mx}$ with respect to two more special finer notions, namely *$\eta$-simulation* and *stable simulation* (and notions below).
...
- WIP @sec-implementations showcases implementations to conveniently perform equivalence spectroscopies in web browsers.
  - Side quest: Peterson's mutual exclusion.
...
The usage of {{<equivio>}} is explained in @sec-equivio-usage.
