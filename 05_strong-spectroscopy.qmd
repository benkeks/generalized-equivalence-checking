...
The number of attacker positions is bounded by $\relsize{\states} \cdot 2^{\relsize{\states}}$.
...
1. Since the specific coordinates in @fig-strong-ltbt-spectrum only use 0, 1, and ∞ in components, we can employ @lem-declining-update-complexity-flattened to slightly improve complexity bounds.
...
Now, the number of all game positions is bounded by $\relsize{G_{\blacktriangle}} \in \relsize{\states} \cdot 2^{\relsize{\states}}$.
...
Let us first explicate how to derive characterize equivalences through reachability games derived according to @def-derived-reachability-game.
...
@lem-conjunction-optimization allows us to prune away all observation moves deriving from there, and return to an attacker position with $\relsize{Q''} \leq 1$.
...
{{< paragraph_heading "Original formulation." >}}
...
We could go a step further in the polynomially-sized derivatives of the spectroscopy game, and “bake together” the attacker positions after observations with the following defender positions.
