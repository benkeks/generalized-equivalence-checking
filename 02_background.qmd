...
\begin{array}{lr}
  ...
  \quad \literal{attacker\_win} := \set{} & \\
  ...
  \quad \kw{return} \literal{attacker\_win} & \\
\end{array}
$$
<!--
```tikz
...
\Fn{$\literal{compute\_winning\_region}(\game=(G,G_\defender,\gamemoveblank))$}{
  
  $\literal{defender\_options}\, :=
    [g \mapsto n \mid g \in G \land n = \relsize{\set{p \mid g \gamemoveblank p}}]$

  $\literal{attacker\_win}\, := \set{}$

  $\literal{todo}\, := \set{g\in G_\defender \mid \literal{defender\_options}[g]=0}$

  \While{$\literal{propagate\_attacker\_win}(g)$}{

    \If(){$g \notin \literal{attacker\_win}$}{

      $\literal{attacker\_win} := \literal{attacker\_win} \cup \set{g}$

      \For(){$g_{p}\in (\cdot \gamemoveblank g)$}{

        $\literal{defender\_options}[g_{p}]:=\literal{defender\_options}[g_{p}]-1$

        \If(){$g_{p}\in G_\attacker\lor\literal{defender\_options}[g_{p}]=0$}{

          $\literal{propagate\_attacker\_win}(g_{p})$
