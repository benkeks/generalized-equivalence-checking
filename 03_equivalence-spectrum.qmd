...
By $\unit{k}$ we denote the *unit vector* where the $k$-th component is 1 and all other components equal 0.
For example, $\unit{2}$ in a 3-dimensional context equals $(0,1,0)$.
...
\begin{array}{rcll}
  φ^N & \grammardef & \hmltrue{} & \\
      ...
      & \grammaror  & \hmlands\set{\varphi^{(N-\unit{2}) \sqcap (N_3, \infty, \infty, \infty, \infty, \infty)}, \\
      &             & \quad\;
                        ψ^{(N-\unit{2}) \sqcap (\infty, \infty, \infty, N_3 \infty, \infty)},
                        ψ^{(N-\unit{2}) \sqcap (\infty, \infty, \infty, N_3, \infty, \infty)}, \ldots} & \\
  ψ^N & \grammardef &
      φ^{(N \sqcap (N_4, \infty, \infty, \infty, \infty, \infty))} & \\
      & \grammaror  &
      \hmlneg φ^{(N \sqcap (N_5, \infty, \infty, \infty, \infty, \infty)) - \unit{6}} & \\
\end{array}
$$
...
\begin{align*}
  ...
  \expr(\hmlobs{\alpha}\varphi) & \defeq \textcolor{obsColor}{\unit{1}} + \expr(\varphi) \\
  \expr(\hmlneg\varphi) & \defeq \textcolor{negsColor}{\unit{6}} + \expr(\varphi) \\
  \textstyle\expr(\bigwedge\limits_{i \in I}\psi_i) & \defeq
  \textcolor{conjColor}{\unit{2}} + \sup\Biggl(\big\{\!
  \begin{pmatrix}
    0\\
    0\\
...
There is a minor divergence from @bisping2023equivalenceEnergyGames in that the present thesis prices $\expr(\hmltrue)$ at $\zerovec$ instead of $\unit{2}$.
