...
The expressiveness price $\expr$ of formulas in the strong spectrum (@def-strong-spectrum) can be recursively characterized as described in @bisping2023equivalenceEnergyGames:
...
If we restrict our focus to a grid of ternary entries $\set{0,1,\infty}$, we can be sure to need at least 4 dimensions:
With 3 dimensions, the height of the lattice is 7, that is, the maximal number of nodes on increasing paths between $(0,0,0)$ and $(\infty, \infty, \infty)$.
But the hierarchy of @fig-strong-ltbt-spectrum has height 9!

With a grid of 4 ternary dimensions, we can recreate the hierarchy of @fig-strong-ltbt-spectrum.
