...
::::{#related-8 .column-margin}
`\vspace{-2.1em}`{=latex}
{{< paragraph_heading "Related publications." >}}
Conference artifacts of {{< equivio >}} (reviewed by the artifact evaluation committees of TACAS'21 and CAV'23) are archived as *Linear-time–branching-time spectroscope* [@bisping2021ltbtsZenodo; @bisping2023ltbtsZenodo3].
Some paragraphs of discussion in this chapter have already appeared in @bjn2022decidingAllBehavioralEqs and @bisping2023equivalenceEnergyGames.
::::
...
We demonstrate its use through the example of Peterson's mutex.
...
In @sec-implementations-discussion, we compare our implementations to similar tools in the lineage of the “Concurrency Workbench” [@cps1990concurrencyWorkbench].

::::{.content-visible when-format="html" .column-margin}
{{< contents related-8 >}}
::::

{{< contents wrapper-fig-equivio-screenshot >}}

## Prototype: *equiv.io* {#sec-equivio}

The “Linear-time–branching-time spectroscope” at {{< equivio >}} is a small web tool to check equivalence and preorder relations on CCS processes.
...
:::: {#wrapper-fig-equivio-screenshot}

::: {#fig-equivio-screenshot fig-pos="b"}
...
| Strong variant       | Weak variants                      | Stable/stability-resp.&nbsp;variants   |
|:-----------------------|:------------------------------|:---------------------------------|
...
| `simulation`           | `weak-simulation` <br> `eta-simulation`                  | `stable-simulation`                        |
...
- `@preprocessing method1, method2...` –
  ...
  - `weakness_saturated` – Replace the transition relation with *weak transitions*.
...
### Application to Peterson's Mutual Exclusion Protocol {#sec-equivio-peterson}
...
Assuming an atomic memory model [cf. @vgls2025justVerificationMutex], the protocol works because each process will only enter the critical section if no other process is waiting or if it has been yielded the turn (by the other process).
...
• Left-right-distinguished by:
    ⟨ϵ⟩Λ{¬⟨ϵ⟩⟨ecA⟩⊤,¬⟨τ⟩⊤} (stable-failure)
    ⟨ϵ⟩Λ{¬⟨ϵ⟩⟨ecB⟩⊤} (weak-failure)
• Equated by:
    weak-simulation
```
...
The weak failure `⟨ϵ⟩Λ{¬⟨ϵ⟩⟨ecB⟩⊤}` can be understood to point out that `Pe` *exceeds* `Mx` in that
...
• Preordered by:
    ...
    s-ready-simulation
• Left-right-distinguished by:
    ⟨ϵ⟩⟨ecA⟩⟨ϵ⟩Λ{¬⟨ϵ⟩⟨lcA⟩⟨ϵ⟩⟨ecA⟩⊤,¬⟨τ⟩⊤} (s-impossible-future)
    ⟨ϵ⟩⟨ecB⟩⟨ϵ⟩⟨lcB⟩Λ{¬⟨ϵ⟩⟨ecB⟩⊤} (delay-bisimulation)
    ⟨ϵ⟩⟨ecA⟩⟨ϵ⟩Λ{¬⟨ϵ⟩⟨lcA⟩⟨ϵ⟩⟨ecA⟩⊤} (weak-impossible-future)
• Equated by:
    stable-readiness
    eta-simulation
    weak-ready-simulation
...
In above output, this is mirrored by the impossible-future formula `⟨ϵ⟩⟨ecA⟩⟨ϵ⟩Λ{¬⟨ϵ⟩⟨lcA⟩⟨ϵ⟩⟨ecA⟩⊤}`.

We can find this scenario already in the pseudocode.
...
3. **Spectroscopy.**
    ...
    1. The spectroscopy implementation defines a *spectroscopy game* (e.g. {{< code_link "shared/src/main/scala-2.12/io/equiv/eqfiddle" spectroscopy.StrongSpectroscopyGame >}}).
       The game must be a {{< code_link "shared/src/main/scala-2.12/io/equiv/eqfiddle" game.EnergyGame >}}, inheriting a decision procedure along the lines of @algo-deciding-energy-games from trait {{< code_link "shared/src/main/scala-2.12/io/equiv/eqfiddle" game.GameLazyDecision >}}.
...
4. **Validation (optional).**
    If the user demands, `SpectroscopyFramework.decideAll` continues to construct cheapest distinguishing formulas for the query with `buildHMLWitness`.
    ...
    If no formula construction is requested, `SpectroscopyFramework.decideAll` works on *flattened energies* (@def-flattened-energies), which generally leads to better performance.
5. **Presentation.**
...
Another trait {{< code_link "shared/src/main/scala-2.12/io/equiv/eqfiddle" spectroscopy.EquivalenceChecking >}} follows the second path of @fig-strong-spectroscopy-big-picture to decide individual equivalences through reachability games.
We will not go into detail here.
The core feature is to derive a game {{< code_link "shared/src/main/scala-2.12/io/equiv/eqfiddle" game.MaterializedEnergyGame >}} as in @def-derived-reachability-game.
{{< code_link "shared/src/main/scala-2.12/io/equiv/eqfiddle" game.WinningRegionComputation >}} determines its winner according to @algo-deciding-games.
...
This mostly matches the results already reported in @bisping2023equivalenceEnergyGames.^[
  The benchmarking is in the class
  {{< code_link "shared/src/main/scala-2.12/io/equiv/eqfiddle" tool.benchmark.VeryLargeTransitionSystems >}}.
  It can be started via `sbt "shared/run benchmark --strong-game --reduced-sizes --include-hard"`.
]
...
#| tbl-colwidths: [16,10,12,15,12,10,10,13]
#| echo: false
...
This indicates that the examples are based around models that avoid the expressive power of finer branching-time notions.
...
You can enjoy the game in the browser at <https://games.equiv.io/play/ltbt-game/>.
...
gpuequiv parallelizes the budget computation of @algo-deciding-energy-games.
...
For the system `vasy_18_73`, @vogel2024energyGamesWebGPU also fails to complete the spectroscopy (like our experience in @sec-benchmarks):
...
{{< equivio >}} can check behavioral preorders and equivalences for more than 39 notions of strong and weak spectrum.
Due to the possibility to mark divergences, even more notions are available via preprocessing.
...
- **mCRL2** [@groote2014mcrl2], built around an ACP/CCS-like modelling language of the same name, supports a wide-range of notions with highly efficient implementations and a strong focus on branching bisimilarity.
  On the notions that it supports, it will generally be much faster than the spectroscopy game algorithm.
...
The implementations of this chapter realize @algo-deciding-energy-games quite directly to decide energy games.
...
Another route to this goal of genericity might be the approach of *equivalences-as-formulas* from @lange2014modelCheckingProcEqs.
...
A light-weight variant of the functor-approach is offered by the paradigm *equivalences-as-signatures*.
