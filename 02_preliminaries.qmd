...
- the universal relation between all final states $\rel R_\mathrm{fin} ≔ \set{\literal{p_1}, \literal{p_2}, \literal{q_1}, \literal{q_2}} \times \set{\literal{p_1}, \literal{p_2}, \literal{q_1}, \literal{q_2}}$,
...
- and the combination of the above $\rel R_\mathrm{big} ≔ \rel R_\mathrm{id} ∪ \rel R_\mathrm{fin} ∪ \rel R_\mathrm{up} ∪ \rel R_\literal{PQ}$.
...
\begin{tikzpicture}[auto,node distance=2cm]
  ...
  \node[state, below left of=q0] (qAB){$\literal{q_{ab}}$};
  \node[state, below left of=qAB] (q1){$\literal{q_{1}}$};
  \node[state, below right of=qAB] (q2){$\literal{q_{2}}$};
...
(Reason: The relation $\set{(\literal Q, \literal{T}), (\literal{T}, \literal{Q})} \cup \identity{\set{ \literal{q_{ab}}, \literal{q_1}, \literal{q_2}, \troll}}$ is a simulation.)
...
In @exm-bisim-sim of “trolled philosophers,” all terminal states are bisimilar, $\literal{q_1} \beq{B} \literal{q_2} \beq{B} \troll$.
...
\begin{tikzpicture}[auto,node distance=2cm]
  ...
  \node[state, below left of=q0] (qAB){$\literal{q^m_{ab}}$};
  \node[state, below right of=qAB] (q123){$\literal{q^m_{123}}$};
  \path[->]
    (q0)
