...
As mentioned in @rem-explicit-expr, @bisping2023equivalenceEnergyGames uses a slightly different pricing, where $\expr(\hmltrue) = \unit{2}$.
This difference is reflected in the game by @def-strong-spectroscopy-game charging $-\unit{2}$ for conjunctions after the defender conjunction positions instead of before.
With this, the attacker wins $\attackerpos{p, \varnothing} \gamemove{\zerovec} \defenderpos{p, \varnothing, \varnothing}$ with budget $\zerovec$, while the original definition in @bisping2023equivalenceEnergyGames would cost them $\unit{2}$ for $\attackerpos{p, \varnothing} \gamemove{-\unit{2}} \defenderpos{p, \varnothing, \varnothing}$.
...
Finally, we will take the last point to the extreme by instantiating the spectroscopy game down to only decide single equivalences.
...
### Deriving Equivalence Games

We can use simpler derivatives of the strong spectroscopy game to decide inividual equivalences.
The core idea is to *derive a reachability game* according to @def-derived-reachability-game, starting with an energy level from the strong spectrum, @def-strong-ltbt-spectrum.
...
The following processes are two of the more complex ones of the separating examples in @glabbeek2001ltbtsiReport.
They are equal with respect to ready traces, but not for simulation-like and possible-future-like equivalences.

```ccs
ABCACB = a.(b.d.0 + c.e.0) + a.(c.f.0 + b.g.0)
ABC    = a.(b.d.0 + c.e.0 + c.f.0 + b.g.0)

@check "traces",            ABCACB, ABC
...
ABC(x=683, y=136)
"0"(x=404, y=669)
"c.f.0 + b.g.0"(x=438, y=307)
"b.d.0 + c.e.0"(x=220, y=424)
"g.0"(x=441, y=528)
"f.0"(x=557, y=599)
"d.0"(x=299, y=520)
"e.0"(x=259, y=605)
"b.d.0 + c.e.0 + c.f.0 + b.g.0"(x=627, y=372)
```

The `@check` keyword performs individual comparisons on `equiv.io`.
...
:::{#fig-derived-trace-eq-game-true fig-pos='t' fig-env="figure*"}
```dot
...
### Regaining Polynomiality in Derived Equivalence Games
...
For $N \geq (0,\infty,\infty,\infty,0,0)$ and $N \in \set{0, 1, \infty}^6$, one can decide $p \bpreord{\observations{N}} q$ 
in $\bigo{...}$ time and space
by deciding who wins the $N$-preorder game.
:::
:::proof
@lem-conjunction-optimization
TODO

We observe that the energies in the game might only create a constant increase in the size, compared to the underlying spectroscopy game.
...
In the clever spectroscopy game, there is a maximum of only five defender positions per such attacker position.
...
the size of the $N$-preorder game derived from the clever spectroscopy game is polynomial, $\relsize{G_{\blacktriangle N}} \in \bigo{\relsize{\states} \cdot \relsize{\step{}}}$.
...
Let us see how the instance game sizes behave
$\relsize{\game} = \relsize{G} + \relsize{\gamemoveblank}$
:::
