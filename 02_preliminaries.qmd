...
\end{gathered}
$$
A transition system representation of $\literal{P_A}$'s behavior can be seen in the margin.
...
For instance, the transition $\literal P \step{τ} \literal{p_a}$ in the CCS system of @exm-ts would be justified as follows:
...
TODO: Fix the game to use minimized troll / philosopher system....
...
```tikz
  \begin{tikzpicture}[>->, shorten <=1pt, shorten >=0.5pt, auto, node distance=2cm,
    ...
    \node[posStyle, defWins]
      (Att_qab_tab) [below right = .1cm and 1.5cm of Dfn_t_qab_Qp] {$[\literal{q_{ab}}, \literal{t_{ab}}]$};
    \node[posStyle, defWins]
    ...
    \node[posStyle, defender]
      (Dfn_a_q1_q3) [above right = .1cm and 1.5cm of Att_qab_q3] {$(\literal{a}, \literal{q_1}, \troll)$};
    \node[posStyle, defender]
    ...
    \node[posStyle]
      (Att_q3_qab) [below = 1cm of Att_tab_qab] {$[\troll, \literal{q_{ab}}]$};
    ...
    ;
    \path[dashed]
      (Att_qab_tab) edge[swap] node {$\literal{q_{ab}} \step{\literal{a}} \literal{q_1}$} ([xshift=1cm, yshift=.2cm]Att_qab_tab.east);
  \end{tikzpicture}
```
