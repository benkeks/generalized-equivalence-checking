...
Moves to allow internal behavior between observations:
$$
\begin{array}{rrcll}
  \textit{delay}
  & \attackerpos{p,Q}
...
\end{array}
$$
moves to encode branching conjunctions $\hmlands \set{\hmlsoft{\alpha}\varphi, \psi, \psi, ...}$:
...
\end{array}
$$
and moves to encode stable conjunctions $\hmlands \set{\hmlneg\hmlobs{\tau}\hmltrue, \psi, \psi, ...}$:
...
Weak strategy formulas are distinguishing.
  ...
  \begin{tikzpicture}[>->,shorten <=1pt,shorten >=0.5pt,auto,node distance=2cm, rel/.style={dashed,font=\it},
    posStyle/.style={draw, inner sep=1ex,minimum size=1cm,minimum width=2cm,anchor=center,draw,black,fill=gray!5}]
      ...
      \path
        ...
        (AttDelay) edge
          node[align=center,pos=0.3,label={[label distance=0.1cm]-100:$\textcolor{gray}{\zerovec}$}] {$p \stepsoft{\alpha} p'$\\$Q_\varepsilon \neq \varnothing$} (DefBranch)
        (Def) edge[bend right=17]
          node[pos=.85, label={[label distance=0.1cm]-100:$\textcolor{gray}{-\unitvec{3}}$}] {$q \in Q$} (AttConj)
        (DefStab) edge[bend right=10]
          node[align=right, pos=.85, label={[label distance=0.3cm]-170:$\textcolor{gray}{-\unitvec{4}}$}] {\mbox{}\hspace*{-1.5em}$q \in Q_\varepsilon$\\[2pt]\mbox{}\hspace*{-1.5em}$q \nostep{\tau}$} (AttConj)
...
The simplified game part encodes nested conjunctions of the form $\hmlands\set{ \hmlsoft{\alpha} \mathhl{\hmlands \Psi'}, \psi_1, \ldots}$ or the cheaper form $\hmlands\set{ \hmlsoft{\alpha} \mathhl{\hmleps \hmlands \Psi'}, \psi_1, \ldots}$.
...
The practical difference is huge on transition systems with relevant branching-degree with respect to internal behavior such as the initial Peterson system of @fig-peterson-mutex:
