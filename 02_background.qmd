...
- If there is a positional defender strategy $\rel s$ winning from $[p, q]$,
  ...
  $$\rel R ≔ \set{(p,q) \mid \text{there is a play $(p_0, q_0),...,(p,q)$ following strategy $\rel s$}}.$$
:::
...
φ_{\rel s}([p,q]) ≔ \begin{cases}
  \hmlneg φ_{\rel f}([q,p])  & \text{if } \rel s([p,q]) = [q,p] \\
  \hmlobs{\alpha} \hmlands \set{ φ_{\rel s}([p',q']) \mid q \step{\alpha} q' } & \text{if } \rel f([p,q]) = (\alpha,p',q)
\end{cases}
$$
