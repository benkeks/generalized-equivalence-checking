...
\begin{tikzpicture}[auto,node distance=1.1cm,
  ...
  \path
    ...
    (Rel) edge[draw=Fuchsia] node[label={[align=center]below:Hennessy--Milner\\Thm.~\ref{thm-hennessy-milner}}] {XOR} (Distinction)
...
To provide them with a bridge, we will start with relational and denotational definitions---but once we have crossed this bridge, we will stay in the realm of games and logics.
...
1. Should one care about nondeterminism in programs? @sec-ccs shows how nondeterminism arises naturally in concurrent programs.
...
Then, $\ccs$ is the set of processes, communicating via actions $\ccsactions ≔ \ccschannels \cup \set{ \coaction{c} \mid c ∈ \ccschannels } \cup \set{τ}$, as given by the following grammar:^[
...
Consider two philosophers $\literal{P_A}$ and $\literal{P_B}$ who want to grab a resource $\literal{fork}$, in order to eat, which we model as receiving communication.
...
\noindent
A process $P ∈ \ccs$ now denotes a node within the labeled transition system $(\ccs, \ccsactions, \step{}_\ccs)$.
:::
...
Such sequences are referred to as *traces*.
...
An $n$-ary function $f \colon B_1 \times \dots \times B_n \to C$ is called *monotonic* with respect to a family of preorders $(B_k, \leq_k)$ and $(C, \leq)$ iff, for all $b \in B_1 \times \dots \times B_n$ and $b' \in B_1 \times \dots \times B_n$, it is the case that $b_k \leq b'_k$ for all $k \leq n$ implies that $f(b) \leq f(b')$.
...
By now, we have ecountered some behavioral equivalences.
...
We also usually omit them when writing down formulas, e.g., shortening $\hmlobs{\literal a}\hmlobs{\literal b}\hmltrue$ to $\hmlobs{\literal a}\hmlobs{\literal b}$, which says that one may observe $\literal{a}$ and then $\literal{b}$.
...
The smaller bisimulation logic $\observations{\floor{B}}$ will become important again later in @sec-bisim-game-hml.
...
The player with a winning strategy from $g_{0}$ is said to *win* $\game$ from $g_{0}$.
...
Game scheme of the bisimulation game ${\game}_\notionname{B}$ (@def-bisim-game).
:::
...
:::: {#fig-bisim-game-even-odd fig-pos="ht"}
  ```tikz
...
::: {#thm-game-formulas}
The attacker wins ${\game}_\notionname{B}$ from $[p,q]$ precisely if there is a formula $φ ∈ \hml$ distinguishing $p$ from $q$.
:::
...
\begin{array}{lr}
  ...
  \quad \quad \quad \quad \kw{if} \variable{g_{p}} \in G_\attacker \colon & \\
  \quad \quad \quad \quad \quad \variable{todo} ≔ \variable{todo} \cup \set{\variable{g_{p}}} & \\
  \quad \quad \quad \quad \kw{else} \colon & \\
  \quad \quad \quad \quad \quad \variable{defender\_options}[\variable{g_{p}}]≔\variable{defender\_options}[\variable{g_{p}}]-1 & \\
  \quad \quad \quad \quad \quad \kw{if} \variable{defender\_options}[\variable{g_{p}}]=0 \colon & \\
  \quad \quad \quad \quad \quad \quad \variable{todo} ≔ \variable{todo} \cup \set{\variable{g_{p}}} & \\
...
The following @tbl-game-example-execution lists how @alg-deciding-games computes the winning region $\attackerwins = \set{[\literal{2a}], (\literal{3})}$ of the simple choice game of @exm-formula-game.
...
- **Fixed-point iteration.**
  ...
  In effect, a chain reaction happens analogous to the one that @alg-deciding-games performs on the bisimulation game.
...
But first, we have to make formal what we mean by *spectra of behavioral equivalences*.
