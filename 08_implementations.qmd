...
\begin{tikzpicture}[auto,node distance=2cm,
  ...
  \node[right of=Notions, node distance=6.1cm, align=center] (PriceYesNo) {Price okay? \\ $✔$ / $\questionmark$  / $✗$};
  ...
  \begin{pgfonlayer}{bg}
    \node[draw=gray, rounded corners, densely dotted, thick, inner xsep=.15cm, inner ysep=.2cm, fit=(Result.center) (Formulas)] {xas};
  \end{pgfonlayer}

  \draw
    (Source) edge[->, swap] node {\texttt{ccs.Parser}} (Syntax)
    (Syntax) edge[->] node {\texttt{ccs.Interpreter}} (TS)
    ([xshift=-1cm]TS.south) |- ([yshift=1cm,xshift=-1cm]SpectroGame.north) edge[->] node {\texttt{EnergyGame.populateGame}} ([xshift=-1cm]SpectroGame.north);
  \draw
