...
#### A Classic Example
  ...
  Consider the transition system $\system_\literal{PQ} = (\set{\literal P, \literal{p_a}, \literal{p_b}, \literal{p_1}, \literal{p_2}, \literal Q, \literal{q_{ab}}, \literal{q_1}, \literal{q_2}}, \set{\literal a, \literal b, \tau}, \step{\cdot}_\literal{PQ})$ given by the following graph:
...
::::
  ...
  However, to bisimilarity, $\literal{Q'} \step{\tau} \literal{q_3}$ constitutes a difference.
...
$$
\begin{array}{rcl}
  \semantics{\hmlobserve{α}\varphi} & := & \set{p \mid \exists p' ∈ \semantics{\varphi} \ldotp p \step α p'},\\
  \semantics{\hmland{i}{I}\varphi_i} & := & \bigcap_{i ∈ I} \semantics{\varphi_i},\\
  \semantics{\hmlneg \varphi} & := & \states \setminus \semantics{\varphi}.
\end{array}
$$
:::
