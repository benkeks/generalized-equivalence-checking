...
4. Symmetry of $\beq{B}$:<br>
  $p \beq{B} q$ implies there is a symmetric simulation $\rel R$ with $(p,q) \in \rel R$.
  As $\rel R = \inverse{\rel R}$, this implies $(q,p) \in \rel R$ and thus $q \beq{B} p$.
...
## Observations as Modal Logic {#sec-hml}
...
We say a sublogic $\observationsvar{N} ⊆ \hml$ *preorders* state $p$ to $q$, $p \bpreordvar{\observationsvar{N}} q$, if no $φ ∈ \observationsvar{N}$ is distinguishing $p$ from $q$.
If the preordering goes in both directions, we say that $p$ and $q$ are equivalent with respect to sublogic $\observationsvar{N}$, written $p \beqvar{\observationsvar{N}} q$.
:::
...
::: {#prp-hml-eq}
Consider an arbitrary HML sublogic $\observationsvar{N} ⊆ \hml$.
Then, $\bpreordvar{\observationsvar{N}}$ is a preorder,
and $\beqvar{\observationsvar{N}}$ an equivalence relation.
:::
...
::: {.proof}
Assume it were not.
Then there would need to be $p \beqvar{\hml} q$ with step $p \step \alpha p'$, and no $q'$ such that $q \step α q'$ and $p' \beqvar{\hml} q'$.
...
If $p \in \semantics{φ}$ and $p \beq{B} q$ then $q \in \semantics{φ}$.
:::
::: {.proof}
...
This means that $p \step α \cdot$ is finite for every $p \in \states$.
...
The original proof TODO: CITEHML used binary conjunction and thus could only express finitary conjunctions.
:::
...
If a finite play $g_{0}\dots g_{n}\!\ngamemoveblank$ is stuck, the stuck player loses: The defender wins if $g_{n}\in G_\attacker$, and the attacker wins if $g_{n}\in G_\defender$.
:::

<!--
%
\begin{defi}[Strategies and winning strategies]
  \label{def:strategies}
  A (positional, non-deterministic) \emph{attacker strategy} is a subset of the moves starting in attacker states,
  $F ⊆ (G_\attackerSubscript × G) \cap \mathord{\gameMove}$.
  Similarly, a \emph{defender strategy} is a subset of the moves starting in defender states,
  $F ⊆ (G_\defenderSubscript × G) \cap \mathord{\gameMove}$.
  If (fairly) picking elements of strategy $F$ ensures a player to win,
  $F$ is called a \emph{winning strategy} for this player.
  The player with a winning strategy for $\game[g_{0}]$ is said to \emph{win} $\game[g_{0}]$.
  If $F$ is a function on $G_\attackerSubscript$ or $G_\defenderSubscript$, respectively, we call it a \emph{deterministic} strategy.
\end{defi}
%
...
\begin{defi}[Winning regions]
  \label{def:winning-region}
  The set $W_\attackerSubscript ⊆ G$ of all positions $g$ where the attacker wins $\game[g]$ is called the \emph{attacker winning region.}
  (The defender winning region $W_\defenderSubscript$ is defined analogously.)
\end{defi}
-->
