...
This chapter presents the spectrum of modal languages from @bisping2023equivalenceEnergyGames in a slightly clarified manner.
The used hierarchy itself derives from @bjn2022decidingAllBehavioralEqs.
The alignment to the authoritative work by @glabbeek1990ltbt1 has been studied by @mattes2024expressivePowerHML.
::::
...
However, we will order the equivalences using the approach of parameterized *notions of observability* from the later by @glabbeek1993ltbt.
...
Note that this minimum does not need to exists---but it will for the spectra we will be working with.
...
Using @exm-simple-spectrum: $\expr(\hmlobs{\tau}\hmlobs{\literal a}) = \varnothing \subset \set{⊗} = \expr(\hmlobs{\tau} \hmlands\set{\hmlneg\hmlobs{\literal{a}}})$, which captures that we need a strictly smaller part of the grammar to construct the trace formula.
...
$$
\begin{array}{rl}
  \expr(\hmltrue) & = \zerovec \\
  \expr(\hmlobs{\alpha}\varphi) & = \textcolor{obsColor}{\unitvec{1}} + \expr(\varphi) \\
  \expr(\hmlneg\varphi) & = \textcolor{negsColor}{\unitvec{6}} + \expr(\varphi) \\
  \textstyle\expr(\bigwedge\limits_{i \in I}\psi_i) & =
  ...
  &
  \begin{array}{rl}
    \mathit{Neg} & \defeq \set{i \in I \mid \exists \varphi'_i \ldotp \psi_i = \hmlneg \varphi'_i }\\
    \mathit{Pos} & \defeq I \setminus \mathit{Neg}\\
    \mathit{R} & \defeq
    \begin{cases}
      \varnothing & \text{if } \mathit{Pos} = \varnothing\\
      \set{ r } & \text{for some $r \in \mathit{Pos}$ where $\expr_1(\psi_r)$ maximal for $\mathit{Pos}$.}
    \end{cases}
  \end{array}
\end{array}
$$
...
- For case (4) of failures, we must note that $\observationsvar[\literal{strong}]{(\infty, 1, 0, 0, 1, 1)}$ also contains failure observations that end in conjunctions with a $\hmlneg \hmltrue$-conjunct.
  But these are trivially false and thus cannot add to distinctiveness.\qedhere
:::

So far, we have only established that the six-dimensional spectrum covers the notions that @exm-simple-spectrum has already covered–in a more complicated way.
...
@fig-strong-grammars-linear and @fig-strong-grammars-branching give the grammars that are described through the coordinates in interplay with @def-strong-spectrum.
...
If a nonterminal appears only once in the conjunction production, at most one such sub formula is allowed.
