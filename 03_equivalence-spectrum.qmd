...
Let the strong notions of equivalence over vectors of extended naturals^[We write $\natsext$ for $\nats \cup \set{\infty}$.]
...
Because the coordinate of $2$-nested simulation, $\notionname{2S} = (\infty,\infty,\infty,\infty,\infty,1)$ is not less or equal to $(2,2,0,0,2,2)$, we arrive at $\literal{Q} \bpreord{2S} \literal{Q'}$, which implies *all* preorders of @fig-full-strong-spectrum except for bisimilarity.
...
\begin{tikzpicture}[]
    \begin{scope}[rotate=30, scale=1]
      ...
      \draw[thick, opacity=0.5, text opacity=.8, xshift=-.7505cm, yshift=-1.3cm] (-.25,2) -- (4,2) node[pos=0,left] {conjunctions = 2};
      \draw[thick, opacity=0.5, text opacity=.8, xshift=-.7505cm, yshift=-1.3cm] (2,-.25) -- (2,4) node[pos=0,right] {neg.\ clause depth = 2};
