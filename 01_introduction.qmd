...
My first formal encounter with the question of behavioral equivalence came when I was implementing a translation from the process algebra Timed CSP to Timed Automata.^[
    ...
    The previous work had serious flaws:
...
The above question serves as the *research question of this thesis*.
...
It includes numerous concepts that we are going to define and discuss more deeply on our way there.
...
::::{#fig-peterson-mutex fig-pos="tbp"}
```{=latex}
...
In @sec-equivio-peterson, we will regard how the transitions of $\literal{Pe}$ derive from pseudocode and formal model.
...
This query leads to the output in @fig-peterson-spectrum,^[
  ...
  You can examine and modify the example models by clicking on the [links reading `Interactive model on equiv.io`]{.content-visible unless-profile="foc"}[triangles $\scriptsize\triangle$ next to the listings]{.content-visible when-profile="foc"}.
]
...
Blue half-circles ![](img/equivio-preord.svg){width=0.9em} indicate preordering in only from blue to red side.
...
Neither do we aim to re-survey behavioral equivalences in encyclopedic detail, which means that several notions are covered without a detailed discussion of their features and merits.
...
The following five publications fuel the thesis:
...
- “Galois Energy Games: To Solve All Kinds of Quantitative Reachability Problems” [CONCUR -@lb2025galoisEnergyGames, with Lemke] generalizes and formalizes the decision procedure of my energy game model. [Lemke's Isabelle/HOL theory: AFP -@lemke2025afpGaloisEnergyGames]
...
- “One energy game for the spectrum between branching bisimilarity and weak trace semantics” [EXPRESS/SOS -@bj2024silentStepSpectroscopyExpress, with Jansen] adapts the *spectroscopy approach for the weak spectrum*.
  [Journal version: Preprint -@bj2025silentStepSpectroscopyJournal; Isabelle/HOL theory: AFP -@bhlmmb2025weakSpectroscopyIsabelle]
...
In order to relieve the document of technical proofs, these are contained in machine-checkable *Isabelle/HOL theories* archived on <https://proofs.equiv.io/>.
They are split into two Isabelle sessions:

- *“The weak spectroscopy game to characterize behavioral equivalences”* formalizes the core results of @sec-weak-spectrum and @sec-weak-spectroscopy: <https://proofs.equiv.io/AFP/Weak_Spectroscopy/>.
  The theory [@bhlmmb2025weakSpectroscopyIsabelle] is archived in the *Archive of Formal Proofs* and has been developed together with TU Berlin students Barthel, Hübner, Lemke, Mattes, and Mollenkopf.
  Also, many theorems of earlier chapters can be understood to be supported by the formalization.
- Supplementatry proofs to support @sec-preliminaries and @sec-spectrum are sorted under: <https://proofs.equiv.io/AFP/Lineartime_Branchingtime_Spectrum_I/>.
...
- @watermann2025webAppProcessEquivalence: *“User-friendly web application for high-performance process equivalence checking.”*
- @lifante2025legalReasoningIsabelle: *“Automating legal reasoning in Isabelle/HOL: An embedding of logics for legal dynamics.”*
