...
So far, we have only looked into the characterization of bisimilarity through the whole of $\hml$ in @lem-hennessy-milner or through $\observations{\floor{B}}$ in @exm-game-formulas.
...
So, @prp-difference-contraposition (together with @lem-hennessy-milner for $\beq{B}$ and $\hml$) yields another way of establishing the hierarchy between trace equivalence, similarity, and bisimilarity of Subsection [-@sec-hierarchy-equivalences].
