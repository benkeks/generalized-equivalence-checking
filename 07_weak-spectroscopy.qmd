...
its Isabelle/HOL formalization [@bhlmmb2025weakSpectroscopyIsabelle].
...
This makes the schematic depiction in @fig-weak-spectroscopy-game-schematic already look quite entangled.
  ...
  \begin{tikzpicture}[>->,shorten <=1pt,shorten >=0.5pt,auto,node distance=2cm, rel/.style={dashed,font=\it},
    ...
    \path
      ...
      (DefStab) edge[bend left=10]
        node[align=right, pos=.4, label={[label distance=0.1cm]165:$\textcolor{gray}{-\unitvec{4}-\unitvec{8}}$}] {$Q = \varnothing$} (Def)
...
- **new:** *attacker branching positions* $\attackerpos[\eta]{p,Q} \in G_\attacker$,
...
- *$\eta$-simulation* features branching conjunction moves:
...
- *Stable simulation* allows for stable conjunction moves:
  $\attackerpos[\epsilon]{\literal{q_{ab}}, \set{\literal{P}, \literal{p_a}, \literal{p_b}}}
...
The set of *strategy formulas* for a game position $g$ and a budget $e$, $\stratweak(g, e)$, in the context of a weak spectroscopy game $\gamespectroscopyw^{\system}$ is defined inductively by the rules in @fig-weak-spectroscopy-strategy-formulas.{{<isb inductive Strategy_Formulas lts_tau.strategy_formula>}}
:::
...
\inferrule[conj.\, answer]{
  \forall q \in Q \ldotp \;
  \defenderpos{p,Q} \gamemove{u_q} \attackerpos[\land]{p,q} \; \land \;
...
\inferrule[stable\, conj.\, answer]{
  Q \neq \varnothing \\
  \forall q \in Q \ldotp \;
  \defenderpos[s]{p,Q} \gamemove{u_q} \attackerpos[\land]{p,q} \; \land \;
  e_q = \energyupdate(u_q, e) \in \attackerwins(\attackerpos[\land]{p,q}) \; \land \;
  \psi_q \in \stratweak(\attackerpos[\land]{p,q}, e_q)
} {
...
\inferrule[stable\, finishing]{
  ...
  \hmlands \set{\hmlneg\hmlobs{\tau}\hmltrue } \in \stratweak(\defenderpos[s]{p,\varnothing}, e)
}
$$
$$
...
The set of *strategy formulas* for a game position $g$ and a budget $e$, $\stratweak(g, e)$, in the context of a weak spectroscopy game $\gamespectroscopyw^{\system}$ is defined inductively by the rules in @fig-weak-spectroscopy-strategy-formulas and @fig-weak-spectroscopy-strategy-formulas-cont .{{<isb inductive Strategy_Formulas lts_tau.strategy_formula>}}
:::
...
\inferrule[procrastination]{
  \attackerpos[\varepsilon]{p,Q} \gamemove{u} \attackerpos[\varepsilon]{p',Q} \quad
  e' = \energyupdate(u, e) \in \attackerwins(\attackerpos[\varepsilon]{p',Q}) \\
  \chi \in \stratweak(\attackerpos[\varepsilon]{p',Q}, e')
} {
  \chi \in \stratweak(\attackerpos[\varepsilon]{p,Q}, e)
}
...
\inferrule[immediate\, conj.]{
  \attackerpos{p,Q} \gamemove{u} \defenderpos{p,Q} \quad
  e' = \energyupdate(u, e) \in \attackerwins(\defenderpos{p,Q}) \\
  \varphi \in \stratweak(\defenderpos{p,Q}, e')
} {
  \varphi \in \stratweak(\attackerpos{p,Q}, e)
}
...
Strategy formulas for the weak spectroscopy (continues in @fig-weak-spectroscopy-strategy-formulas-cont).
:::

:::{#fig-weak-spectroscopy-strategy-formulas-cont .column-body-outset fig-pos='t' fig-env="figure*"}
$$
\inferrule[stable\, conj.]{
  \attackerpos[\varepsilon]{p,Q} \gamemove{u} \defenderpos[s]{p,Q'} \quad
  e' = \energyupdate(u, e) \in \attackerwins(\defenderpos[s]{p,Q'}) \\
  \chi \in \stratweak(\defenderpos[s]{p,Q'}, e')
} {
  \chi \in \stratweak(\attackerpos[\varepsilon]{p,Q}, e)
}
...
$$
\inferrule[branching\, conj.]{
  \attackerpos[\varepsilon]{p,Q} \gamemove{u} \defenderpos[\eta]{p,\alpha,p',Q',Q_\alpha} \\
  e' = \energyupdate(u, e) \in \attackerwins(\defenderpos[\eta]{p,\alpha,p',Q',Q_\alpha}) \\
  \chi \in \stratweak(\defenderpos[\eta]{p,\alpha,p',Q',Q_\alpha}, e')
} {
  \chi \in \stratweak(\attackerpos[\varepsilon]{p,Q}, e)
...
\inferrule[branch.\, answer]{
  g_\mathrm{d} = \defenderpos[\eta]{p,\alpha,p',Q,Q_\alpha} \gamemove{u_\alpha} \attackerpos[\eta]{p',Q'} \gamemove{u'_\alpha} \attackerpos{p',Q'} \\
  e_\alpha = \energyupdate(u'_\alpha, \energyupdate(u_\alpha, e)) \in \attackerwins(\attackerpos{p',Q'}) \\
  \varphi_\alpha \in \stratweak(\attackerpos{p',Q'}, e_\alpha)\\
  \forall q \in Q \ldotp \;
  g_\mathrm{d} \gamemove{u_q} \attackerpos[\land]{p,q} \; \land \;
  e_q = \energyupdate(u_q, e) \in \attackerwins(\attackerpos[\land]{p,q}) \\ \; \land \;
...
Strategy formulas for the weak spectroscopy (cont.).
:::
...
If $e \in \attackerwins^{\gamespectroscopyw}(\attackerpos{p,Q})$, then there is $\varphi \in \stratweak (\attackerpos{p,Q}, e)$ with $\expr[\literal{weak}](\varphi) \leq e$.{{<isb lemma Strategy_Formulas lts_tau.winning_budget_implies_strategy_formula>}}
:::
:::proof
#### Approach
...
6. If $e \in \attackerwins(\defenderpos[\eta]{p,\alpha,p',Q \setminus Q_\alpha,Q_\alpha})$,
  then there is a $\hmlands \left(\set{\hmlsoft{\alpha}\varphi' } \cup \Psi\right) \in \stratweak(\defenderpos[\eta]{p,\alpha,p',Q \setminus Q_\alpha,Q_\alpha}, e)$ with $\expr[\varepsilon](\hmlands \left(\set{\hmlsoft{\alpha}\varphi' } \cup \Psi\right)) \leq e$. \qedhere
:::
...
then $\varphi$ distinguishes $p$ from $Q$.{{<isb lemma Strategy_Formulas lts_tau.strategy_formulas_distinguish>}}
:::
:::proof
#### Approach
...
If $\varphi \in \hmlsrbb$ distinguishes $p$ from $Q$, then $\expr[\literal{weak}](\varphi) \in \attackerwins^{\gamespectroscopyw}(\attackerpos{p,Q})$.{{<isb lemma Distinction_Implies_Winning_Budgets lts_tau.distinction_implies_winning_budgets>}}
:::
:::proof
#### Approach
...
For all $N \in \notions^\literal{weak}$, $p \in \states$, $Q \in \powerset{\states}\!$, the following are equivalent:{{<isb theorem Silent_Step_Spectroscopy lts_tau.spectroscopy_game_correctness>}}

- There exists a formula $\varphi \in \hmlsrbb$ with price $\expr[\literal{weak}](\varphi) \leq N$ that distinguishes $p$ from $Q$.
- Attacker wins $\gamespectroscopyw^{\system}$ from $\attackerpos{p,Q}$ with energy $N$.
:::

### Isabelle/HOL Formalization {#sec-isabelle-formalization}

@bhlmmb2025weakSpectroscopyIsabelle formalize the correctness result for the weak spectroscopy game in the interactive proof assistant Isabelle/HOL. 
...
The game itself is then built within the *locale* for transition systems with internal actions `lts_tau` (with a transition relation `step`)
using an `energy_game` locale.
The latter is instantiated with the moves, a predicate `spectroscopy_defender` singling out defender positions, and the $\leq$-relation on 8-dimensional energies.
...
because the grammar of $\hmlsrbb$ (@def-hml-srbb) is implemented as three mutually recursive data types (one per non-terminal).{{<isb inductive Strategy_Formulas lts_tau.strategy_formula>}}
...
For this, the theory uses the inductive characterization of $\attackerwins$ in @prp-winning-budget-rules as definition for `spectro_att_wins`.{{<isb lemma Strategy_Formulas lts_tau.winning_budget_implies_strategy_formula>}}

```isabelle
lemma winning_budget_implies_strategy_formula:
  assumes ‹spectro_att_wins e g›
  shows
    ‹case g of
...
The (mutual) induction on the formula structure to establish the distinctiveness of $\stratweak$-formulas of @lem-weak-spectroscopy-game-soundness begins:{{<isb lemma Strategy_Formulas lts_tau.strategy_formulas_distinguish>}}
...
For the other direction, the following expresses distinction completeness (@lem-weak-spectroscopy-game-completeness) that attacks exist for formulas.{{<isb lemma Distinction_Implies_Winning_Budgets lts_tau.distinction_implies_winning_budgets>}}

```isabelle
lemma distinction_implies_winning_budgets:
  assumes ‹distinguishes_from φ p Q›
  shows ‹spectro_att_wins (expressiveness_price φ)
                          (Attacker_Immediate p Q)›
```
...
The following listing reproduces it in full to convey an idea of what Isar proofs look like.{{<isb theorem Silent_Step_Spectroscopy lts_tau.spectroscopy_game_correctness>}}
...
As a cheapest distinguishing formula of `P_Para` from `P_Seq`, the spectroscopy reports `⟨ϵ⟩⟨printOutput!⟩⟨ϵ⟩Λ{⟨ϵ⟩⟨printA!⟩⊤,⟨ϵ⟩⟨printB!⟩⊤}`.
...
Schematic *simplified* weak spectroscopy game $\gamespectroscopywsimple$ with adapted branching conjunction section (the violet part).
:::
...
- Attacker wins $\gamespectroscopywsimple^{\system}$ from $\attackerpos{p,Q}$ with energy $N$.
:::
...
\colorlet{stabilityColor}{DarkViolet}
\begin{adjustbox}{trim=1cm 0cm, max width=\linewidth, center}
  \begin{tikzpicture}[>->,shorten <=1pt,shorten >=0.5pt,auto,node distance=2cm, rel/.style={dashed,font=\it},
    posStyle/.style={draw, inner sep=1ex,minimum size=1cm,minimum width=2cm,anchor=center,draw,black,fill=gray!5}]
      \node[posStyle, initial, initial text={}]
...
Modifying stable conjunction moves to include revivals (violet part).
...
Therefore, it is relieving to have the Isabelle formalization of @sec-isabelle-formalization by @bhlmmb2025weakSpectroscopyIsabelle to verify the construction.
...
Like @mg2024minDepthDistBranchBisim, we find minimal-depth distinguishing formulas for branching bisimilarity without the need for a special *until* operator, but we solve the problem for *all* weak notions at once.
...
As outlined in @sec-extending-to-other-eqs, matching adaptions in game and semantic model are usually straightforward thanks to the clear connection of productions in the modal grammar and mechanics in the spectroscopy game.

But we have assembled enough theory to perform generalized equivalence checking on common models of concurrent systems.
Time for Part IV to move beyond theory!
