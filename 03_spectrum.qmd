...
The alignment to the authoritative @glabbeek1990ltbt1 has been studied by @mattes2024expressivePowerHML.
::::
...
Equivalences become naturally comparable in the modal realm.
...
Given linearly-ordered set $(B, \leq_B)$, its $d$-ary Cartesian product with pointwise order $(B^d, \leq)$ forms a lattice, where $b \leq b'$ iff $b_k \leq b'_k$ for all $k \in \set{1, …, d}$.
...
The important point here is that we can allow *one* arbitrarily deep positive conjunct but prevent any others,
...
::: {#exm-non-ready-sim}
#### Simulation & Failures, do not mean ready-simulation
...
Intuitively, the problem is about finding the ways in which processes can be related, beyond the black-and-white of the bisimilarity problem.
So, we aim to *split up* information, analogously to a prism with light (@fig-prism), or also to more advanced forms of *spectroscopy* in physical experiments.
