...
At the heart, we aim to find a modal spectrum characterization, where HML grammar and pricing are designed in such a way that we can apply the spectroscopy approach in the following @sec-silent-step-spectroscopy.

This is where we arrive at “The Linear Time–Branching Time Spectrum II: The Semantics of Sequential Systems with Silent Moves” [@glabbeek1993ltbt], already featured in @fig-ltbts2-paper of the introduction.
We will reframe a big chunk of it to fit our modal framework.
...
::: {#idea-weak-hml .callout-idea .callout-note}
#### A Weak Subset

We can use a subset of $\hml$ to characterize most interesting weak equivalences.
:::

@sec-weak-equivalences-in-general provides some preliminaries and define the $\hml$ subset, which will correspond to the notion of stability-respecting branching bisimilarity.
We illustrate the use of some oft the weak notions through small examples in @sec-case-stduies-other-eqs.
In @sec-weak-spectrum-quantities, we unfold the modal characterization for the weak spectrum.
...
For a fuller presentation of the spectrum of weak equivalences itself, @glabbeek1993ltbt is the authorative work.
...
<!--TODO: Would $\hmland{m}{\nats} \hmlobs{\tau}^n \hmland{n}{\nats} \hmlobs{\tau}^n$ catch infinitary divergence? -> No, as the evaluation would still eventually run into a point from where one knows the bound. -->
:::
...
In @sec-tackling-case-studies, we will pick up on the examples to see how the spectroscopy algorithm can indeed simplify a researcher's life.
...
Sangiorgi notes that the induced stable bisimilarity relation would not be transitive on transition sysstems with divergences.
...
As all their non-empty word transitions end in instable states, the variant from @sangiorgi2012 ignores their differences alltogether.
...
Let *weak failure equivalence and preorder* be defined by the weak failure observations $\observations{WF}$ produced by $φ^\mathrm{WF}$ in the following grammar.
The same for *stable failure equivalence and preorder*, using $φ^\mathrm{SF}$.
$$
...
$$
  φ^\mathrm{SF} \grammardef
    ...
    \hmleps\hmland{i}{I} \hmlneg \hmlobs{α_i}\hmltrue
      \text{ where }I=\varnothing \text{ or } 0 \in I \land α_0=\tau
$$
:::
...
*Stable failures* demand a negative $\hmlneg\hmlobs{\tau}$-conjunct in the conjunctions of refused actions.
...
For instance, in $\literal{P}$ is distinguished from $\literal{Q}$ by weak failure $\hmleps\hmlands\set{\hmlneg\hmleps\hmlobs{\literal{a}}}$ and by stable failure $\hmleps\hmlands\set{\hmlneg\hmlobs{\tau}, \hmlneg\hmlobs{\literal{a}}}$ alike.
...
@fig-idle-process-abstraction presents transition systems of four processes:
...
@glabbeek1993ltbt only includes variants of stable failures in his weak spectrum, disregarding weak failures.
...
\begin{array}{rl}
  ...
  \begin{cases}
      ...
      \unitvec{2} + \unitvec{3} & \text{if there is } \hmlopt{\alpha}\varphi \in \Psi \\
...
\begin{tikzpicture}[auto,node distance=2.5cm,align=center]
  ...
  \node (F)[below right of=R]{weak failures $\notionname{WF}$\\
    \EqCoords{\infty,0,1,0,0,0,1,1}};
  \node (T)[below right of=F]{weak traces $\notionname{WT}$\\
...
The subset $\observationsvar[\literal{weak}]{\notionname{T}} = \observationsvar[\literal{weak}]{(\infty,0,0,0,0,0,0,0)} \subseteq \hmlsrbb$ can be given by the following grammar:
\begin{align*}
  \varphi_\notionname{T} \grammardef & \; \hmleps\hmlobs{a}\varphi_\notionname{T}
              \grammaror \hmleps\hmltrue 
              \grammaror \hmltrue
              \qquad \text{with $a \in \actionsvis$.}
\end{align*}
There is a formula $\varphi \in \observationsvar[\literal{weak}]{T}$ distinguishing $p$ from $q$ precisely if $p$ is *not* trace-preordered to $q$.{{<isb lemma Weak_Traces LTS_Tau.trace_equals_trace_to_formula>}}
:::
:::proof
For a word $\vec{w} = a_1 \dots a_n \in \actionsvis^*$, the formula $\hmleps\hmlobs{a_1}\dots\hmleps\hmlobs{a_n}\hmleps\hmltrue \in \observationsvar[\literal{weak}]{T}$ is true for a state $p$ precisely if there is a $p'$ such that $p \stepweakword{\vec{w}} p'$, that is, if $\vec{w} \in \wtraces{p}$.
...
The interesting points are highlighted in $\textcolor{RoyalBlue}{blue}$.
Productions that could be left out without affecting distinctiveness are set in $\textcolor{gray}{gray}$.

:::{#fig-weak-grammars-linear fig-pos='t!'}
...
:::{#fig-weak-grammars-bisim-like fig-pos='t!'}
...
\parbox{12cm}{
  ...
  \intertext{
    ...
    \chi_{\eta\notionname{B}} \grammardef &
      ...
      \grammaror \hmlands \set{ \mathhl{\hmlopt{\alpha}\varphi_{\eta\notionname{B}}}, \psi_{\eta\notionname{B}}, \psi_{\eta\notionname{B}}, \ldots } \\
    \psi_{\eta\notionname{B}} \grammardef &
  ...
  \intertext{
    ...
    \chi_{\notionname{BB}} \grammardef &
      \grammarsp \hmlobs{a}\varphi_{\notionname{BB}}
      \grammaror \hmlands \set{ \psi_{\notionname{BB}}, \psi_{\notionname{BB}}, \ldots }
      \grammaror \hmlands \set{ \mathhl{\hmlopt{\alpha}\varphi_{\notionname{BB}}}, \psi_{\notionname{BB}}, \psi_{\notionname{BB}}, \ldots } \\
    \psi_{\notionname{BB}} \grammardef &
      \grammarsp \hmlneg\hmleps\chi_{\notionname{BB}}
      \grammaror \hmleps\chi_{\notionname{BB}}\\
  \intertext{
...
The first interesting feature of the weak hierachy is an effect of some languages not enforcing top-level negation as observed in @bj2024silentStepSpectroscopyExpress [Ex. 2.5].
...
@rem-ambiguous-coordinates on ambiguous coordinates has mentioned that $(\infty, \infty, 0, 0, \infty, \infty) \in \notions^\literal{strong}$, which disallows positive conjuncts, already characterizes strong bisimilarity.
...
:::{#fig-weak-grammars-sim-like fig-pos='t!'}
...
\parbox{12cm}{
  ...
  \intertext{
    ...
    \psi_{\eta\notionname{S}} \grammardef &
      ...
      \grammaror \hmleps\hmlands \set{ \mathhl{\hmlopt{\alpha}\varphi_{\eta\notionname{S}}}, \psi_{\eta\notionname{S}}, \psi_{\eta\notionname{S}}, \ldots }
...
Several of our weak grammars allow bogous trivial conjuncts.
...
Even more, the minimal winning budgets in the spectroscopy energy game will only ever lead to witness formulas that are minimal with respect to syntactic expressiveness prices.
...
This adds the preliminaries to consider their decision procedures in @sec-silent-step-spectroscopy.

In examples, we have seen how weak equivalences handle internal non-determinism due to communcation in different ways.
Practical scenarios, such as parallelizing compilers and process abstraction, reveal the possibilities and limitations of the different design decisions behind process equivalences.
...
We however have skipped some classical notions such as stable failure traces and coupled simulation, which would have called for even more dimensions in the already 8-dimensional pricing metric.^[
  Especially for coupled similarity, this gap is ironic, as my research into game chracterizations of equivalences has been kicked off by an interest in decision procedures for coupled similarity in @bn2019coupledsimTacas and @bnp2020coupledsim30.
]
