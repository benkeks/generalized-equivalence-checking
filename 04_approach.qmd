...
After introducing such games in @sec-energy-games, we will prove in @sec-characterzing-ptime-part that several core equivalences can be characterized attacker energy budgets where the defender wins.
...
The section's proofs give the blueprint for proof of stronger results for more general games in upcoming chapters.
...
- $φ = \hmlobs{α}φ'$ with $φ' ∈ \observationsvar[\literal{poly-strong}]{e - \unitvec{1}}$.
  As $φ$ distinguishes $p$ from $q$, there must be a $p' ∈ \semantics{φ'}$ such that $p \step{α} p'$ and, for all $q' ∈ \derivatives{q, α}$, $q' \notin \semantics{φ'}$.
  Due to the induction hypothesis, $e - \unitvec{1} ∈ \attackerwins^{\game_\notionname{B}^🗲}([p', q'])$ for all such $q' ∈ \derivatives{q, α}$.
  Therefore, $e - \unitvec{1} ∈ \attackerwins^{\game_\notionname{B}^🗲}((α, p', q))$.
  As the attacker can move $[p, q] \gamemove{-\unitvec{1}} (α, p', q)$, this proves $e ∈ \attackerwins^{\game_\notionname{B}^🗲}([p, q])$ by @prp-winning-budget-rules.
- $φ = \hmlands\setΨ$ one of the $ψ ∈ Ψ$ must be false for $q$.
  Consider the possible forms of $ψ$:
    - Case $ψ = \hmlobs{α}φ'$ with $φ' ∈ \observationsvar[\literal{poly-strong}]{e - \unitvec{1}}$.
      Then we can apply the same argument as in the case of $φ = \hmlobs{α}φ'$.
    - Case $ψ = \hmlneg\hmlobs{α}φ'$ with $φ' ∈ \observationsvar[\literal{poly-strong}]{(e \sqcap (e_2, \infty, \infty)) - \unitvec{1} - \unitvec{3}}$.
      Therefore, $\hmlobs{α}φ' ∈ \observationsvar[\literal{poly-strong}]{(e \sqcap (e_2, \infty, \infty)) - \unitvec{3}}$ distinguishes $q$ from $p$.
      This time, we can employ the same argument as in the case of $φ = \hmlobs{α}φ'$ to obtain that $(e \sqcap (e_2, \infty, \infty)) - \unitvec{3} ∈ \attackerwins^{\game_\notionname{B}^🗲}([q, p])$.
      The move $[p, q] \gamemove{\updmin{1,2},0,-1}^🗲_\notionname{B} [q, p]$ and $\energyupdate((\updmin{1,2},0,-1), e) = (e \sqcap (e_2, \infty, \infty)) - \unitvec{3}$ justify that $e ∈ \attackerwins^{\game_\notionname{B}^🗲}([p, q])$.
      \qedhere
:::
