...
# Spectroscopy of the Strong Equivalence Spectrum {#sec-strong-spectroscopy}
...
::::{#related-5 .column-margin}
`\vspace{-2.1em}`{=latex}
{{< paragraph_heading "Related publications." >}}
This chapter showcases the main result of “Process equivalence problems as energy games” [@bisping2023equivalenceEnergyGames].
We move beyond it by also deriving individual equivalence games from our characterization and showing how these enable polynomial-time decision procedures for $\complexityname{P}$-easy notions.
::::
...
The core ingredient of this chapter is a richer energy game, we dub “spectroscopy game,” to be presented in @sec-strong-spectro-game.
...
::: {#idea-subset-construction .callout-idea .callout-tip}
#### Subsets to separate observations and conjunctions
...
Some of it can be alleviated, as we will see in @sec-specific-spectro-games, but most is necessary when deciding strong notions collectively.
In @sec-deciding-individual-eqs, we discuss how to get rid of the blow-up where possible when instantiating the game to *decide equivalences individually*.

::: {#idea-instance-games .callout-idea .callout-tip}
#### Generating equivalence checkers
...
@fig-strong-spectroscopy-big-picture gives an overview of the two routes to decide equivalences collectively and individually that we will explore.

::: {#fig-strong-spectroscopy-big-picture fig-pos="t" fig-env="figure*"}
```tikz
\begin{adjustbox}{max width=\linewidth, center}
  \begin{tikzpicture}[auto,node distance=1.1cm,
    algstep/.style={minimum width=4cm, minimum height=1.2cm, draw=gray, rectangle,align=center,rounded corners}]
    \node[algstep, draw] (States) {States $p, q$ in \\ transition system $\system$};
    \node[algstep, draw, right of=States, node distance=7.5cm] (Preorders) {Strong notions that \\ fit to preorder $p$ to $q$};
    ...
    \path
      ...
      (States) edge[->, draw=TealBlue] node {Definition~\ref{def-strong-spectroscopy-game}} (EnergyGame)
      (AttackerWinningBudgets) edge[->, draw=TealBlue] node[label={[align=center]right:XOR}] {Theorem~\ref{thm-strong-spectroscopy-game-characterization}} (Preorders)
      (EnergyGame) edge[->, draw=TealBlue] node {Algorithm~\ref{algo-deciding-energy-games}} (AttackerWinningBudgets);
    
    \path
      (States) edge[->, dashed, bend left=20] node {$N$-preorder problem} (IndividualNotion)
      (Preorders) edge[->, gray] node {} (IndividualNotion)
      (EnergyGame) edge[->, draw=Peach] node {Definition~\ref{def-derived-eq-game}} (ReachabilityGame)
      (ReachabilityGame) edge[->, draw=Peach] node {Algorithm \ref{algo-deciding-games}} (AttackerWinningReachability)
    ...
    \node[text=TealBlue, right=2mm of AttackerWinningBudgets, align=right] (Sec51) {Section \ref{sec-strong-spectro-game} \\ and \ref{sec-specific-spectro-games}};
    \node[text=Peach, right=2mm of AttackerWinningReachability] (Sec52) {Section \ref{sec-deciding-individual-eqs}};
...
## The Strong Spectroscopy Game {#sec-strong-spectro-game}
...
But by the closing arguments of @sec-spectrum-approach-discussion, and also according to @martensGroote2023minimalDistFormulasNP, this must inherently be $\complexityname{NP}$-hard.
...
As long as observations happen, the defender is moved to the set $Q$ of states that are reachable through nondeterminism; only at conjunction moves does the defender have to choose a $q \in Q$.
Treating conjunctions more explicitly allows us to track the depth of positive conjuncts in sufficient detail.

::: {#fig-strong-spectroscopy-game fig-pos="t" fig-env="figure*"}
  ...
  \begin{tikzpicture}[>->,shorten <=1pt,shorten >=0.5pt,auto,node distance=2cm,
    ...
    \path
      ...
      (AttConj) edge[bend right=15]
        node[align=center, label={[label distance=0.0cm]-90:$\textcolor{gray}{\updmin{\textcolor{obsColor}{1},\textcolor{negObsColor}{5}},0,0,0,0,\textcolor{negsColor}{-1}}$}] {$p \neq q$} (AttSwap)
...
- The third, fourth, and fifth dimensions limit how deeply observations may nest underneath a conjunction, for which they do not need to change themselves.
  The third stands for one of the deepest positive conjuncts and the fourth for the other positive conjuncts; the fifth stands for negative conjuncts.
- The last dimension limits how often the attacker may use negations to enforce symmetry by swapping sides.

The moves of the following definition closely match productions in the grammar of observations for the strong spectrum of @def-strong-spectrum.

::: {#def-strong-spectroscopy-game}

#### Strong spectroscopy game

For a system $\system={(\states,\actions,\step{})}$,
the $6$-dimensional *strong spectroscopy game*
$\gamespectroscopy^{\system}=(G,G_{\defender},\gamemoveblank,w)$
consists of

- *attacker (main) positions* $\attackerpos{p,Q} \in G_\attacker$,
...
To point out a similar distinction in the spectroscopy game, the attacker moves via $\step{\tau}$ as follows:
$$
\begin{array}{rcl}
...
#### Half-simulated philosophers

Let us see what our prototype implementation on {{< equivio >}} reports about our original example processes $\literal{P}$ and $\literal{Q}$ of @sec-preliminaries.
We can input the philosopher processes of @exm-ccs and @exm-deterministic-phil,^[
  A usage guide on the syntax can be found in @sec-equivio-usage.
]
obtaining a transition system matching @exm-ts:
...
Starting `@compare` (on {{< equivio >}}) triggers a spectroscopy along the lines we have discussed, leading to the output:^[
  A spectroscopy game on this example can also be played through in the computer game *The Spectroscopy Invaders*, see @sec-spectro-invaders.
]
...
That no equivalence besides or above simulation can hold, is justified to the user by the failure `⟨τ⟩Λ{¬⟨a⟩⊤}`, which we also discussed in @sec-incomparabilities.
In the upcoming @sec-correctness-strong-spectroscopy, we will also give the rules that the tool uses to construct this witness in $\stratstrong$.

Invoking `@compare Q, P` for the other direction, {{< equivio >}} reports `⟨τ⟩Λ{⟨b⟩⊤,⟨a⟩⊤}` as a distinguishing formula disproving simulation (as we have found in @exm-hml).
:::

The spectroscopy game still is a bisimulation game in the following sense.

::: {#prp-spectroscopy-bisimulation-game}

#### Defender bisimilarity

States $p_0$ and $q_0$ are bisimilar
precisely if
the defender wins $\gamespectroscopy$ from $\attackerpos{p_0, \{q_0\}}$ for every initial energy budget $e_0$,
...
:::{#rem-spectroscopy-triangle}
#### The $\triangle$-symbol

We use the $\triangle$ in $\gamespectroscopy$ with a double meaning:
To symbolize a *prism*, which reveals the spectrum of light,
and to mean *difference*, as the game expresses an abstract form of subtraction in the sense of @sec-abstract-subtraction.
:::
...
In the context of a strong spectroscopy game $\gamespectroscopy^{\system}$,
the set of *strategy formulas* $\stratstrong(g, e)$ for a game position $g$ and a budget $e$ is defined inductively by the following rules in @fig-strategy-formulas-strong.
:::

::: {#fig-strategy-formulas-strong fig-pos="h" fig-env="figure*"}
$$
...
\inferrule[conjunction]{
  ...
  e ∈ \attackerwins^{\gamespectroscopy}(\defenderpos{p, Q', Q_*}) \\
...
\inferrule[conj.~answer]{
  \begin{matrix}
    \forall q ∈ Q \ldotp \;
    \defenderpos{p, Q, \varnothing} \gamemove{u_q} \attackerpos[\wedge]{p, q}
    \; \land \;
    e_q = \energyupdate(u_q, e) ∈ \attackerwins^{\gamespectroscopy}(\attackerpos[\wedge]{p, q})
    \\ \land \;
    \psi_q ∈ \stratstrong(\attackerpos[\wedge]{p, q}, e_q)
  \end{matrix}
}{
...
$$
$$
\inferrule[conj.~revival]{
  \begin{matrix}
    \forall q ∈ Q \ldotp \;
    \defenderpos{p, Q, Q_*} \gamemove{u_q} \attackerpos[\wedge]{p, q}
    \; \land \;
    e_q = \energyupdate(u_q, e) ∈ \attackerwins^{\gamespectroscopy}(\attackerpos[\wedge]{p, q})
    \\ \land \;
    \psi_q ∈ \stratstrong(\attackerpos[\wedge]{p, q}, e_q)
...
Rules to derive distinguishing formulas from winning attack strategies.
:::

\noindent
The base case of the definition is the rule for *conjunction answers* at $\defenderpos{p, \varnothing, \varnothing}$
It yields the strategy formula $\hmltrue$, which trivially distinguishes any $p$ from the empty set.

The correctness proof for $\gamespectroscopy$ proceeds basically as with the bisimulation energy game $\game^{🗲}_{\notionname{B}}$, establishing soundness as in @lem-bisim-energy-game-soundness and completeness as in @lem-bisim-energy-game-completeness.
...
then there is a formula $φ ∈ \stratstrong(\attackerpos{p, Q}, e)$ with $φ ∈ \observationsvar[\literal{strong}]{e}$, $p ∈ \semantics{φ}$ and $Q \cap \semantics{φ} = \varnothing$.^[
...
If there is $φ ∈ \observationsvar[\literal{strong}]{N}$ with $p ∈ \semantics{φ}$ and $Q \cap \semantics{φ} = \varnothing$,
then $N ∈ \attackerwins^{\gamespectroscopy}(\attackerpos{p, Q})$.^[
...
#### $\notions^\literal{strong}$-characterization

$p \bpreordvar{\observationsvar{N}} q$ for $N ∈ \notions^\literal{strong}$ precisely if the defender wins $\gamespectroscopy$ for the attacker starting from $\attackerpos{p,\set{q}}$ with budget $N$.
:::

Thus, we can solve the spectroscopy problem (@prob-spectroscopy-problem) by computing $\notions^\literal{strong}_{p,q} = \notions^\literal{strong}_{\vphantom{p}} \setminus \attackerwins^{\gamespectroscopy}(\attackerpos{p, \set{q}})$.
...
The conference paper results apply to the present presentation *mutatis mutandis*.
However, we will achieve better complexity results in the next section, thanks to @lb2025galoisEnergyGames.
:::
...
This comes as no surprise, for we have already discussed in @sec-equivalence-complexities that the spectroscopy problem on the whole strong spectrum is $\complexityname{PSPACE}$-hard.

::: {#thm-strong-spectroscopy-complexity}
#### Strong spectroscopy complexity
...
The number of attacker main positions is bounded by $\relsize{\states} \cdot 2^{\relsize{\states}}$.
...
This leads to $o_{\gamemoveblank}$ in $\bigo{2^{\relsize{\states}}}$ and to $\relsize{G_{\triangle}}$ in $\bigo{\relsize{\states} \cdot 3^{\relsize{\states}}}$.
Filling in the time bounds of @lem-declining-update-complexity yields:
$$
\begin{array}{rlccccccc}
    & O( & o_{\gamemoveblank} & \cdot & \relsize{G}^{2 \cdot d} & \cdot & (d^2 + \relsize{G}^{d-1} \cdot d) & ) \\
...
2. Since the specific coordinates in @fig-strong-ltbt-spectrum only use 0, 1, and ∞ in components, we can employ @lem-declining-update-complexity-flattened to slightly improve complexity bounds.
3. With more specific equivalences in mind, we can also focus on subgraphs of the spectroscopy game $\gamespectroscopy$ in order to obtain better bounds.

@sec-pruning-logic will first discuss how parts of the game graph can be pruned thanks to properties of bisimilarity.
Then, we use all listed tricks in the “clever” strong spectroscopy game in @sec-clever-strong-spectro.
In the subsequent @sec-deciding-individual-eqs, we will take the last point to the extreme by instantiating the spectroscopy game down to only decide single equivalences.

### Pruning with Logic {#sec-pruning-logic}

We can exploit that the spectroscopy game is a bisimulation game by @prp-spectroscopy-bisimulation-game to reduce game graph size.
...
On a strong spectroscopy game, $p \in Q$ implies $\attackerwins(\attackerpos{p,Q}) = \varnothing$.
:::
:::proof
...
All game graph parts that are only reached through such positions can be disregarded.

Another important trick to reduce game size is to first apply bisimulation minimization to the transition system and then solely construct the spectroscopy game for the minimized system, $\gamespectroscopy^{\quotient{\system}{\beq{B}}}$.
This approach is sound by the following lemma.

::: {#lem-quotienting-spectro-game}
#### Quotienting by bisimilarity

The attacker wins $\attackerpos{p, \set{q}}$ on $\gamespectroscopy^{\system}$ with $e$ precisely if they win $\attackerpos{\eqclass{p}_{\beq{B}}}, \set{\eqclass{q}_{\beq{B}}}$ on $\gamespectroscopy^{\smash{\quotient{\system}{\beq{B}}}}$.
:::
::: proof
...
The system has only $\relsize{\states} = 20$ states, but a lot of nondeterminism due to saturation with internal behavior.

:::: {#fig-minimization-lattice .column-margin}

```tikz
%\begin{adjustbox}{max width=\linewidth, center, keepaspectratio}
...
The bisimulation reduction is surprisingly effective, given that the bisimulation minimized Peterson system still has 19 states, that is, only one fewer than the original system.
...
This over-proportionate effect on the one hand has to do with the general exponentialities of the game graph, but also with the fact that bisimulation minimization increases the instances where pruning is applicable.^[
  This interplay can already be seen on the bisimulation game.
  @fig-bisim-sim-game contains a big region after $[\literal{t_{ab}}, \literal{q_{ab}}]$ and $[\literal{t_{ab}}, \literal{q_{ab}}]$.
  Bisimulation minimization of the input system would shrink this region as $\literal{t_{ab}} \beq{B} \literal{q_{ab}}$.
  The two positions would be merged into $[\set{\literal{t_{ab}}, \literal{q_{ab}}}, \set{\literal{t_{ab}}, \literal{q_{ab}}}]$.
  But this is a symmetric position!
  Therefore, we can drop this part altogether.
]
:::

The implementations (@sec-implementations) sometimes use small additional tricks for pruning.
For instance, one can leave out observation moves behind $\attackerpos{p, \varnothing}$ positions and finish the game optimally by conjunction.
...
In the following, we focus on the lower-resolution sub-spectrum $\widehat{\notions}^\literal{strong} \subseteq \set{0, 1, \infty}^6 \subseteq \notions^\literal{strong}$, where dimensions 3, 4, and 5 of modal depth for revivals, positive and negative conjuncts can only appear in certain combinations.

::: {#def-simpler-strong-spectrum}
#### Simpler strong spectrum

The *simpler strong spectrum*, $\widehat{\notions}^\literal{strong}$, is defined as
$$
\widehat{\notions}^\literal{strong} ≔
  \set{N \in \set{0, 1, \infty}^6 \mid N_4 \leq N_3 \land (N_5 = \infty \longrightarrow N_3 = N_4)}.
$$
:::

Observe that all the coordinates of the strong linear-time–branching-time spectrum (@fig-strong-ltbt-spectrum) are still covered by the simpler spectrum.
...
- $\set{ q \in Q \mid \initials{q} \subseteq \initials{p} }$ – enabledness-dominated revivals,
- $\set{ q \in Q \mid \initials{p} \subseteq \initials{q} }$ – enabledness-dominating revivals, or
- $\set{ q \in Q \mid \initials{p} = \initials{q}}$ – enabledness-matching revivals.
:::

The idea here is that the attacker only thinks about certain partitionings:
Those where the $Q_*$-revival deals with parts of $Q$ that cannot be discharged through failure or readiness observations.
The usage of $\initials{\cdot}$ is comparable to a small look-ahead into the possibilities to win through observations inside the conjunction.

::: {#thm-clever-strong-spectroscopy-correctness}
#### Correctness of cleverness

For $N \in \widehat{\notions}^\literal{strong}$, the attacker wins $\gamespectroscopyclever$ from $\attackerpos{p_0,Q_0}$ with energy $N$ precisely if they win $\gamespectroscopy$ from $\attackerpos{p_0,Q_0}$ with energy $N$.^[
  Proof in Theorem 2 of @bisping2023equivalenceEnergyGamesReport.
]
:::

::: {#thm-clever-strong-spectroscopy-complexity}
#### Complexity of cleverness
...
Now, the number of all game positions is bounded by $\relsize{G_{\blacktriangle}} \in \bigo{\relsize{\states} \cdot 2^{\relsize{\states}}}$.
...
$$
\begin{array}{rlccccccc}
    & O( & o_{\gamemoveblank_\blacktriangle} & \cdot & \relsize{G_\blacktriangle}^{2 \cdot d} & \cdot & (d^2 + 3^{d-1} \cdot d)) \\
...
This is due to the high nondeterminism of the Peterson system, which leads to big $Q$-sets in $\attackerpos{p,Q}$-positions.
...
Without these tricks, the approach would not be applicable to systems with relevant nondeterminism due to communication.^[
  The blow-up on Peterson's mutex is precisely the argument I made in the introduction of @bisping2023equivalenceEnergyGames for why one has to move beyond the original spectroscopy algorithm of @bjn2022decidingAllBehavioralEqs.
  Therefore, the original/deprecated approach also is not present in this thesis.
]
:::

## Deciding Individual Equivalences {#sec-deciding-individual-eqs}
...
This section is about the feasibility to decide individual preorders using the spectroscopy game.
...
Then, @algo-deciding-games can be employed to decide player winning regions of the game and thus the equivalence.
...
@sec-regaining-polynomiality will *regain polynomiality* for the $\complexityname{P}$-easy portion of equivalence problems.

### Deriving Equivalence Games {#sec-deriving-eq-games}

Let us first explicate how to characterize equivalences through reachability games derived according to @def-derived-reachability-game.
...
is defined as the part of the reachability game $(\gamespectroscopy^{\system})^R$ derived from the spectroscopy game
under starting position $(\attackerpos{p,\set{q}}, N)$.
:::

We thus receive a game characterization for every equivalence, generalizing Stirling's result of the bisimulation game from @thm-bisim-game-characterization.
...
Lifted via @prp-derived-reachability-energies, this implies $(\attackerpos{p, \set{q}}, N) \notin \attackerwins$ for the derived reachability game $(\gamespectroscopy^{\system})^R$.
By @def-derived-eq-game, the defender thus wins $\smash{\gamespectroscopyeq{N}^{\system,p,q}}$.
...
Using @algo-deciding-games, our results entail decision procedures for each individual equivalence of the strong spectrum.
...
The following processes are two of the more complex ones of the separating examples by @glabbeek2001ltbtsiReport [p. 59].
...
Interestingly, the derived reachability game of @fig-derived-trace-eq-game has *fewer* positions than the pruned clever spectroscopy game below $\attackerpos{\literal{ABCACB}, \set{\literal{ABC}}}$ would have.^[
...
The {{< equivio >}} implementation does prune some attacker moves that cannot be winning, in particular non-empty conjunctions if the conjunction budget is at 0.
Therefore, the subgraph used by the tool for @exm-spectroscopy-equivalence-checking in reality is the one in @fig-derived-trace-eq-game-true.

:::{#fig-derived-trace-eq-game-true fig-pos='t'}
```{dot}
digraph rel{
695410661 [shape=square, color=blue, style=bold, label="e, {e,f}\n(∞,0,0,0,0,0)"];
945131307 [shape=square, color=blue, style=bold, label="g, {d,g}\n(∞,0,0,0,0,0)"];
11287976 [shape=square, color=blue, style=bold, label="b.d + c.e, {b.d + c.e + c.f + b.g}\n(∞,0,0,0,0,0)"];
-58372795 [shape=square, color=blue, style=bold, label="c.f + b.g, {b.d + c.e + c.f + b.g}\n(∞,0,0,0,0,0)"];
-1478041764 [shape=square, color=blue, style=bold, label="ABCACB, {ABC}\n(∞,0,0,0,0,0)"];
1777001188 [shape=square, color=blue, style=bold, label="d, {d,g}\n(∞,0,0,0,0,0)"];

-1092705565 [shape=square, color=blue, style=bold, label="f, {e,f}\n(∞,0,0,0,0,0)"];-58372795->945131307[color=black, label="(-1,0,0,0,0,0)"];
-58372795->-1092705565[color=black, label="(-1,0,0,0,0,0)"];
-1478041764->11287976[color=black, label="(-1,0,0,0,0,0)"];
-1478041764->-58372795[color=black, label="(-1,0,0,0,0,0)"];
11287976->1777001188[color=black, label="(-1,0,0,0,0,0)"];
11287976->695410661[color=black, label="(-1,0,0,0,0,0)"]}
```

The subgraph of @fig-derived-trace-eq-game that actually is used in {{< equivio >}}, disregarding some attacker options.
:::

Still, nondeterminism in the transition system might lead to exponentially-sized derived games.
Especially in the case of bisimilarity and similarity, such a blow-up is not necessary.
As @sec-approach has shown, they should be $\complexityname{P}$-easy.

### Regaining Polynomiality in Derived Games {#sec-regaining-polynomiality}

So far, the derived games include the exponentiality due to the subset construction.
But often, we can restrict the attacker to only use a polynomially-sized portion of the game.
If they have energy for unbounded conjunctions, this restriction will not affect their wins.
...
This is a side benefit of the spectroscopy characterization (@lem-strong-spectroscopy-game-soundness and -@lem-strong-spectroscopy-game-completeness) through distinctions, as 
$Q \cap \semantics{\varphi} = \varnothing$ implies $Q' \cap \semantics{\varphi} = \varnothing$ for any $\varphi$.
...
:::
:::proof
Any observation move $(\attackerpos{p, Q}, e) \gamemoveblank (\attackerpos{p', Q'}, e')$ must be due to a transition $p \step{α} p'$ with $Q \step{α} Q'$. 
...
After the defender choice of $q \in Q$, the game continues with the same energy $\dots \gamemoveblank (\attackerpos[\wedge]{p, q}, \mathhl{e}) \gamemoveblank (\attackerpos{p, \set{q}}, e) \gamemoveblank (\attackerpos{p', \derivatives{q, α}}, e')$, thanks to the sufficient energy budget.
As $\derivatives{q, α} \subseteq Q'$, we can use @prp-defender-options.
:::

Thus the closure of a logic under conjunctions allows us to use a game that, practically, leaves out many formulas, without losing distinctiveness.
This trick is basically the same as in the crucial $\hmlobs{\alpha}$-case of @lem-game-formula-hml that “game HML” $\observations{\floor{B}}$ and $\hml$ are equally distinctive.
...
#### Polynomial equivalence checking

For a strong notion $N \in \set{0, 1, \infty}^6$ with $N \geq (0,\infty,\infty,\infty,0,0)$, and a system $(\states,\actions,\step{})$,
one can decide $p \bpreordvar{\observationsvar{N}} q$ in polynomial time,
by computing who wins the $N$-preorder game in $\bigo{\relsize{\states} \cdot \relsize{\step{}}}$ time and space.
:::
:::proof
...
For $N \geq (0,\infty,\infty,\infty,0,0)$, this also works on the subgraph where nondeterminism is resolved immediately thanks to @lem-conjunction-optimization.
...
We observe that the $\set{0,1,\infty}$-valued energy components in the game might only create a constant-factor increase in the size of $\gamespectroscopyceq{N}$, compared to the underlying game $\gamespectroscopyclever$.^[
  If we were to allow $k$-step equivalences as in @fig-polynomial-strong-spectrum, $k$ would also become a factor.
]
Therefore, the following arguments focus on the latter.

There are at most $\relsize{\states}^2$ positions of the form $\attackerpos{p,\set{q}}$.
Whenever they reach $\attackerpos{p',Q'}$ with $\relsize{Q'} > 1$,
@lem-conjunction-optimization allows us to prune away all observation moves deriving from there, and return to an attacker main position with $\relsize{Q''} \leq 1$.
The intermediate $\attackerpos{p',Q'}$-positions are a function of initial $q$ and the transitions from $p$, and thus collectively bounded by $\bigo{\relsize{\states} \cdot \relsize{\step{}}}$, which bounds attacker positions as a whole.
...
Also, the subsequent attacker conjunct positions, bounded by $\relsize{\states}^2$, do not affect the bound.

For the moves, only observation moves and conjunct answers can impact complexity.
...
Inserting our bounds in @prp-reachability-game-complexity, we obtain time and space complexity of $\bigo{\relsize{\states} \cdot \relsize{\step{}}}$.^[
  This is with the understanding that one can ignore storage space for the positions:
  One can use pointers to $Q'$-sets in the attacker and defender positions, and not store them verbatim.
  This saves the bound as there can only be $\relsize{\step{}}$ such sets of size $\relsize{Q'} > 1$.
  So, overall, these sets also take up $\bigo{\relsize{\states} \cdot \relsize{\step{}}}$ space.
]
:::

::: {#exm-instance-game-sizes}
#### Measurements of derived games

Let us examine how the instance game sizes behave on the games of @exm-spectroscopy-equivalence-checking!
@fig-instance-games-strong-ltbt-spectrum maps out the sizes of game instantiations for the preorders of the strong spectrum as in @fig-strong-ltbt-spectrum.^[
  The reported numbers are produced via `sbt "shared/run eqchecks --strong-game"` for the pair `L50/R50` by {{< code_link "shared/src/main/scala-2.12/io/equiv/eqfiddle" tool.benchmark.LTBTSEquivalenceChecks >}}.
]
Each game is derived from the clever spectroscopy game, with pruning and with capped nondeterminism where possible; to enable equivalence checks, the games combine the parts under $\attackerpos{\literal{ABCACB}, \set{\literal{ABC}}}$ and under $\attackerpos{\literal{ABC}, \set{\literal{ABCACB}}}$.
...
:::: {#fig-instance-games-strong-ltbt-spectrum fig-pos="t"}
...
%\begin{adjustbox}{max width=\textwidth}
  \begin{tikzpicture}[auto,node distance=2cm,align=center]
    ...
    \node[fill=purple!30!yellow!60] (IF)[below right of=R]{
      $\relsize{\gamespectroscopyceq{\notionname{IF}}} = 105$\\
      $\infty,1,0,0,\infty,1$};
    \node[fill=purple!32!yellow!60] (S)[left of=FT, node distance=3cm]{
      $\relsize{\gamespectroscopyceq{\notionname{1S}}} = 95$\\
...
The game for ready traces $\gamespectroscopyceq{\notionname{RT}}$, where all dimensions play a role, is the most costly.
:::

## Discussion {#sec-discussion-strong-spectroscopy}

With this chapter, we have broadened the spectroscopy game approach of @sec-approach to account for the full strong spectrum of @sec-spectrum, thereby solving its spectroscopy problem (@prob-spectroscopy-problem).

{{< paragraph_heading "One game, many notions." >}}
The critical ingredient has been to *cover trace-like observations* through a *subset construction* on states (@idea-subset-construction).
This construction can also be viewed game-theoretically as a way to model an attacker with *imperfect information* about the exact choices the defender makes in simulating steps.
The conjunction and negation points then describe moves where the attacker can obtain this information.
@fahrenberg2014quantitativeLTBTS employ this alternative view in their game description for quantitative variants of strong equivalences.
...
The *general idea of giving generalized games* for the strong spectrum first appears in @shr1995hornsatGames.
It receives a fuller treatment by @cd2008gameCharacetrizations.
Both publications, however, use word-transition moves for trace-like equivalences.
In algorithms, this is unfortunate, since most interesting systems allow infinitely long transition sequences:
For finite-state systems, this infinity appears as soon as there is a transition cycle.
Therefore, the subset construction (or imperfect-information view) is technologically superior.
On the side of theory, the link between moves in the game and conjunctions in modal logics adds clarity to our construction.
It reinforces the idea of @sec-abstract-subtraction that equivalence checking is, in a way, subtraction of satisfied formulas.

{{< paragraph_heading "Difference to original formulation." >}}
This chapter's presentation improves in several ways on my original spectroscopy games.
The idea to alternate *state subsets for nondeterministic words and conjunction moves to eliminate the blow-up* is introduced by @bisping2021ltbtsTacas.
Our initial 2021 game is unable to correctly handle failure-traces and similar observations, which has been corrected in @bjn2022decidingAllBehavioralEqs, thanks to Jansen's input.
With respect to @bjn2022decidingAllBehavioralEqs, the presentation in this chapter has three advantages:

1. @bjn2022decidingAllBehavioralEqs uses a reachability game, on which we construct distinguishing formulas.
  The formulas are then pruned on-the-fly to select those with minimal syntactic prices.
  Introduced by @bisping2023equivalenceEnergyGames, we now can perform the whole *spectroscopy on energy vectors*, thereby making costly explicit formula construction optional.
2. @bjn2022decidingAllBehavioralEqs's game has super-exponential out-degree for conjunction moves.
  With the present formulation [from @bisping2023equivalenceEnergyGames], we can achieve constant degree, using the cleverness of @sec-clever-strong-spectro.
3. On a pure reachability game, one cannot easily instantiate to select the subgraphs for individual equivalences as in @sec-deciding-individual-eqs.

\vspace*{-\medskipamount}

{{< paragraph_heading "Back to many games." >}}
The point of *deriving equivalence games from the energy game* (@idea-instance-games) connects the one-energy-game approach back to the hierarchy-of-games approach of prior publications.
With @sec-deciding-individual-eqs, the present chapter has moved beyond the scope of @bisping2023equivalenceEnergyGames.
Interestingly, @sec-regaining-polynomiality shows that, by prioritizing conjunctions in derived simulation-like games, one obtains $\bigo{\relsize{\states} \cdot \relsize{\step{}}}$ complexity, restoring the $\complexityname{P}$-easiness of the preceding chapter.
For strong similarity, this matches the best known time bound, as discussed in @sec-equivalence-complexities.
For bisimilarity, however, partition refinement outperforms the game approach.

We could go a step further in the polynomially-sized derivatives of the spectroscopy game, and “bake together” the attacker main positions after observations with the following defender positions.
This would (almost) arrive at the game structure of last chapter's bisimulation energy game!
Conceptionally, the following relation emerges:
The bisimulation game $\gamebisim$ of @sec-preliminaries *is a shadow of* the bisimulation energy game $\gamebisimen$ of @sec-approach, which in turn *is a shadow of* the strong spectroscopy game $\gamespectroscopy$ of the present chapter.

The dimensions of energy games allow us to encode different games *within* one game.
This is analogous to stereoscopic pictures, decodable by 3D glasses (@fig-3d-glasses).

:::{#fig-3d-glasses .column-margin}

{{< pic img/3d-games-dcon25.jpg >}}

The author, discussing 3D energy games with participants of D-CON'25.
{{< photo_credits "Nadine Karsten" >}}
:::

{{< paragraph_heading "What's next?" >}}
Continuing the trajectory of shadows, Part III of the thesis will abstract further what we have been doing.
The strong spectrum of equivalences is usually too strong for the analysis of concurrent systems.
Therefore, we will extend our approach to the *weak spectrum* of equivalences, accounting for the nuances of silent $\tau$-steps.
This is of double relevance:
Practical applications of concurrency theory tend to call for weak semantics,
