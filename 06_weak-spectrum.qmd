...
In upcoming definitions and facts, we use the convention that $\alpha$ continues to stand for elements of $\actions$, while $a$ comes from $\actions \setminus \set{\tau}$.
...
It is a popular anecdote among those who know branching bisimilarity that the original “assumed” proof of its transitivity does not exist.
Six years after its inception, @bastern1996branchingBisimEqIndeed closes the gap by a proof that is surprisingly complex.
...
## Case Studies and the Need for Other Equivalences
...
```ruby
print("output:") || x = compute_A_or_B()
print(x)
```

The `||` connector is supposed to mean a parallel execution of commands that synchronizes the branches at the end.
In real-world programming languages, this would happen through spawning and joining subprocesses, future objects, or `async` blocks.
...
The action $\literal{ao}$ stands  for announcing the coming output, and $\literal{A}$ and $\literal{B}$ are the values of `x` that are actually printed.
