...
- There are various *quantitative notions*, often understood in terms of *behavioral distances*.
  ...
  Combining it with our game would likely demand deeper changes to our game mechanics, because one has to remember traces (and not just current states) to compute the resulting distances in generarlity.
  ...
  So, this might be the kind of quantitaive equivalences, where a connection to our approach works best.
...
- Some kinds of bisimilarity are based on *special modalities*.
  ...
  For modalities where satisfaction depends on infinite behavior, the reachability game framework is not sufficently expressive.
...
This thesis has approached the rich body of work on equivalence and refinement through one unified lense of *finding differences* in behavior.
