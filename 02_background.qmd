...
We say a sublogic $\observationsvar{X} ⊆ \hml$ *preorders* state $p$ to $q$, $p \bpreordvar{\observationsvar{X}} q$, if no $φ ∈ \observationsvar{X}$ is distinguishing $p$ from $q$.
If the preordering goes in both directions, we say that $p$ and $q$ are equivalent with respect to sublogic $\observationsvar{X}$, written $p \beqvar{\observationsvar{X}} q$.
:::
...
::: {#prp-hml-eq}
Consider an arbitrary HML sublogic $\observationsvar{X} ⊆ \hml$.
Then, $\bpreordvar{\observationsvar{X}}$ is a preorder,
and $\beqvar{\observationsvar{X}}$ an equivalence relation.
:::
