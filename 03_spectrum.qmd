...
However, we order the equivalences using the approach of parameterized *notions of observability* from the later “weak spectrum” by @glabbeek1993ltbt.

@sec-obs-hierarchies provides the background on observability hierarchies, which motivate the chapter's first core idea:
...
So, @prp-difference-contraposition that sublogics equate at least as much as their parent logic, yields another way of establishing the entailment hierarchy between bisimilarity, similarity, and trace equivalence of @sec-hierarchy-equivalences.^[Taking the two preceding lemmas together with @thm-hennessy-milner for $\beq{B}$ and $\hml$.]
...
{{< pic "img/hermanns-hartmanns-spectrum.jpg" "80%" >}}
...
but is something that we cannot observe using simulation observations.
...
Equivalence hierarchy including failure equivalence.
::::
...
- Dually, some $B' \subseteq B$ with no two distinct elements $b_1, b_2 \in B'$ such that $b_1 \leq b_2$ is called an *anti-chain*.
- The *upward closure* ${\upset\! B'}$ of a $B' \subseteq B$ is defined as $\set{b \in B \mid \exists b' \in B' \ldotp b' \leq b}$.
  Analogously, the *downward closure* is given by ${\downset\! B'} \defeq \set{b \in B \mid \exists b' \in B' \ldotp b \leq b'}$.
:::

::: {#wrapper-fig-set-lattice}
...
{{< contents wrapper-fig-set-lattice >}}
...
:::: {#fig-nat-lattice fig-pos="H"}
{{< contents wrapper-fig-nat-lattice >}}
...
Van Glabbeek's two papers on the “linear-time--branching-time spectrum” [-@glabbeek1990ltbt1 ;-@glabbeek1993ltbt] show how all common equivalences can be understood to form a hierarchy of sublanguages of $\hml$ (and a variant of $\hml$ for equivalences with silent steps).
His hierarchy of equivalences derives from a lattice of *notions of observability*.^[
...
(From now on, there are no arrows of implication:
The lines stand for $\leq$ or $\subseteq$ relationships in the lattice!)
::::
:::
...
Note that, given our definition of spectra, the minimum of @def-expressiveness-price does not need to exist---but it does for the spectra we will be working with.
...
A visualization for how $\expr[\literal{strong}](\hmlobs{τ}\hmlands\set{\hmlneg\hmlobs{\literal a}\hmltrue}) = (2,1,0,0,1,1)$ (according to @def-expressiveness-price) comes together is given in @fig-formula-pricing.
...
\varphi^{(2,1,0,0,1,1)} & \leadsto & \hmlobs{τ}\varphi^{(1,1,0,0,1,1)} \\
                        ...
                        & \leadsto & \hmlobs{τ}\hmlands\set{\hmlneg\varphi^{(1,0,0,0,1,0)}} \\
...
::: {#fig-formula-pricing fig-pos="tb"}
```tikz
\begin{tikzpicture}
  \node(Obs1) {$\hmlobs\tau$};
...
Pricing formula $\hmlobs{τ}\hmlands\set{\hmlneg\hmlobs{\literal a}\hmltrue}$ with syntactic expressiveness price of $(2,1,0,0,1,1)$.
:::
...
::: {#fig-formula-pricing-complex fig-pos="t"}
```tikz
...
This change is done to align the pricing to the one that we need for the weak spectrum in later chapters.
...
The conjunction productions with “$\dots$” are to be read in the sense that they allow arbitrarily many conjuncts; in particular, empty and infinitary conjunctions are possible.
...
This entails a lot of fine print, which we do not reproduce here.
...
We will refer to the deep positive conjunct of a conjunction as the *revival*, because revivals equivalence [@rrs2007responsivenessRevivals] is the minimal notion with this feature.
It also matters for ready traces, as seen in the ready trace formula of @fig-formula-pricing-complex.
...
- $k$-nested similarity: $(\infty,\infty,\infty,\infty,\infty,k-1)$ for $k>1$ defines a spectrum of modal quantifier alternation depth between similarity and bisimilarity [@grooteVaandrager1992tyfttyxt].
:::
...
:::: {#fig-spectroscopy-front fig-pos='t'}
...
This falls in line with our observations about the $\leq$-game on numbers in @exm-less-eq-game and (bi-)simulation games.
Comparison is inherently linked to subtraction.
Some object $A$ is less or equal to some $B$ if nothing is left after we take everything that makes up $B$ away from $A$.
...
The best known sequential bisimilarity algorithms for finite-state transition systems take $\bigo{\relsize{\step{}} \log \relsize{\states}}$ time.
They usually employ partition refinement [e.g. @valmari2009bisimMin], deriving from @paigetarjan1987partitionrefinement.

::: {#fig-complexity-slope fig-pos="t"}
```tikz
\begin{tikzpicture}[]
  \begin{axis}[
        title = {},
    ...
    \addplot+ [smooth, thick, TealBlue, mark options={fill=TealBlue}] coordinates {(ID,0) (GI,4) (B,2) (S,3) (T,5) (E,1) (U,0)};
    \draw[draw=DarkViolet] (axis cs:{[normalized]1.9},-0.1) rectangle (axis cs:{[normalized]6.1},5.1);
  \end{axis}
\end{tikzpicture}
```
...
For coarser simulation-like equivalences, the best known algorithms need $\bigo{\relsize{\step{}} \relsize{\states}}$ time [@rt2010efficientSimulation].^[Or $\bigo{\relsize{\step{}} \relsize{\smash{\relqotient{\states}{\beq{S}}}}}$ to name the bound as @rt2010efficientSimulation present it.]
