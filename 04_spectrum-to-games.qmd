...
An $d$-dimensional *declining energy game* is played on energy vectors:
...
Given a weight labeling $w \colon (\gamemoveblank) \to \energyupdates$, an $d$-dimensional *declining energy game* $\game^🗲$ is an energy reachability game with energies $\abstractenergies ≔ \energies$ and with edges labeled $\abstractupdate(m) ≔ \energyupdate(w(m), \cdot)$.
...
The undo-update function $\energyupdateinv$ on an $d$-dimensional declining energy game is defined component-wise for $k \in \set{1 … d}$ as:
$$
\begin{array}{rl}
...
@alg-deciding-energy-games solves the energy game problem on an $d$-dimensional *declining* energy game $\game^🗲=(G,G_\defender,\gamemoveblank, w)$ in time
$$
...
@alg-deciding-energy-games solves the *flattened* energy game problem on an $d$-dimensional declining energy game $\game^🗲=(G,G_\defender,\gamemoveblank, w)$ in time
$$
