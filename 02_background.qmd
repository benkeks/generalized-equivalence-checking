...
They are accompanied by many more notions for behavioral equivalence and refinement to relate or distinguish their models.

This chapter takes an agnostic *and* opinionated tour into this field.
We are *agnostic* in building on the most basic and versatile formalism of *labeled transition systems* and on standard equivalences such as *trace equivalence* and *bisimilarity*.
The approach is *opinionated* in focussing on Robin Milner's tradition of concurrency theory with a strong pull towards *game characterizations*.

@fig-equivalence-big-picture shows the scaffold we plan to build to understand behavioral equivalences, instantiated to bisimilarity:

- @sec-behavioral-eq introduces bisimilarity through its *relational characterization* in terms of symmetric simulation relations.
- @sec-hml treats the 1:1-connection of bisimilarity to the distinguishing powers of a *modal logic*, known as the Hennessy--Milner theorem
- @sec-games shows that both, the relational and the modal characterization, appear as certificates of defender and attacker strategies in a reachability game.
...
\begin{tikzpicture}[auto,node distance=1.1cm,
  ...
  \node[algstep] (Bisim) {$p \beq{B} q$\\ $p$ and $q$ are bisimilar};
  \node[algstep, right of=Bisim, node distance=7.5cm] (NotBisim) {$p \nbeq{B} q$ \\ $p$ and $q$ are not bisimilar};
  \node[below of=Bisim, rotate=270, label={[align=center]270:(Def~2.7)}] (Biff) {$:\!\!\iff$};
  \node[below of=NotBisim, gray, rotate=270] (NBiff) {$\iff$};
  \node[algstep, below of=Biff] (Rel) {There is bisimulation $\rel{R}$\\such that $(p,q) \in \rel{R}$};
  \node[algstep, below of=NBiff] (Distinction) {Some $\varphi \in \hml$\\distinguishes $p$ from $q$};
  \node[below of=Rel, rotate=270, label={[align=right]270:Stirling's\\characterization}] (Siff) {$\iff$};
  \node[below of=Distinction, rotate=270, label=90:{Section 2.4.4}] (Giff) {$\iff$};
  \node[algstep, below of=Siff] (DefenderWins) {Defender wins bisimulation\\game from $[p,q]$};
  \node[algstep, below of=Giff] (AttackerWins) {Attacker wins bisimulation\\game from $[p,q]$};
  \path
    (Bisim) edge node[label=below:{(Def~2.7)}] {XOR} (NotBisim)
    (Rel) edge node[label={[align=center]below:Hennessy--Milner\\theorem}] {XOR} (Distinction)
    (DefenderWins) edge node[label={[align=center]below:Determinacy\\of games}] {XOR} (AttackerWins);
\end{tikzpicture}
```

Core-correlations between behavioral equivalence, modal distinguishability, and game for bisimilarity.
:::
...
::: {#nte-idea-modal-first .callout-note}
#### Modal first!
...
To provide them a bridge, we will start with relational and denotational definitions---but once we have crossed the bridge, we will not go back.
...
and $\initials{p}$ for $\set{α \mid \exists p' \ldotp p \step α p'}$
:::
...
Thirdly, as a corollary of @prp-difference-contraposition, modal characterizations *ensure equivalences to be abstractions of bisimilarity*, which is a sensible base notion of equivalence.
...
::: {#def-leq-distincitve}
We say that an $\actions$-observation language $\observationsvar{}$ is *less or equal in distinctiveness* to another $\observationsvar{}'$ iff, for any $\actions$-transition system and states $p$ and $q$, for each $φ ∈ \observationsvar{}$ that distinguishes $p$ from $q$, there is $φ' ∈ \observationsvar{}'$ that distinguishes $p$ from $q$.{{<isb definition LTS_Semantics lts_semantics.leq_distinctive>}}
...
*Gale--Stewart-style reachability games* [in the tradition of @galeStewart1953infiniteGamesPerfectInformation] where the defender wins all infinite plays.
...
An *attacker strategy* is a (partial) function mapping play fragments ending at attacker positions to next positions to move to, $\rel f_\attacker \colon G^*G_\attacker \to G$, where $g_\attacker \gamemoveblank \rel f_\attacker(\rho g_\attacker)$ must hold for all $\rho g_\attacker$ where $\rel f_\attacker$ is defined.

A play ${g_0}{g_1} … ∈ G^{\infty}$ is consistent with an attacker strategy $\rel f_\attacker$ if, for all its prefixes $g_0 … g_i$ ending in $g_i \in G_\attacker$, $g_{i + 1} = \rel f_\attacker(g_0 … g_i)$.

*Defender strategies* are defined analogously, $\rel f_\defender \colon G^*G_\defender \to G$.

If $\rel f$ ensures a player to win, $\rel f$ is called a *winning strategy* for this player.
The player with a winning strategy for $\game(g_{0})$ is said to *win* $\game(g_{0})$.

Usually, we will focus on positional strategies, that is, strategies that only depend on the most recent position, which we will type $\rel f_\attacker \colon G_\attacker \to G$ (or $\rel f_\defender \colon G_\defender \to G$, respectively).
:::
...
The reachability games we consider essentially are a subclass of parity games only colored by 0.
So the games are positionally determined.
This means, for each possible initial position, exactly one of the two players has a positional winning strategy $\rel f$.
...
This means we distribute the roles such that the defender winning matches output `True` and the attacker output `False`, and the two base cases need to be dead ends for one of the players.
...
But the following games will demand the players to make choices.
:::
...
Let $\rel f$ be a positional winning strategy for the attacker on ${\game}_\notionname{B}$ from $[p,q]$.
...
The following @tbl-game-example-execution lists how @alg-deciding-games computes the winning region $\attackerwins = \set{[\literal{2a}], (\literal{3})}$ of the game of @exm-game-formulas.
...
: Solving the game of @exm-game-formulas {#tbl-game-example-execution}
...
- relational characterizations = defender view; modal characterizations = attacker view.
  - mention axiomatic characterizations
