...
This section is about adapting the idea of back-propagating how the defender loses in reachability games from @alg-deciding-games to work for (declining) energy games as well.

The change is that we have to propagate not just attacker wins, but Pareto frontiers of attacker-winning energy levels.
...
::: {#exm-galois-equality}
If a function $f \colon C \to A$ is surjective and injective, it has a unique inverse function $f^{-1}$ such that $f^{-1} \circ f = \identity{C}$ and $f \circ f^{-1} = \identity{A}$.
If $f$ is monotone, $f$ and $f^{-1}$ form a Galois connection (for all partial orders on $A$ and $C$).
:::
