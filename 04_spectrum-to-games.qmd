...
To implement a back-propagation algorithm, we have to assume that we know a function that undoes energy updates, $\abstractupdateinv \colon (\gamemoveblank) \to (\abstractenergies \cup \set{\bot}) \to (\abstractenergies \cup \set{\bot})$ with $\abstractupdateinv(g, g')(e') = \inf \set{e \mid e' \leq_\bot \abstractupdate(g, g')(e)}$.
That means, we assume that there is a Galois connection $(\abstractupdateinv, \abstractupdate)$ on energy vectors.
...
TODO: Add illustrations how attacker winning budgets are combined at attacker position, and how they are intersected at defender positions.
