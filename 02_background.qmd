...
{{<noindent>}} The program described by the transitions from $\literal{P}$ choses non-deterministically during a $τ$-step between two options and then offers only either $\literal a$ *or* $\literal b$.
...
- If there is a positional defender strategy $\rel s$ winning from $[p, q]$,
  ...
  $$\rel{R_s} ≔ \set{(p,q) \mid \text{there is a play $(p_0, q_0),\dots,(p,q)$ following strategy $\rel s$}}.$$
:::
...
: Solving the game of @exm-game-formulas {#tbl-game-example-execution fig-pos='b'}
