...
This chapter revolves around the ideas I aimed to convey by my talk “Deciding All Behavioral Equivalences at Once through Energy Games” at D-CON'25 (Augsburg, 2025-03),
...
We demand the following energy games to be monotonic in the following sense:
...
Generalizing the concept winning regions as in @def-winning-region, we define *winning budgets* for game positions:

::: {#def-winning-budgets}
#### Winning budgets

For each position $g_0 ∈ G$ of energy game $\game^🗲$, the *attacker winning budgets*, $\attackerwins(g_0) \subseteq \abstractenergies$ are the energies $e_0$ where the attacker wins $\game^🗲(g_0, e_0)$.
The defender winning budgets $\defenderwins$ are defined analogously.

We write the shorthand $\attackerwinsmin$ for *minimal attacker winning budgets*, $\attackerwinsmin(g_0) \defeq \Min(\attackerwins(g_0))$.
:::
...
No, as all outgoing paths would lead to $⊥$-energy and thus to the defender winning, before reaching $\literal{g_6}$.
...
We now turn to a special kind of monotonic energy games with vector-valued energy levels where updates will never increase energy levels in any component.
...
The author, illustrating multi-dimensional energy updates on declining games to the audience of CAV'23.  
(Photo: Fei Bian)
:::
...
$$
\notions^\literal{peasy-strong} ≔ \natsext^3,
$$
and the family of strong observation languages
$$
\observationsvar[\literal{peasy-strong}]{(N_1, N_2, N_3) ∈ \notions^\literal{peasy-strong}} ≔ \observationsvar[\literal{strong}]{(N_1,∞,∞,∞,N_2,N_3)}.
$$
:::

\noindent
In effect, $\observationsvar{N ∈ \notions^\literal{peasy-strong}}$ can be characterized by $φ^N$ in the following grammar:
$$
\begin{array}{rcll}
...
@fig-bisim-energy-game-scheme visualizes the scheme of game rules, differing from @fig-game-scheme through energy updates.
...
\begin{adjustbox}{max width=\textwidth, center}
  \begin{tikzpicture}[>->, shorten <=1pt, shorten >=0.5pt, auto, node distance=2cm,
    ...
    \node[posStyle, initial, initial text={}, label={[text=red]90:$\hmlands\set{\hmlneg\hmlobs{\tau}\hmlands\set{\hmlneg\hmlobs{\literal{a}}\hmltrue}} \in \observationsvar[\literal{peasy-strong}]{(2,2,2)}$}]
      (Att_Q_Qp){$[\literal{Q}, \literal{T}]$};
    \node[posStyle, label={[text=red]270:$\hmlobs{\tau}\hmlands\set{\hmlneg\hmlobs{\literal{a}}\hmltrue} \in \observationsvar[\literal{peasy-strong}]{(2,1,1)}$}]
    ...
    \node[posStyle, label={[text=red]95:$\hmlobs{\literal{a}}\hmltrue \in \observationsvar[\literal{peasy-strong}]{(1,0,0)}$}]
    ...
    \node[posStyle, label={[text=red]270:$\hmlands\set{\hmlneg\hmlobs{\literal{a}}\hmltrue} \in \observationsvar[\literal{peasy-strong}]{(1,1,1)}$}]
...
Thus, $\notions^{\literal{peasy-strong}}_{\literal{Q}, \literal{T}} = \set{N ∈ \notions^\literal{peasy-strong} \mid (2,2,2) \nleq N}$ is the solution for the ($\complexityname{P}$-easy strong) spectroscopy problem.
...
then there is $φ ∈ \strat_\notionname{B}([p, q], e)$ with $φ ∈ \observationsvar[\literal{peasy-strong}]{e}$, $p ∈ \semantics{φ}$ and $q \notin \semantics{φ}$.
:::
::: proof
We prove the following (more general) property by induction.

1. If $e ∈ \attackerwins^{\game_\notionname{B}^🗲}([p, q])$,
then there is $ψ ∈ \strat_\notionname{B}([p, q], e)$ of form $ψ = \hmlobs{α}φ'$ or $\hmlneg\hmlobs{α}φ'$ with $\hmlands\set{ψ} ∈ \observationsvar[\literal{peasy-strong}]{e}$, $p ∈ \semantics{ψ}$ and $q \notin \semantics{ψ}$.
2. If $e ∈ \attackerwins^{\game_\notionname{B}^🗲}((α, p', q))$,
then there is $φ ∈ \strat_\notionname{B}((α, p', q), e)$ of form $φ = \hmlands Ψ$ with $φ ∈ \observationsvar[\literal{peasy-strong}]{e}$, $p' ∈ \semantics{φ}$ and $q \notin \semantics{\hmlobs{α}φ}$.
...
1. Assume $e ∈ \attackerwins^{\game_\notionname{B}^🗲}([p, q])$.
  This must be due to one of the following moves:
    - Case $[p, q] \gamemoveblank^🗲_\notionname{B} [q, p]$ with $e' = \energyupdate((\updmin{1,2},0,-1), e) ∈ \attackerwins^{\game_\notionname{B}^🗲}([q, p])$.
      By induction hypothesis on $[q, p]$, there is $ψ ∈ \strat_\notionname{B}([q, p], e')$ with $\hmlands\set{ψ} ∈ \observationsvar[\literal{peasy-strong}]{e'}$, $q ∈ \semantics{ψ}$ and $p \notin \semantics{ψ}$.
      Consider the possible forms of $ψ$:
        - $ψ = \hmlobs{α}φ'$.
          ...
          Also, $\hmlands\set{\hmlneg\hmlobs{α}φ'} ∈ \observationsvar[\literal{peasy-strong}]{e}$, by the calculations of the grammar in @def-polynomial-strong-spectrum.
        - $ψ = \hmlneg\hmlobs{α}φ'$.
          ...
          Clearly, $\hmlobs{α}φ'$ must distinguish $p$ from $q$ and $\hmlands\set{\hmlobs{α}φ'} ∈ \observationsvar[\literal{peasy-strong}]{e}$.
    - Case $[p, q] \gamemoveblank^🗲_\notionname{B} (α, p', q)$ with $p \step{α} p'$ and $e' = \energyupdate((-1,0,0), e) ∈ \attackerwins^{\game_\notionname{B}^🗲}((α, p', q))$.
      By induction hypothesis on $(α, p', q)$, there is $\hmlands Ψ ∈ \strat_\notionname{B}((α, p', q), e')$ and $\hmlands Ψ ∈ \observationsvar[\literal{peasy-strong}]{e'}$ with $p' ∈ \semantics{\hmlands Ψ}$ and $q \notin \semantics{\hmlobs{α}\hmlands Ψ}$.
      ...
      Also, $\hmlobs{α}\hmlands Ψ ∈ \observationsvar[\literal{peasy-strong}]{e}$ as $\hmlands Ψ ∈ \observationsvar[\literal{peasy-strong}]{e - \unitvec{1}}$.
2. Assume $e ∈ \attackerwins^{\game_\notionname{B}^🗲}((α, p', q))$.
  ...
  For each $q'$, we employ the induction hypothesis to obtain $ψ_{q'} ∈ \strat_\notionname{B}([p', q'], e)$ of form $ψ_{q'} = \hmlobs{α}φ'$ or $\hmlneg\hmlobs{α}φ'$ with $\hmlands\set{ψ_{q'}} ∈ \observationsvar[\literal{peasy-strong}]{e}$, $p' ∈ \semantics{ψ_{q'}}$ and $q' \notin \semantics{ψ_{q'}}$.
  ...
  Moreover, $(\hmland{q'}{\derivatives{q, α}} ψ_{q'}) ∈ \observationsvar[\literal{peasy-strong}]{e}$ by the grammar.\qedhere 
:::

::: {#lem-bisim-energy-game-completeness}
#### Distinction completeness

If there is $φ ∈ \observationsvar[\literal{peasy-strong}]{N}$ with $p ∈ \semantics{φ}$ and $q \notin \semantics{φ}$,
then $N ∈ \attackerwins^{\game_\notionname{B}^🗲}([p, q])$.
:::
::: proof
By induction on the grammar of $\observationsvar[\literal{peasy-strong}]{N}$.

Consider the cases of $φ ∈ \observationsvar[\literal{peasy-strong}]{N}$ with $p ∈ \semantics{φ}$ and $q \notin \semantics{φ}$.

- $φ = \hmltrue$.
  This cannot be the case as $q \notin \semantics{φ}$ and $\semantics{\hmltrue} = \states$.
- $φ = \hmlobs{α}φ'$ with $φ' ∈ \observationsvar[\literal{peasy-strong}]{N - \unitvec{1}}$.
  As $φ$ distinguishes $p$ from $q$, there must be a $p' ∈ \semantics{φ'}$ such that $p \step{α} p'$ and, for all $q' ∈ \derivatives{q, α}$, $q' \notin \semantics{φ'}$.
  Due to the induction hypothesis, $N - \unitvec{1} ∈ \attackerwins^{\game_\notionname{B}^🗲}([p', q'])$ for all such $q' ∈ \derivatives{q, α}$.
  Therefore, $N - \unitvec{1} ∈ \attackerwins^{\game_\notionname{B}^🗲}((α, p', q))$.
  As the attacker can move $[p, q] \gamemove{-\unitvec{1}} (α, p', q)$, this proves $N ∈ \attackerwins^{\game_\notionname{B}^🗲}([p, q])$ by @prp-winning-budget-rules.
- $φ = \hmlands Ψ$ one of the $ψ ∈ Ψ$ must be false for $q$.
  Consider the possible forms of $ψ$:
    - Case $ψ = \hmlobs{α}φ'$ with $φ' ∈ \observationsvar[\literal{peasy-strong}]{N - \unitvec{1}}$.
      Then we can apply the same argument as in the case of $φ = \hmlobs{α}φ'$.
    - Case $ψ = \hmlneg\hmlobs{α}φ'$ with $φ' ∈ \observationsvar[\literal{peasy-strong}]{(N \sqcap (N_2, \infty, \infty)) - \unitvec{1} - \unitvec{3}}$.
      Therefore, $\hmlobs{α}φ' ∈ \observationsvar[\literal{peasy-strong}]{(N \sqcap (N_2, \infty, \infty)) - \unitvec{3}}$ distinguishes $q$ from $p$.
      This time, we can employ the same argument as in the case of $φ = \hmlobs{α}φ'$ to obtain that $(N \sqcap (N_2, \infty, \infty)) - \unitvec{3} ∈ \attackerwins^{\game_\notionname{B}^🗲}([q, p])$.
      The move $[p, q] \gamemove{\updmin{1,2},0,-1}^🗲_\notionname{B} [q, p]$ and $\energyupdate((\updmin{1,2},0,-1), N) = (N \sqcap (N_2, \infty, \infty)) - \unitvec{3}$ justify that $N ∈ \attackerwins^{\game_\notionname{B}^🗲}([p, q])$.
      \qedhere
:::

::: {#thm-bisim-energy-game-characterization}
#### $\notions^\literal{peasy-strong}$-characterization
...
Many monotonic functions naturally induce a Galois-connected abstraction function.
...
To implement a back-propagation algorithm, we have to assume that we know a function that undoes energy updates, $\abstractupdateinv \colon (\gamemoveblank) \to \abstractenergies \to \abstractenergies$ with $\abstractupdateinv(g, g')(e') = \min \set{e \mid e' \leq \abstractupdate(g, g')(e)}$.
That means, we assume that there is a Galois connection $(\abstractupdateinv(g, g'), \abstractupdate(g, g'))$ between energies $\abstractenergies$ and the domain of updates $\set{e ∈ \abstractenergies \mid \abstractupdate(e) \neq \bot}$ for each $(g,g') \in {\gamemoveblank}$.
...
\begin{array}{rl}
  ...
  14 &  \quad \quad \quad \variable{attacker\_win}[g] ≔ \variable{new\_attacker\_win} \\
  15 &  \quad \quad \quad \variable{todo} ≔ \variable{todo} ∪ (\cdot \gamemoveblank \variable{g}) \\
  16 &  \quad \attackerwinsmin ≔ \variable{attacker\_win} \\
  17 &  \quad \kw{return} \attackerwinsmin \\
\end{array}
$$
...
We define it as:
$$
\abstractupdateinv(m)(e') ≔ \sup(e - \vec{u}, \zerovec) \text{\; if $m$ is $\vec{u}$-labeled}
$$
...
3. $\literal{g_5} ∈ G_\attacker$ is updated with the $\abstractupdateinv((0, -1))((0,0)) = \sup((0,0) - (0,-1), \zerovec) = (0,1)$.
...
4. Now, we can find attacker winning budgets for $\literal{g_4} ∈ G_\defender$:
  ...
  Unless the attacker at least arrives with energy $\sup((0,1), (1,0)) = (1,1)$, the defender has a way of winning.
...
with computable undo function $\abstractupdateinv(g, g')(e') = \min \set{e \mid e' \leq \abstractupdate(g, g')(e)}$ for all moves,
...
- Minimality: For all $e' \leq \energyupdate(u, e)$, we show $\energyupdateinv(u, e') \leq e$.
  ...
  Let us define $d' ≔ \max (\set{0} \cup \set{e'_j \mid \exists D \ldotp u_j = \mathtt{min}_D \land k \in D})$.
...
$$
\bigo{o_{\gamemoveblank} \cdot \relsize{G}^{2 \cdot d} \cdot (d^2 + \relsize{G}^{d-1} \cdot d)}
$$
...
\begin{align*}
  ...
  =\; & \bigo{ \relsize{G}^2 \cdot o_{\gamemoveblank} \cdot (\relsize{G}^{d-1})^2 \cdot (d^2 + d + \relsize{G}^{d-1} \cdot d) } \\
  =\; & \bigo{ o_{\gamemoveblank} \cdot \relsize{G}^{2d+2-2} \cdot (d^2 + \relsize{G}^{d-1} \cdot d) } \\
  =\; & \bigo{ o_{\gamemoveblank} \cdot \relsize{G}^{2d} \cdot (d^2 + \relsize{G}^{d-1} \cdot d) }
\end{align*}
...
$$
\bigo{ o_{\gamemoveblank} \cdot \relsize{G}^{2d} \cdot (d^2 + 3^{d-1} \cdot d) }
$$
...
We have gathered enough material to have this section prove that the spectroscopy problem for the $\complexityname{P}$-easy strong spectrum is indeed solvable in polynomial time, thus justifying the name we chose for the spectrum.
...
$$\notions^\literal{peasy-strong}_{p,q} = \defenderwins^{\game_\notionname{B}^🗲}([p,q]).$$
By the determinacy of winning budgets (@prp-winning-budget-determinacy),
$$\notions^\literal{peasy-strong} \setminus \notions^\literal{peasy-strong}_{p,q} = \attackerwins^{\game_\notionname{B}^🗲}([p,q])$$
and thus
$$\Min(\notions^\literal{peasy-strong} \setminus \notions^\literal{peasy-strong}_{p,q}) = \attackerwins^{\min\game_\notionname{B}^🗲}([p,q]).$$
Thereby, @prob-spectroscopy-problem for $\notions^\literal{peasy-strong}$ on a transition system reduces to @prob-energy-game-problem on the corresponding bisimulation energy game $\game_\notionname{B}^🗲$.
...
the spectroscopy problem for the $\notions^\literal{peasy-strong}$-spectrum can be solved in polynomial time with respect to the size of $\system$.
:::
:::proof
@thm-bisim-energy-game-characterization has established that we can solve the spectroscopy problem for the $\notions^\literal{peasy-strong}$-spectrum by deciding the winning budgets of the bisimulation energy game ${\game}_\notionname{B}^🗲$ on $\system=(\states,\actions,\step{})$.
...
This chapter has shown how to solve the spectroscopy problem for the $\notions^{\literal{peasy-strong}}$ spectrum in polynomial time, thereby deciding all its preorders and equivalences *at once*.
...
how to conveniently decide for a pair of states which notions from a spectrum of behavioral equivalences equate the two.
In the second half of the thesis, we will leverage this approach to treat the “strong spectrum” of concrete equivalences and the “weak spectrum” of equivalences that abstract silent steps.
