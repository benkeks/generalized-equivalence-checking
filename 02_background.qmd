...
\begin{array}{cllr}
  φ \grammardef &
    \hmlobserve{α}φ & \quad\text{with } α ∈ \actions &
      \text{“observation”} \\
    {}\grammaror \quad & \hmland{i}{I}φ_i & \quad\text{with index set } I & \text{“conjunction”} \\
    {}\grammaror \quad & \hmlneg φ & & \text{“negation”} \\
\end{array}
$$
:::
...
We also usually omit them when writing down formulas, e.g., shortening $\hmlobserve{\literal a}\hmlobserve{\literal b}\hmltrue$ to $\hmlobserve{\literal a}\hmlobserve{\literal b}$.

The intuition behind HML is that it describes *what sequences of behavior* one may or may not *observe* of a system.
Observations $\hmlobserve{α}…$ are used to build up possible action sequences;
...
$$
\begin{array}{rcl}
  \semantics{\hmlobserve{α}φ} & := & \set{p \mid \exists p' ∈ \semantics{φ} \ldotp p \step α p'},\\
...
- $\literal P, \literal Q ∈ \semantics{\hmlobserve{τ}\hmlobserve{\literal a}}$ as both expose the trace $τ\literal{a}$,
- $\literal Q ∈ \semantics{\hmlobserve{τ}\hmlands\set{\hmlobserve{\literal a}, \hmlobserve{\literal b}}}$, but $\literal P \notin \semantics{\hmlobserve{τ}\hmlands\set{\hmlobserve{\literal a}, \hmlobserve{\literal b}}}$.
- $\literal P ∈ \semantics{\hmlobserve{τ}\hmlneg\hmlobserve{\literal a}}$, but $\literal Q \notin \semantics{\hmlobserve{τ}\hmlneg\hmlobserve{\literal a}}$.
...
We say a sublogic $\observationsvar{X} \subseteq \hml$ *preorders* state $p$ to $q$ if no $φ ∈ \observationsvar{X}$ is distinguishing $p$ from $q$.
:::

{{<noindent>}} By this account, $\hmlobserve{τ}\hmlneg\hmlobserve{\literal a}$ of @exm-hml distinguishes $\literal P$ from $\literal Q$.
$\hmlobserve{τ}\hmlands\set{\hmlobserve{\literal a}, \hmlobserve{\literal b}}$, on the other hand, distinguishes $\literal Q$ from $\literal P$.
(The direction matters!)
For instance, the sublogic $\set{\hmlobserve{τ}\hmlobserve{\literal a}, \hmlobserve{τ}\hmlobserve{\literal b}}$ preorders $\literal P$ and $\literal Q$ in both directions.
...
$$\semantics{P} := \{ φ ∈ \hml \mid P ∈ \semantics{φ} \}.$$
::: -->
