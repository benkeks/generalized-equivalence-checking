...
\begin{array}{rrcll}
  ...
  \textit{conjunction}
  & \attackerpos{p,Q}
  & \gamemove{0,-1,0,0,0,0}
  & \defenderpos{p,Q \setminus Q_*, Q_*}
  & \text{if $Q_* \subseteq Q$,}
  \\
  \textit{conj.~revival}
  & \defenderpos{p,Q,Q_*}
  & \gamemove{\updmin{1,3},0,0,0,0,0}
  & \attackerpos{p,Q_*}
  & \text{if $Q_* \neq \varnothing$,}
  \\
  \textit{conj.~answer}
  & \defenderpos{p,Q,Q_*}
  & \gamemove{0,0,0,\updmin{3,4},0,0}
...
\begin{tikzpicture}[>->,shorten <=1pt,shorten >=0.5pt,auto,node distance=2cm,
  ...
  \path
    ...
    (Att) edge
      node[label={[label distance=0cm]-90:$\textcolor{gray}{0,\textcolor{conjColor}{-1},0,0,0,0}$}] {$Q_* \subseteq Q$} (Def)
    (Def) edge[bend left=15]
      node[align=right, swap, pos=.1] {$q \in Q \setminus Q_*$\\$\textcolor{gray}{0,0,0,\updmin{\textcolor{mainPosColor}{3},\textcolor{otherPosColor}{4}},0,0}$} (AttConj)
    ...
    (Def) edge[bend left=10]
      node[align=center, label={[label distance=0.0cm]-30:$\textcolor{gray}{\updmin{\textcolor{obsColor}{1},\textcolor{mainPosColor}{3}},0,0,0,0,0}$}] {$p' = p$\\$Q'=Q_* \neq \varnothing$} (AttObs);
\end{tikzpicture}
